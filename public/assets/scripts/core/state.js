...
const DATA_CACHE_VERSION='2026-06-03-schedule-network-only-v1';
const DATASETS_EXCLUDED_FROM_CACHE=new Set(['leads','leadFollowups','students','schedule','packages','purchases','entitlements','entitlementLedger']);
...
function globalTopFilterPages(){
  return ['students','leads','schedule','admin-users','coaches','finance'];
}
...
function renderGlobalTopFilters(){
  ...
  const timeMenu=globalDateFilterQuickOptions().map(label=>`<div class="tms-dropdown-item ${((label==='自定义'&&globalDateRangeFilterValue==='自定义')||label===globalDateRangeFilterValue)?'active':''}" data-value="${esc(label)}" onclick="setGlobalDateRangeFilter(${jsArg(label)},event)">${esc(label)}</div>`).join('');
  const dateMenuClass=`court-top-date-menu ${globalDateRangeFilterValue==='自定义'?'is-custom':'is-quick'}`;
  const dateMenu=globalDateRangeFilterValue==='自定义'
...
function renderCurrentGlobalFilterPage(){
  ...
  if(currentPage==='admin-users')renderAdminUsers();
  if(currentPage==='coaches')renderCoaches();
...
function setGlobalDateRangeFilter(value,event){
  ...
  if(globalDateRangeFilterValue==='自定义'){
    if(!globalDateRangeStart&&!globalDateRangeEnd&&typeof resolveCourtDatePresetRange==='function'){
