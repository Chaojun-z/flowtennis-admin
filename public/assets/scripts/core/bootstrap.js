...
let courtSortKey='lastBookingDate',courtSortDir='desc',stuSortKey='lastLesson',stuSortDir='desc',leadSortKey='',leadSortDir='',purchaseSortDir='desc',courtOwnerFilterValue='',courtAccountTypeFilterValue='',courtCampusFilterValue='',courtDateRangeFilterValue=globalDateRangeFilterValue,courtDateRangeStart=globalDateRangeStart,courtDateRangeEnd=globalDateRangeEnd,leadPageSize=15,stuPageSize=15,schPageSize=15,courtPageSize=15,purPageSize=15,pkgPageSize=15,pricePageSize=15,financeLedgerPageSize=15,financeRevenuePageSize=15,financeRecognizedPageSize=15,adminUserPageSize=15,selectedCourtIds=new Set(),courtBatchMode=false;
...
function setCampus(el,c){document.querySelectorAll('.ctab').forEach(b=>b.classList.remove('active'));if(el)el.classList.add('active');campus=c;localStorage.setItem(CAMPUS_KEY,campus);if(typeof resetAllStudentListPages==='function')resetAllStudentListPages();else stuPage=standardListFirstPage();leadPage=standardListFirstPage();schPage=standardListFirstPage();courtPage=standardListFirstPage();purPage=standardListFirstPage();pkgPage=standardListFirstPage();pricePage=standardListFirstPage();financeLedgerPage=standardListFirstPage();financeRevenuePage=standardListFirstPage();financeRecognizedPage=standardListFirstPage();adminUserPage=standardListFirstPage();membershipOrderAuditPage=standardListFirstPage();membershipLedgerAuditPage=standardListFirstPage();refreshGlobalTopFilters();if(isStudentListPage(currentPage))renderStudents();if(currentPage==='leads')renderLeads();if(currentPage==='operations')reloadOperationsPageDataWithInlineLoading();if(currentPage==='schedule')renderSchedule();if(currentPage==='coachschedule'||currentPage==='coachops')renderCoachOps();if(currentPage==='courts')renderCourts();if(currentPage==='finance')renderFinanceCenter();if(currentPage==='matches')renderMatches();if(currentPage==='admin-users')renderAdminUsers();if(currentPage==='coaches')renderCoaches();if(currentPage==='packages')renderPackages();if(currentPage==='purchases')renderPurchases();if(currentPage==='membership-orders')renderMembershipOrdersAuditPage();if(currentPage==='membership-ledger')renderMembershipLedgerAuditPage();if(currentPage==='prices')renderPrices();}
// ===== 教练管理 =====
// ===== 删除 & 通用 =====
...
function confirmDel(id,name,type){delId=id;delType=type;document.getElementById('confTitle').textContent=type==='court'||type==='lead'||type==='student'?'确认删除/隐藏？':'确认删除？';document.getElementById('confIcon').textContent='!';document.getElementById('confDesc').textContent=type==='court'?'即将处理「'+name+'」。没有财务/会员记录会删除；已有记录会隐藏保留数据。请输入「确认删除」。':type==='lead'?'即将处理线索「'+name+'」。未产生成交或业务关联的线索会删除；已经关联学员、订场用户、会员账户或已成交的线索会作废隐藏，历史数据会保留。请输入「确认删除」。':type==='membership-plan'?'即将删除「'+name+'」。仅草稿/停售且没有购买记录的方案可删除。请输入「确认删除」。':type==='student'?'即将处理学员「'+name+'」。学员会隐藏归档；课包、上课、权益、财务和其他历史数据都会保留。请输入「确认删除」。':'即将删除「'+name+'」，请输入「确认删除」。';document.getElementById('confOv').classList.add('open');var ci=document.getElementById('confInput');ci.style.display='block';ci.value='';var cb=document.getElementById('confYesBtn');cb.textContent='确认删除';cb.style.background='#dc2626';cb.classList.remove('neutral');cb.onclick=doDelete;cb.disabled=true;cb.style.opacity='0.4';cb.style.cursor='not-allowed';var nb=document.getElementById('confNoBtn');if(nb)nb.onclick=closeConf;ci.oninput=function(){if(ci.value.trim()==='确认删除'){cb.disabled=false;cb.style.opacity='1';cb.style.cursor='pointer';}else{cb.disabled=true;cb.style.opacity='0.4';cb.style.cursor='not-allowed';}};}
