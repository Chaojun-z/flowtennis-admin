...
function goPage(pg,el,skipRender=false){
  ...
  const updateDOM = () => {
    ...
    document.getElementById('campusTabs').style.display=['students','leads','schedule','coachops','courts','finance','matches','admin-users','coaches','packages','purchases'].includes(pg)?'flex':'none';
...
function setCampus(el,c){document.querySelectorAll('.ctab').forEach(b=>b.classList.remove('active'));el.classList.add('active');campus=c;localStorage.setItem(CAMPUS_KEY,campus);stuPage=1;leadPage=1;schPage=1;courtPage=1;financeRevenuePage=1;adminUserPage=1;if(currentPage==='students')renderStudents();if(currentPage==='leads')renderLeads();if(currentPage==='schedule')renderSchedule();if(currentPage==='coachops')renderCoachOps();if(currentPage==='courts')renderCourts();if(currentPage==='finance')renderFinanceCenter();if(currentPage==='matches')renderMatches();if(currentPage==='admin-users')renderAdminUsers();if(currentPage==='coaches')renderCoaches();if(currentPage==='packages')renderPackages();}
// ===== 教练管理 =====
// ===== 删除 & 通用 =====
