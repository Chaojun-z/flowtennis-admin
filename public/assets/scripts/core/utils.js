...
function studentEntitlementSummaryHtml(stu){
  ...
  return rows.map(row=>{
    ...
    return `<div class="student-package-card${depleted?' is-depleted':''}"><div class="student-package-icon"><svg class="student-package-icon-svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><path d="M3.5 8.5V6.8c0-.77.63-1.4 1.4-1.4h4.3l1.9 2h8.4c.77 0 1.4.63 1.4 1.4v8.7c0 .77-.63 1.4-1.4 1.4H4.9c-.77 0-1.4-.63-1.4-1.4z"/><path d="M3.5 10h17"/><path d="M7.2 5.4h3.1"/></svg></div><div class="student-package-main"><div class="student-package-head"><div class="student-package-title">[${esc(renderStandardEmptyText(packageText))}]</div>${entitlement?.id?studentManualEntitlementActionsHtml(entitlement):''}</div><div class="student-package-meta"><span>${esc(renderStandardEmptyText(purchaseDate))} 报名</span><span>应付${fmt(systemAmount)} · 实付${fmt(paidAmount)}</span><span>${esc(renderStandardEmptyText(ownerCoach))}</span><strong>已扣 ${lessonQty(used)}${unit}（${lessonQty(remaining)}/${lessonQty(total)}${unit}）</strong><span class="student-package-status">${esc(statusText)}</span></div></div></div>`;
  }).join('');
}
