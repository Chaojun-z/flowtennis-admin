...
function isStoredValuePayMethod(value){
  return String(value||'').trim()==='储值扣款'||String(value||'').trim()==='储值卡';
}
...
function standardPackageLabel(p={},includeStatus=false){
  ...
  if(courseType==='专项课'){const title=specialCourseDisplayTitle(p),parts=['专项课',title,lessons?`${lessons}${unit}`:'',packageTimeBandShortLabel(p.timeBand||p.packageTimeBand||'全天')].filter(Boolean),status=includeStatus?packageStatusText(p):'';if(status)parts.push(status);return parts.join(' · ');}
...
function coachOpsDateLabel(){
  ...
  if(coachOpsMode==='week'){
    const s=dayStart(d),e=addDays(s,6);
...
function membershipPlanSaleWindowText(plan){
  if(!plan)return '-';
  return [renderStandardEmptyText(plan.saleStartDate||''),renderStandardEmptyText(plan.saleEndDate||'')].join(' ~ ');
}
...
function membershipPlanPreviewHtml(input={}){
  ...
  return `<div style="font-size:12px;color:var(--tb);line-height:1.7"><div><strong style="color:var(--th)">${esc(input.name||'未命名方案')}</strong></div><div>会员档位：${esc(renderStandardEmptyText(input.tierCode||input.name||''))}</div><div>充值金额：¥${fmt(parseFloat(input.rechargeAmount)||0)}</div><div>赠送金额：¥${fmt(parseFloat(input.bonusAmount)||0)}</div><div>折扣：${esc(membershipDiscountText(input.discountRate)||'—')}</div><div>售卖时间：${esc(membershipPlanSaleWindowText(input))}</div><div>方案状态：${esc(statusMeta.text)}</div><div>赠送权益：${esc(benefitLines)}</div></div>`;
}
...
function studentPackageLessonMiniBar(stu){
  const meta=studentPackageLessonMeta(stu);
  if(!meta.hasPackage)return renderStandardCellText('-',false);
...
function studentManualEntitlementActionsHtml(entitlement={}){
  const purchase=purchases.find(p=>p.id===entitlement.purchaseId)||{};
...
function studentEntitlementSummaryHtml(stu){
  ...
  return rows.map(row=>{
    const purchaseDate=String(row.purchaseDate||'').slice(0,10);
    const used=Number(row.usedLessons)||0,total=Number(row.totalLessons)||0,remaining=Number(row.remainingLessons)||0,ownerCoach=coachName(row.ownerCoach||''),packageText=row.packageName||'课包',paidAmount=Number(row.paidAmount)||0,statusText=row.statusText||'正常',depleted=remaining<=0||statusText==='已用完',unit=row.unit||'节';
    const matchedEntitlement=row.entitlementId?entitlements.find(item=>String(item.id||'')===String(row.entitlementId)):null;
    const entitlement=matchedEntitlement||((row.entitlementId)?{...row,id:row.entitlementId,studentId:row.studentId||stu?.id,purchaseId:row.purchaseId,status:statusText==='已作废'?'voided':(depleted?'depleted':'active'),remainingLessons:remaining,usedLessons:used,totalLessons:total}:{});
    const priceText=`实付 ¥${fmt(paidAmount)}`;
    const packageRecordKey=row.packageRecordKey||(row.entitlementId?`ent:${row.entitlementId}`:(row.purchaseId?`pur:${row.purchaseId}`:''));
    const recordAction=typeof setStudentLessonRecordPackageFilter==='function'?`<button type="button" class="student-package-action-link" onclick="setStudentLessonRecordPackageFilter(${jsArg(stu.id||stu.studentId)},${jsArg(packageRecordKey)},${jsArg(row.entitlementId||'')},${jsArg(row.purchaseId||'')},${jsArg(packageText)})">查看记录</button>`:'';
    const manualActionHtml=entitlement?.id?studentManualEntitlementActionsHtml(entitlement):'';
    const actionHtml=[recordAction,manualActionHtml].filter(Boolean).join('');
    return `<div class="student-package-card${depleted?' is-depleted':''}"><div class="student-package-icon"><svg class="student-package-icon-svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><path d="M3.5 8.5V6.8c0-.77.63-1.4 1.4-1.4h4.3l1.9 2h8.4c.77 0 1.4.63 1.4 1.4v8.7c0 .77-.63 1.4-1.4 1.4H4.9c-.77 0-1.4-.63-1.4-1.4z"/><path d="M3.5 10h17"/><path d="M7.2 5.4h3.1"/></svg></div><div class="student-package-main"><div class="student-package-head"><div class="student-package-title">${esc(renderStandardEmptyText(packageText))}</div><div class="student-package-actions"><span class="student-package-status">${esc(statusText)}</span>${actionHtml}</div></div><div class="student-package-meta"><span>已上 ${lessonQty(used)}${unit}</span><span>剩余 ${lessonQty(remaining)}${unit}</span><span>${esc(renderStandardEmptyText(purchaseDate))} 报名</span><span>${esc(priceText)}</span><span>${esc(renderStandardEmptyText(ownerCoach))}</span></div></div></div>`;
  }).join('');
}
...
function studentEntitlementLedgerLineHtml(row,ent={}){
  ...
  const line=[
    studentEntitlementLedgerDeltaText(row,ent,schedule),
  ...
  ].map(item=>esc(renderStandardEmptyText(item))).join(' · ');
...
function studentEntitlementLedgerDeltaText(row,ent={},schedule=null){
  const unit=packageBalanceUnitLabel({...ent,...row,packageName:ent.packageName||row?.packageName||''});
  const delta=`${lessonQty(Number(row.lessonDelta)||0)}${unit}`;
  if(unit!=='次'||Number(row.lessonDelta)>=0)return delta;
  return [delta,studentLessonDurationText(schedule)].filter(Boolean).join(' · ');
}
function studentLedgerBalanceAfter(row,ent={}){
  ...
  const unit=packageBalanceUnitLabel({...ent,...row,packageName:ent.packageName||row?.packageName||''});
  return `剩余 ${lessonQty(balance)}/${lessonQty(ent.totalLessons)} ${unit}`;
}
...
function studentLessonDurationText(schedule=null){
  const duration=schedule?.startTime?scheduleDurationLessonUnits(schedule):0;
  return duration?`${lessonQty(duration)}小时`:'';
}
function studentLedgerConsumedText(row={},ent={},schedule=null){
  const unit=packageBalanceUnitLabel({...ent,...row,packageName:ent.packageName||row?.packageName||''});
  if(unit==='次'){
    const count=`${lessonQty(Math.abs(Number(row.lessonDelta)||0))}次`;
    return [count,studentLessonDurationText(schedule)].filter(Boolean).join(' · ');
  }
  return `${lessonQty(studentLedgerLessonUnits(row,schedule))}节`;
}
function studentLessonRecordChargeHtml(row,ent={},balance=null,schedule=null){
  ...
  if(ent?.id){
    const unit=packageBalanceUnitLabel({...ent,...row,packageName:ent.packageName||row?.packageName||''});
    return `<strong>扣${studentLedgerConsumedText(row,ent,schedule)}（${lessonQty(balance?.remaining)}/${lessonQty(balance?.total)}${unit}）</strong>`;
  }
...
function studentLessonRecordPackageText(row,ent={}){
  ...
  return [
    ...
    studentEntitlementLedgerLocationText(row,schedule,ent),
    coachName(schedule.coach||row?.coach||ent?.ownerCoach||''),
    Number(row.lessonDelta)<0?`扣${studentLedgerConsumedText(row,ent,schedule)}`:(payMethodText?payMethodText:'课次'),
...
function studentLessonRecordLedgerText(row,ent={}){
  ...
  const schedule=findScheduleForEntitlementLedgerRow(row,student);
  const unit=packageBalanceUnitLabel({...ent,...row,packageName:ent.packageName||row?.packageName||''});
  const balance=ent?.id?`剩余 ${lessonQty(ent.remainingLessons)}/${lessonQty(ent.totalLessons)} ${unit}`:'';
  return [
    studentEntitlementLedgerTimeText(row,schedule),
    studentEntitlementLedgerLocationText(row,schedule,ent),
    coachName(schedule.coach||row?.coach||ent?.ownerCoach||''),
    `扣${studentLedgerConsumedText(row,ent,schedule)}`,
    balance,
...
function courtFinanceLocal(c){
  ...
  hist.forEach(h=>{
    ...
    else if(h.type==='消费'){if(isInternal)return;t.spentAmount+=amount;if(isStoredValuePayMethod(h.payMethod)){t.storedValueSpent+=amount;t.balance-=amount;}else{t.directPaidSpent+=amount;t.receivedAmount+=amount;}}
    else if(h.type==='退款'){if(h.payMethod==='储值退款')t.balance-=amount;t.receivedAmount-=amount;}
    else if(h.type==='冲正'){t.spentAmount-=amount;if(isStoredValuePayMethod(h.payMethod)){t.storedValueSpent-=amount;t.balance+=amount;}else{t.directPaidSpent-=amount;t.receivedAmount-=amount;}}
  });
...
function courtFinanceConfirmText(h,studentId){
  ...
  if(h.type==='充值')return `确认充值 ¥${fmt(h.amount)}？这笔钱会进入当前余额，以后订场可选择“储值卡”使用${target}。`;
  if(h.type==='退款')return h.payMethod==='储值退款'?`确认从储值余额退款 ¥${fmt(h.amount)}？会减少当前余额${target}。`:`确认退款 ¥${fmt(h.amount)}？用于记录已退回的单次付款${target}。`;
  if(h.type==='冲正')return isStoredValuePayMethod(h.payMethod)?`确认冲正 ¥${fmt(h.amount)}？用于撤回一笔录错的储值卡消费，余额会加回${target}。`:`确认冲正 ¥${fmt(h.amount)}？用于撤回一笔录错的单次支付消费${target}。`;
  if(String(h.category||'').includes('内部占用'))return `确认记录内部占用？该记录只占用场地时间，不计入累计消费和累计实收${h.internalReason?`，原因：${h.internalReason}`:''}。`;
  if(isStoredValuePayMethod(h.payMethod))return `确认用储值卡支付 ¥${fmt(h.amount)}？会从当前余额扣除${target}。`;
