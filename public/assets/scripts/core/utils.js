...
function studentEntitlementSummaryHtml(stu){
  ...
  return rows.map(row=>{
    ...
    const recordAction=typeof setStudentLessonRecordPackageFilter==='function'?`<button type="button" class="student-package-action-link" onclick="setStudentLessonRecordPackageFilter(${jsArg(stu.id||stu.studentId)},${jsArg(row.entitlementId||'')},${jsArg(row.purchaseId||'')},${jsArg(packageText)})">查看记录</button>`:'';
