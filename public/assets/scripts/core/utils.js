...
function studentEntitlementSummaryHtml(stu){
  ...
  return rows.map(row=>{
    ...
    const used=Number(row.usedLessons)||0,total=Number(row.totalLessons)||0,remaining=Number(row.remainingLessons)||0,ownerCoach=coachName(row.ownerCoach||''),packageText=row.packageName||'课包',systemAmount=Number(row.systemAmount)||0,paidAmount=Number(row.paidAmount)||0,statusText=row.statusText||'正常',depleted=remaining<=0||statusText==='已用完',unit=row.unit||'节',entitlement=row.entitlementId?entitlements.find(item=>String(item.id||'')===String(row.entitlementId)):{};
    const priceText=systemAmount&&systemAmount!==paidAmount?`实付 ¥${fmt(paidAmount)}（应付 ¥${fmt(systemAmount)}）`:`实付 ¥${fmt(paidAmount)}`;
