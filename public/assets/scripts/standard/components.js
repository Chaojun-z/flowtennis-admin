...
function renderStandardDropdownHtml(id,label,options,value,isForm=false,onchange=''){
  ...
  return `<div class="tms-dropdown ${isForm?'tms-dropdown-form ':''}${isPageSize?'tms-page-size-dropdown ':''}${hasValue?'has-value':''}" id="${id}_dropdown" data-target="${id}" data-label="${esc(label)}" data-onchange="${onchange}"><input type="hidden" id="${id}" value="${esc(active?.value||value||'')}"><div class="tms-dropdown-display" onclick="toggleStandardDropdown('${id}',event)">${esc(displayLabel)}</div><div class="tms-dropdown-menu" onclick="event.stopPropagation()" style="touch-action:pan-y;-webkit-overflow-scrolling:touch" onwheel="event.stopPropagation();event.preventDefault();this.scrollTop += event.deltaY" ontouchmove="event.stopPropagation()">${list.map(opt=>{const optionLabel=renderStandardOptionLabel(opt);const isActive=active&&String(opt.value)===String(active.value);return `<div class="tms-dropdown-item ${isActive?'active':''} ${opt.disabled?'disabled':''}" data-value="${esc(opt.value??'')}" aria-disabled="${opt.disabled?'true':'false'}" onclick="${opt.disabled?'event.stopPropagation()':`selectStandardDropdownItem('${id}',${jsArg(opt.value)},${jsArg(optionLabel)},event)`}">${isPageSize?checkIcon:''}<span>${esc(optionLabel)}</span></div>`;}).join('')}</div></div>`;
}
function renderStandardSearchableDropdownHtml(config={}){
  ...
  return `<div class="tms-dropdown tms-searchable-dropdown ${config.isForm?'tms-dropdown-form ':''}${hasValue?'has-value':''}" id="${id}_dropdown" data-target="${id}" data-label="${esc(label)}" data-onchange="${esc(onchange)}"><input type="hidden" id="${id}" value="${esc(active?.value||value||'')}"><div class="tms-dropdown-display" onclick="toggleStandardDropdown('${id}',event)">${esc(displayLabel)}</div><div class="tms-dropdown-menu tms-searchable-dropdown-menu" onclick="event.stopPropagation()" style="touch-action:pan-y;-webkit-overflow-scrolling:touch" onwheel="event.stopPropagation();event.preventDefault();const o=this.querySelector('.tms-dropdown-options');if(o)o.scrollTop += event.deltaY" ontouchmove="event.stopPropagation()"><div class="tms-dropdown-search-row" onclick="event.stopPropagation()"><input class="tms-dropdown-search-input" id="${id}_search" placeholder="${esc(searchPlaceholder)}" oninput="filterStandardSearchableDropdown('${id}',this.value)" onkeydown="event.stopPropagation()"></div><div class="tms-dropdown-options">${itemsHtml}</div><div class="tms-dropdown-empty">${esc(emptyText)}</div></div></div>`;
}
...
function closeStandardDropdownElement(el){
  if(!el)return;
  const search=el.querySelector('.tms-dropdown-search-input');
...
function toggleStandardDropdown(id,event){
  ...
  document.querySelectorAll('.tms-dropdown.open').forEach(el=>{
    if(el!==dropdown){
      closeStandardDropdownElement(el);
    }
  });
  dropdown.classList.toggle('open');
...
function selectStandardDropdownItem(id,value,label,event){
  ...
  if(dropdown){
    ...
    closeStandardDropdownElement(dropdown);
...
function standardListPageShellConfigs(){
  return [
    {key:'leads',statsId:'leadStatsRow',toolbar:{search:{id:'leadSearch',oninput:'applyLeadSearch()'},filterHostIds:['leadSourceFilterHost','leadCustomerTypeFilterHost','leadConsultFilterHost','leadStageFilterHost','leadDealTypeFilterHost','leadOwnerFilterHost'],actionsHtml:'<span class="tms-import-action" onclick="openLeadImportPreviewModal()">导入</span><button class="tms-btn tms-btn-primary" onclick="openLeadCreateDrawer()">新增线索</button>'},table:{bodyId:'leadTbody',pager:{infoId:'leadPagerInfo',pageSizeId:'leadPageSize',buttonsId:'leadPagerBtns'},columns:[{label:'姓名',className:'tms-sticky-l',style:'width:130px;padding-left:20px'},{style:'width:120px',html:'<button class="tms-sort-header" data-lead-sort="leadDate" onclick="cycleLeadSort(\'leadDate\')">线索时间<span class="tms-sort-icon"><span class="tms-sort-up"></span><span class="tms-sort-down"></span></span></button>'},{label:'来源',style:'width:110px'},{label:'类型',style:'width:90px'},{label:'需求产品',style:'width:110px'},{label:'水平',style:'width:80px'},{label:'基本信息',style:'width:280px'},{label:'线索阶段',style:'width:140px'},{label:'意向等级',style:'width:90px'},{label:'跟进优先级',style:'width:100px'},{label:'跟进人',style:'width:100px'},{style:'width:220px',html:'<button class="tms-sort-header" data-lead-sort="trialLessonAt" onclick="cycleLeadSort(\'trialLessonAt\')">体验课时间<span class="tms-sort-icon"><span class="tms-sort-up"></span><span class="tms-sort-down"></span></span></button>'},{label:'成交教练',style:'width:110px'},{label:'流失原因',style:'width:170px'},{label:'操作',className:'tms-sticky-r',style:'width:90px;padding-right:20px;text-align:right'}]},afterHtml:'<div id="leadMobileCards" class="admin-h5-card-list"></div>'},
    {key:'students',statsId:'studentStatsRow',toolbar:{search:{id:'stuSearch',oninput:'onStudentFilterChange()'},filterHostIds:['stuTypeFilterHost','stuSourceFilterHost','stuCourseTypeFilterHost','stuTagFilterHost','stuCoachFilterHost'],actionsHtml:'<span class="tms-export-action" onclick="exportStudentCSV()">导出</span><button class="tms-btn tms-btn-primary" onclick="openStudentModal(null)">添加学员</button>'},table:{bodyId:'stuTbody',pager:{infoId:'stuPagerInfo',pageSizeId:'stuPageSize',buttonsId:'stuPagerBtns'},columns:[{label:'姓名',className:'tms-sticky-l',style:'width:130px;padding-left:20px'},{label:'来源',style:'width:110px'},{label:'类型',style:'width:90px'},{label:'校区',style:'width:105px'},{label:'活跃状态',style:'width:120px'},{label:'付费方式',style:'width:120px'},{label:'课包状态',style:'width:120px'},{label:'课包余额',style:'width:110px'},{label:'历史课时',style:'width:110px'},{label:'学员状态',style:'width:120px'},{label:'负责教练',style:'width:110px'},{label:'备注',style:'width:280px'},{label:'操作',className:'tms-sticky-r',style:'width:90px;padding-right:20px;text-align:right'}]},afterHtml:'<div id="studentMobileCards" class="admin-h5-card-list"></div>'},
    {key:'schedule',toolbar:{search:{id:'schSearch',oninput:'onScheduleFilterChange()'},filterHostIds:['schCourseTypeFilterHost','schCoachFilterHost','schProposalFilterHost','schFeedbackFilterHost','schStatusFilterHost'],actionsHtml:'<button class="tms-btn tms-btn-primary" onclick="openScheduleModal(null)">添加排课</button>'},table:{bodyId:'schTbody',pager:{infoId:'schPagerInfo',pageSizeId:'schPageSize',buttonsId:'schPagerBtns',pageSizeTag:'span'},columns:[{label:'日期',className:'tms-sticky-l',style:'width:84px;padding-left:14px'},{label:'上课时间',style:'width:92px'},{label:'时长',style:'width:52px'},{label:'校区/场地',style:'width:125px'},{label:'教练',style:'width:68px'},{label:'学员',style:'width:120px'},{label:'课程类型',style:'width:100px'},{label:'课前教案',style:'width:72px'},{label:'课后反馈',style:'width:72px'},{label:'重复?',style:'width:64px'},{label:'状态',style:'width:68px'},{label:'操作',className:'tms-sticky-r',style:'width:118px;padding-right:10px;text-align:right'}]},afterHtml:'<div id="scheduleMobileCards" class="admin-h5-card-list"></div>'},
    {key:'admin-users',toolbar:{search:{id:'adminUserSearch',oninput:'onAdminUserFilterChange()'},actionsHtml:'<button class="tms-btn tms-btn-primary" onclick="openAdminUserModal(null)">新增账号</button>'},table:{bodyId:'adminUserTbody',pager:{infoId:'adminUserPagerInfo',pageSizeId:'adminUserPageSize',buttonsId:'adminUserPagerBtns'},columns:[{label:'账号名',className:'tms-sticky-l',style:'width:130px;padding-left:20px'},{label:'姓名',style:'width:100px'},{label:'手机号',style:'width:112px'},{label:'角色',style:'width:90px'},{label:'绑定教练',style:'width:100px'},{label:'微信绑定',style:'width:130px'},{label:'服务号绑定',style:'width:130px'},{label:'状态',style:'width:82px'},{label:'权限说明',style:'width:260px'},{label:'操作',className:'tms-sticky-r',style:'width:170px;padding-right:20px;text-align:right'}]},afterHtml:'<div id="adminUserMobileCards" class="admin-h5-card-list"></div>'},
    {key:'packages',containerClass:'course-package-showcase',toolbar:{toolbarClass:'course-package-showcase-toolbar',filterClass:'course-package-showcase-filters',actionsClass:'course-package-showcase-actions',search:{id:'pkgSearch',oninput:'onPackageFilterChange()'},filterHostIds:['pkgTypeFilterHost','pkgAudienceFilterHost','pkgCoachFilterHost','pkgStatusFilterHost','pkgTimeBandFilterHost'],actionsHtml:'<button class="tms-btn tms-btn-ghost" onclick="goPage(\'purchases\')">购买记录</button><button class="tms-btn tms-btn-ghost" onclick="openPackageMergeModal()">合并课包</button><button class="tms-btn tms-btn-primary" onclick="openPackageModal(null)">创建课包</button>'},bodyHtml:'<div class="course-package-showcase-grid" id="packageGrid"></div>'},
    {key:'purchases',toolbar:{search:{id:'purSearch',oninput:'onPurchaseSearchChange()'},filterHostIds:['purCourseTypeFilterHost','purInPeriodFilterHost','purPayStatusFilterHost'],actionsHtml:'<button class="tms-btn tms-btn-primary" onclick="openPurchaseEntryModal()">课包购买</button>'},table:{bodyId:'purchaseTbody',pager:{infoId:'purPagerInfo',pageSizeId:'purPageSize',buttonsId:'purPagerBtns'},columns:[{label:'姓名',style:'width:120px;padding-left:20px'},{style:'width:100px',html:'<button class="tms-sort-header" data-purchase-sort="purchaseDate" onclick="cyclePurchaseSort()">报名时间<span class="tms-sort-icon"><span class="tms-sort-up"></span><span class="tms-sort-down"></span></span></button>'},{label:'课程类型',style:'width:110px'},{label:'用户类型',style:'width:64px'},{label:'上课人数',style:'width:86px'},{label:'在期状态',style:'width:108px'},{label:'付费状态',style:'width:74px'},{label:'报名课时',style:'width:74px'},{label:'报名费用',style:'width:84px'},{label:'归属教练',style:'width:90px'},{label:'赠送课时',style:'width:74px'},{label:'课包余额',style:'width:92px'},{label:'操作',className:'tms-sticky-r',style:'width:108px;padding-right:20px;text-align:right'}]},afterHtml:'<div id="purchaseMobileCards" class="admin-h5-card-list"></div>'},
    {key:'prices',noteHtml:'价格在这里统一维护。订场和渠道核销会自动引用价格，并在流水里保存当时的价格快照。',toolbar:{search:{id:'priceSearch',oninput:'onPriceFilterChange()'},filterHostIds:['priceTypeFilterHost','priceProductTypeFilterHost'],actionsHtml:'<button class="tms-btn tms-btn-primary" onclick="openPriceModal()">新增价格</button>'},table:{bodyId:'priceTbody',tableClass:'price-table',pager:{infoId:'pricePagerInfo',pageSizeId:'pricePageSize',buttonsId:'pricePagerBtns'},columns:[{label:'类型',style:'width:66px;padding-left:12px'},{label:'渠道',style:'width:64px'},{label:'名称',style:'width:372px'},{label:'场地类型',style:'width:50px'},{label:'日期类型',style:'width:60px'},{label:'商品类型',style:'width:62px'},{label:'关联业务',style:'width:58px'},{label:'时间段',style:'width:88px'},{label:'时长',style:'width:58px'},{label:'价格',style:'width:54px'},{label:'状态',style:'width:52px'},{label:'操作',className:'tms-sticky-r',style:'width:88px;padding-right:12px;text-align:right'}]},afterHtml:'<div id="priceMobileCards" class="admin-h5-card-list"></div>'},
    {key:'coachops',toolbar:false,bodyHtml:'<div class="coach-ops-shell"><div class="coach-ops-toolbar"><div class="coach-ops-toolbar-main"><div id="coachOpsWorkloadRangeHost"></div><button class="coach-ops-nav coach-ops-arrow" onclick="shiftCoachOpsDate(-1)" title="上一段">‹</button><div class="coach-date-wrap"><input id="coachOpsWorkloadDate" type="hidden" value=""><button class="coach-date-btn" id="coachOpsWorkloadDateBtn" onclick="toggleCoachOpsPicker(event)" type="button"></button><div class="coach-date-pop" id="coachOpsWorkloadPicker"></div></div><button class="coach-ops-nav coach-ops-arrow" onclick="shiftCoachOpsDate(1)" title="下一段">›</button><button class="coach-ops-nav" onclick="setCoachOpsToday()">今天</button></div></div></div>'+renderStandardTableShellHtml({bodyId:'coachOpsTbody',columns:[{label:'教练',className:'tms-sticky-l',style:'width:120px;padding-left:20px'},{label:'当前筛选课数',style:'width:150px'},{label:'体验课转化率',style:'width:130px'},{label:'课程类型分布',style:'width:220px'},{label:'已反馈',style:'width:90px'},{label:'未反馈',style:'width:90px'},{label:'校区分布',style:'width:180px'},{label:'时间段',style:'width:140px'}]})+'<div id="coachOpsMobileCards" class="coach-ops-mobile-cards"></div>'},
    {key:'matches',statsId:'matchStatsRow',toolbar:{search:{id:'matchSearch',oninput:'renderMatches()'},filterHostIds:['matchStatusFilterHost'],actionsHtml:'<button class="tms-btn tms-btn-ghost" onclick="loadMatches(true)">刷新</button>'},table:{bodyId:'matchTbody',columns:[{label:'球局',style:'width:130px;padding-left:20px'},{label:'时间',style:'width:150px'},{label:'场地',style:'width:160px'},{label:'人数',style:'width:90px'},{label:'状态',style:'width:100px'},{label:'预计费用',style:'width:120px'},{label:'最终费用',style:'width:120px'},{label:'报名名单',style:'width:240px'},{label:'操作',className:'tms-sticky-r',style:'width:90px;padding-right:20px;text-align:right'}]},afterHtml:'<div id="matchMobileCards" class="admin-h5-card-list"></div>'},
    {key:'coaches',toolbar:{search:{id:'coachSearch',oninput:'renderCoaches()'},actionsHtml:'<button class="tms-btn tms-btn-primary" onclick="openCoachModal(null)">新增教练</button>'},table:{bodyId:'coachTbody',columns:[{label:'姓名',style:'width:16%;padding-left:20px'},{label:'电话',style:'width:16%'},{label:'校区',style:'width:14%'},{label:'入职时间',style:'width:12%'},{label:'状态',style:'width:10%'},{label:'备注',style:'width:18%'},{label:'操作',className:'tms-sticky-r',style:'width:180px;padding-right:20px;text-align:right'}]},afterHtml:'<div id="coachMobileCards" class="admin-h5-card-list"></div>'},
    {key:'courts',statsId:'courtStatsRow',toolbar:{search:{id:'courtSearch',oninput:'onCourtFilterChange()'},filterHostIds:['courtAccountTypeFilter','courtOwnerFilter','courtMoreActions'],filterHtmls:['<div class="court-batch-toolbar" id="courtBatchToolbar" style="display:none"><span class="court-batch-count" id="courtBatchCount">已选 0 条</span><button class="tms-btn tms-btn-danger" id="courtBatchDelBtn" onclick="batchDeleteCourts()" disabled>批量删除</button><button class="tms-btn tms-btn-ghost" id="courtBatchCancelBtn" onclick="setCourtBatchMode(false)">取消</button></div>'],actionsHtml:'<span class="tms-export-action" onclick="exportCourtCSV()">导出</span><button class="tms-btn tms-btn-primary" onclick="openCourtModal(null)">添加订场用户</button>'},table:{bodyId:'courtTbody',pager:{infoId:'courtPagerInfo',pageSizeId:'courtPageSize',buttonsId:'courtPagerBtns'},columns:[{className:'tms-sticky-l',style:'width:130px;padding-left:20px',html:'<label class="tms-th-checkbox"><input type="checkbox" class="tms-checkbox" id="courtSelectAll" onchange="toggleCourtPageSelection(this.checked)"><span>姓名</span></label>'},{label:'手机号',style:'width:124px'},{label:'校区',style:'width:100px'},{label:'账户类型',style:'width:100px'},{label:'会员类型',style:'width:100px'},{style:'width:112px',html:'<button class="tms-sort-header" data-court-sort="balance" onclick="setCourtSort(\'balance\')">会员余额<span class="tms-sort-icon"><span class="tms-sort-up"></span><span class="tms-sort-down"></span></span></button>'},{style:'width:170px',html:'<button class="tms-sort-header" data-court-sort="lastBookingDate" onclick="setCourtSort(\'lastBookingDate\')">最近订场<span class="tms-sort-icon"><span class="tms-sort-up"></span><span class="tms-sort-down"></span></span></button>'},{style:'width:92px',html:'<button class="tms-sort-header" data-court-sort="memberBookingCount" onclick="setCourtSort(\'memberBookingCount\')">会员订场<span class="tms-sort-icon"><span class="tms-sort-up"></span><span class="tms-sort-down"></span></span></button>'},{style:'width:92px',html:'<button class="tms-sort-header" data-court-sort="bookingCount" onclick="setCourtSort(\'bookingCount\')">累计订场<span class="tms-sort-icon"><span class="tms-sort-up"></span><span class="tms-sort-down"></span></span></button>'},{style:'width:112px',html:'<button class="tms-sort-header" data-court-sort="bookingAmount" onclick="setCourtSort(\'bookingAmount\')">累计消费<span class="tms-sort-icon"><span class="tms-sort-up"></span><span class="tms-sort-down"></span></span></button>'},{label:'跟进人',style:'width:100px'},{label:'储值态度',style:'width:118px'},{label:'备注',style:'width:190px'},{label:'操作',className:'tms-sticky-r',style:'width:90px;padding-right:20px;text-align:right'}]},afterHtml:'<div id="courtMobileCards" class="admin-h5-card-list"></div>'},
    {key:'memberships',statsId:'membershipStatsRow',toolbar:{search:{id:'membershipSearch',oninput:'onMembershipSearchChange()'},filterHostIds:['membershipTierFilter'],actionsClass:'tms-toolbar-right tms-toolbar-secondary-actions',actionsHtml:'<span class="tms-membership-audit-action tms-membership-audit-action-orders" onclick="goPage(\'membership-orders\')"><span>历史订单</span></span><span class="tms-membership-audit-action tms-membership-audit-action-ledger" onclick="goPage(\'membership-ledger\')"><span>权益消耗记录</span></span>'},table:{bodyId:'membershipTbody',pager:{infoId:'membershipPagerInfo',pageSizeId:'membershipPageSize',buttonsId:'membershipPagerBtns'},columns:[{label:'姓名',className:'tms-sticky-l',style:'width:130px;padding-left:20px'},{label:'手机号',style:'width:140px'},{label:'会员类型',style:'width:160px'},{style:'width:150px',html:'<button class="tms-sort-header" data-membership-sort="firstOpenDate" onclick="setMembershipSort(\'firstOpenDate\')">首次开卡时间<span class="tms-sort-icon"><span class="tms-sort-up"></span><span class="tms-sort-down"></span></span></button>'},{label:'续费次数',style:'width:90px'},{style:'width:120px',html:'<button class="tms-sort-header" data-membership-sort="balance" onclick="setMembershipSort(\'balance\')">会员余额<span class="tms-sort-icon"><span class="tms-sort-up"></span><span class="tms-sort-down"></span></span></button>'},{label:'当前折扣',style:'width:100px'},{style:'width:110px',html:'<button class="tms-sort-header" data-membership-sort="memberBookingCount" onclick="setMembershipSort(\'memberBookingCount\')">会员订场<span class="tms-sort-icon"><span class="tms-sort-up"></span><span class="tms-sort-down"></span></span></button>'},{style:'width:110px',html:'<button class="tms-sort-header" data-membership-sort="bookingCount" onclick="setMembershipSort(\'bookingCount\')">累计订场<span class="tms-sort-icon"><span class="tms-sort-up"></span><span class="tms-sort-down"></span></span></button>'},{label:'可用权益',style:'width:420px'},{label:'操作',className:'tms-sticky-r',style:'width:90px;padding-right:12px;text-align:right'}]},afterHtml:'<div id="membershipMobileCards" class="admin-h5-card-list"></div>'},
    {key:'membership-orders',noteHtml:'此页面仅用于审计与追溯，不用于日常操作。',toolbar:{search:{id:'membershipOrderAuditSearch',oninput:'onMembershipOrderAuditSearchChange()'}},table:{bodyId:'membershipOrdersAuditTbody',pager:{infoId:'membershipOrdersAuditPagerInfo',pageSizeId:'membershipOrdersAuditPageSize',buttonsId:'membershipOrdersAuditPagerBtns'},columns:[{label:'支付日期',style:'padding-left:20px;width:120px'},{label:'录入时间',style:'width:170px'},{label:'订场用户',style:'width:120px'},{label:'会员方案',style:'width:150px'},{label:'系统价',style:'width:100px'},{label:'成交价',style:'width:100px'},{label:'赠送金额',style:'width:110px'},{label:'折扣',style:'width:90px'},{label:'是否重置有效期',style:'width:120px'},{label:'改价原因',style:'width:140px'},{label:'当次权益摘要',style:'width:320px'},{label:'状态',style:'width:100px'}]},afterHtml:'<div id="membershipOrderMobileCards" class="admin-h5-card-list"></div>'},
    {key:'membership-ledger',noteHtml:'此页面仅用于审计与追溯，不用于日常操作。',toolbar:{search:{id:'membershipLedgerAuditSearch',oninput:'onMembershipLedgerAuditSearchChange()'}},table:{bodyId:'membershipLedgerAuditTbody',pager:{infoId:'membershipLedgerAuditPagerInfo',pageSizeId:'membershipLedgerAuditPageSize',buttonsId:'membershipLedgerAuditPagerBtns'},columns:[{label:'时间',style:'padding-left:20px;width:170px'},{label:'订场用户',style:'width:120px'},{label:'购买批次',style:'width:150px'},{label:'权益',style:'width:160px'},{label:'变动',style:'width:90px'},{label:'动作',style:'width:100px'},{label:'操作账号',style:'width:120px'},{label:'原因',style:'width:320px'}]},afterHtml:'<div id="membershipLedgerMobileCards" class="admin-h5-card-list"></div>'},
    {key:'membership-plans',toolbar:{search:{id:'membershipPlanSearch',oninput:'renderMembershipPlans()'},actionsHtml:'<button class="tms-btn tms-btn-primary" onclick="openMembershipPlanModal(null)">新增会员方案</button>'},table:{bodyId:'membershipPlanTbody',columns:[{label:'会员方案',className:'tms-sticky-l',style:'width:170px;padding-left:20px'},{label:'档位',style:'width:140px'},{label:'充值金额',style:'width:120px'},{label:'赠送金额',style:'width:120px'},{label:'折扣',style:'width:90px'},{label:'售卖时间',style:'width:180px'},{label:'方案状态',style:'width:120px'},{label:'赠送权益',style:'width:560px'},{label:'备注',style:'width:180px'},{label:'操作',className:'tms-sticky-r',style:'width:168px;padding-right:20px;text-align:right'}]},afterHtml:'<div id="membershipPlanMobileCards" class="admin-h5-card-list"></div>'},
    ...
    {key:'entitlements',noteHtml:'课包余额是学员当前可用课时余额。排课时实际消耗的是这里，不是直接消耗课程产品。',toolbar:{search:{id:'entSearch',oninput:'renderEntitlements()'},filterHostIds:['entStatusFilterHost']},table:{bodyId:'entitlementTbody',columns:[{label:'姓名',style:'width:150px;padding-left:20px'},{label:'课包',style:'width:180px'},{label:'课程类型',style:'width:120px'},{label:'课时余额',style:'width:120px'},{label:'有效期',style:'width:190px'},{label:'时段',style:'width:120px'},{label:'状态',className:'tms-sticky-r',style:'width:110px;padding-right:20px;text-align:right'}]},afterHtml:'<div id="entitlementMobileCards" class="admin-h5-card-list"></div>'},
    {key:'mystudents',statsId:'myStudentStats',toolbar:false,noteHtml:'这里查看你当前可见的学员，能区分自己负责的学员、代上过的学员，以及最近上课和课时进度。',table:{bodyId:'myStuTbody',tableClass:'desktop-only',columns:[{label:'姓名',style:'width:96px;padding-left:20px'},{label:'校区',style:'width:96px'},{label:'手机号',style:'width:148px'},{label:'类型',style:'width:62px'},{label:'所上班次',style:'width:108px'},{label:'负责教练',style:'width:96px'},{label:'归属教练',style:'width:96px'},{label:'累计上课',style:'width:72px'},{label:'最后上课',style:'width:140px'},{label:'课包进度',style:'width:84px'},{label:'剩余课时',style:'width:84px'},{label:'备注'},{label:'操作',style:'width:72px;padding-right:20px;text-align:right'}]},afterHtml:'<div id="myStudentMobileList" class="coach-mobile-only coach-mobile-list"></div>'},
