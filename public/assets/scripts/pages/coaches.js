...
function openCoachModal(id){
  ...
  const body=`<div class="tms-section-header" style="margin-top:0;">基础信息</div><div class="tms-form-row"><div class="tms-form-item"><label class="tms-form-label">姓名 *</label><input class="finput tms-form-control" id="co_name" value="${rv(c,'name')}"></div><div class="tms-form-item"><label class="tms-form-label">电话</label><input class="finput tms-form-control" id="co_phone" value="${rv(c,'phone')}"></div></div><div class="tms-form-row"><div class="tms-form-item"><label class="tms-form-label">校区</label>${renderCourtDropdownHtml('co_campus','校区',[{value:'',label:'不选择'},...campusOptions],rv(c,'campus'),true)}</div><div class="tms-form-item"><label class="tms-form-label">入职时间</label>${courtDateButtonHtml('co_hireDate',rv(c,'hireDate'),'入职日期')}</div><div class="tms-form-item"><label class="tms-form-label">状态</label>${renderCourtDropdownHtml('co_status','状态',statusOptions,rv(c,'status','active'),true)}</div></div><div class="tms-form-row" style="margin-bottom:0"><div class="tms-form-item full-width"><label class="tms-form-label">备注</label><textarea class="finput tms-form-control" id="co_notes">${esc(rv(c,'notes'))}</textarea></div></div>`;
