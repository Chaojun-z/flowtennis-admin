...
function renderMyStudents(){
  ...
  const rosterRows=myStudentRosterRows();
  const myStus=rosterRows.length?rosterRows:students.filter(s=>coachName(s.primaryCoach)===cn2||myStudentLessonCount(s,cn2)>0);
  const visibleStudentCount=rosterRows.length?(Number(studentRoster?.stats?.totalCount)||myStus.length):myStus.length;
  const ownerStudentCount=rosterRows.length?(Number(studentRoster?.stats?.ownedCount)||myStus.filter(s=>Array.isArray(s.relationLabels)&&s.relationLabels.includes('归属')).length):myStus.filter(s=>coachName(s.primaryCoach)===cn2).length;
  const substituteStudentCount=rosterRows.length?(Number(studentRoster?.stats?.substituteCount)||myStus.filter(s=>Array.isArray(s.relationLabels)&&s.relationLabels.includes('代课')).length):myStus.filter(s=>coachName(s.primaryCoach)!==cn2&&myStudentLessonCount(s,cn2)>0).length;
  ...
  document.getElementById('myStuTbody').innerHTML=myStus.length?myStus.map(s=>{
    ...
    const lastLesson={dateText:firstNonEmpty(s.lastClassText,myStudentLastLessonMeta(s,cn2).dateText)||'—',fullText:firstNonEmpty(s.detailRecentLessonText,myStudentLastLessonMeta(s,cn2).fullText)||'—'};
    const packageMeta={progress:firstNonEmpty(s.packageText,myStudentPackageProgressMeta(s).progress)||'-',remaining:firstNonEmpty(s.detailPackageProgressText,myStudentPackageProgressMeta(s).remaining)||'-'};
    ...
    const primaryCoach=firstNonEmpty(s.primaryCoach,s.formalCoach,studentPrimaryCoachText(s));
    const ownerCoach=firstNonEmpty(s.ownerCoach,myStudentOwnerCoachText(s));
    const relationLabel=firstNonEmpty(s.relationType,Array.isArray(s.relationLabels)?s.relationLabels.join(' / '):'',coachName(s.primaryCoach)===cn2?'负责学员':'代上学员');
    return `<tr><td style="padding-left:20px"><div class="tms-text-primary">${esc(s.name)}</div></td><td>${renderStandardCellText(cn(s.campus))}</td><td><div class="tms-text-primary" style="white-space:nowrap">${esc(renderStandardEmptyText(s.phone))}</div></td><td>${renderStandardCellText(s.type)}</td><td><div class="tms-text-remark" title="${esc(cb)}">${esc(cb)}</div></td><td>${renderStandardCellText(primaryCoach)}</td><td>${renderStandardCellText(ownerCoach)}</td><td>${renderStandardCellText(firstNonEmpty(s.cumulative,myStudentLessonCount(s,cn2)),false)}</td><td><div class="tms-text-remark" title="${esc(lastLesson.fullText)}">${esc(lastLesson.dateText)}</div></td><td>${renderStandardCellText(packageMeta.progress,false)}</td><td>${renderStandardCellText(packageMeta.remaining,false)}</td><td><div class="tms-text-remark" title="${esc(noteSummary)}">${esc(renderStandardEmptyText(noteSummary))}</div></td><td class="tms-action-cell" style="width:72px;padding-right:20px;text-align:right"><span class="tms-action-link" onclick="openMyStudentDetail('${s.id}')">查看</span></td></tr>`;
  ...
  if(mobile){
    mobile.innerHTML=myStus.length?`<div class="ios-list-group">${myStus.map(s=>{
      ...
      const lastLesson={dateText:firstNonEmpty(s.lastClassText,myStudentLastLessonMeta(s,cn2).dateText)||'—'};
      const packageMeta={progress:firstNonEmpty(s.packageText,myStudentPackageProgressMeta(s).progress)||'-',remaining:firstNonEmpty(s.detailPackageProgressText,myStudentPackageProgressMeta(s).remaining)||'-'};
      const relationLabel=firstNonEmpty(s.relationType,Array.isArray(s.relationLabels)?s.relationLabels.join(' / '):'',coachName(s.primaryCoach)===cn2?'负责学员':'代上学员');
      const tagColor=relationLabel==='负责学员'?'tms-tag-tier-gold':'tms-tag-tier-slate';
      return `<div class="ios-list-item" onclick="openMyStudentDetail('${s.id}')"><div class="ios-list-body"><div class="ios-list-title">${esc(renderStandardEmptyText(s.name))} <span class="tms-tag ${tagColor}" style="transform:scale(0.85);transform-origin:left center">${relationLabel}</span></div><div class="ios-list-sub">班次：${esc(classText)}<br>课包进度：${esc(packageMeta.progress)}，剩 ${esc(packageMeta.remaining)}时<br>最后上课：${esc(lastLesson.dateText)}</div></div><div class="ios-list-chevron"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m9 18 6-6-6-6"/></svg></div></div>`;
    }).join('')}</div>`:'';
  }
}
function openMyStudentDetail(id){
  const s=myStudentRosterRows().find(x=>x.id===id)||students.find(x=>x.id===id);if(!s)return;
...
function renderMyClasses(){
  ...
  document.getElementById('myClsTbody').innerHTML=myCls.length?myCls.map(c=>{
    ...
    return `<tr><td style="padding-left:20px"><div class="tms-text-primary">${esc(c.className)||'—'}</div><div class="tms-text-secondary">${esc(c.classNo)||''}</div></td><td>${renderStandardCellText(prod?prod.name:'—')}</td><td><div class="tms-text-remark" title="${names}">${names}</div></td><td><div class="prog-wrap"><div class="prog-track"><div class="prog-fill ${pc}" style="width:${pct}%"></div></div><span class="prog-txt">${ul}/${tl} 剩${rem}</span></div></td><td>${renderStandardCellText(`${c.startDate||'—'} ~ ${c.endDate||'—'}`,false)}</td><td><span class="tms-tag ${c.status==='已结课'?'tms-tag-green':c.status==='已取消'?'tms-tag-tier-slate':'tms-tag-tier-blue'}">${c.status||'已排班'}</span></td><td class="tms-action-cell" style="width:72px;padding-right:20px;text-align:right"><span class="tms-action-link" onclick="openMyClassDetail('${c.id}')">查看</span></td></tr>`;
  ...
  if(mobile){
    mobile.innerHTML=myCls.length?`<div class="ios-list-group">${myCls.map(c=>{
      ...
      return `<div class="ios-list-item" onclick="openMyClassDetail('${c.id}')"><div class="ios-list-body"><div class="ios-list-title">${esc(renderStandardEmptyText(c.className||c.classNo))} <span class="tms-tag ${ssc[c.status||'已排班']||'tms-tag-tier-slate'}" style="transform:scale(0.85);transform-origin:left center">${esc(renderStandardEmptyText(c.status||'已排班'))}</span></div><div class="ios-list-sub">课程：${esc(renderStandardEmptyText(prod?.name||c.productName))}<br>学员：${esc(studentText)}<br>进度：${ul}/${tl} 节，剩余 ${rem} 节</div></div><div class="ios-list-chevron"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m9 18 6-6-6-6"/></svg></div></div>`;
    }).join('')}</div>`:'';
  }
}
function openMyClassDetail(id){
  ...
  const footer=`<button class="tms-btn tms-btn-default" onclick="closeModal()">关闭</button>`;
  openStandardModal({title:'查看班次',bodyHtml:body,actionsHtml:footer,extraClass:'modal-tight'});
}
