...
function renderMyStudents(){
  ...
  document.getElementById('myStuTbody').innerHTML=myStus.length?myStus.map(s=>{
    ...
    return `<tr><td style="padding-left:20px"><div class="tms-text-primary">${esc(s.name)}</div></td><td>${renderStandardCellText(cn(s.campus))}</td><td><div class="tms-text-primary" style="white-space:nowrap">${esc(renderStandardEmptyText(s.phone))}</div></td><td>${renderStandardCellText(s.type)}</td><td><div class="tms-text-remark" title="${esc(cb)}">${esc(cb)}</div></td><td>${renderStandardCellText(primaryCoach)}</td><td>${renderStandardCellText(ownerCoach)}</td><td>${renderStandardCellText(myStudentLessonCount(s,cn2),false)}</td><td><div class="tms-text-remark" title="${esc(lastLesson.fullText)}">${esc(lastLesson.dateText)}</div></td><td>${renderStandardCellText(packageMeta.progress,false)}</td><td>${renderStandardCellText(packageMeta.remaining,false)}</td><td><div class="tms-text-remark" title="${esc(noteSummary)}">${esc(renderStandardEmptyText(noteSummary))}</div></td><td class="tms-action-cell" style="width:72px;padding-right:20px;text-align:right"><span class="tms-action-link" onclick="openMyStudentDetail('${s.id}')">查看</span></td></tr>`;
