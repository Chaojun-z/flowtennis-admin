...
function scheduleRepeatBadgeHtml(schedule){
  return schedule?.scheduleSource==='循环排课'?'<span class="tms-tag tms-tag-tier-slate">循环</span>':'';
}
function renderSchedule(){
  ...
  document.getElementById('schTbody').innerHTML=slice.length?slice.map(s=>{
    ...
    return `<tr><td style="padding-left:14px">${renderCourtCellText(dateText,false)}</td><td>${renderCourtCellText(timeText,false)}</td><td>${renderCourtCellText(scheduleDurationText(s),false)}</td><td><div class="tms-cell-text" title="${esc(s.externalNotes||scheduleLocationText(s))}">${esc(scheduleLocationText(s))}</div></td><td>${renderCourtCellText(s.coach,false)}</td><td><div class="tms-text-primary">${esc(scheduleListStudentSummary(s))}</div></td><td><div style="display:flex;gap:6px;align-items:center;flex-wrap:wrap"><span class="tms-tag ${productTypeTagClass(scheduleCourseType(s))}">${esc(scheduleCourseType(s))}</span>${scheduleRepeatBadgeHtml(s)}</div></td><td><span class="tms-action-link" onclick="openFeedbackModal('${s.id}')">${scheduleFeedbackStatusText(s)}</span></td><td><span class="tms-tag ${scheduleStatusTagClass(status)}">${scheduleStatusLabel(status)}</span>${status==='已取消'&&s.cancelReason?`<div class="tms-text-secondary" style="margin-top:6px">${esc(s.cancelReason)}</div>`:''}</td><td class="tms-sticky-r tms-action-cell schedule-action-cell"><span class="tms-action-link" onclick="openScheduleDetail('${s.id}')">查看</span><span class="tms-action-link" onclick="openScheduleModal('${s.id}')">编辑</span><span class="tms-action-link" onclick="openCancelScheduleModal('${s.id}')">取消</span>${scheduleCanDeleteMistake(s)?`<span class="tms-action-link" onclick="confirmDel('${s.id}','误建排课','schedule')">删除</span>`:''}</td></tr>`;
...
function toggleScheduleLocationType(){
  ...
  if(type==='own'){
    ...
    renderScheduleVenueField();
  }
  refreshSchEntitlementOptions();
}
function renderScheduleVenueField(){
  const host=document.getElementById('sch_venueHost');
  if(!host)return;
  const campus=document.getElementById('sch_campus')?.value||'';
  const currentValue=document.getElementById('sch_venue')?.value||'';
  host.innerHTML=venueFieldHtml('sch_venue','场地',campus,currentValue,true);
}
function onScheduleCampusChange(){
  renderScheduleVenueField();
  refreshSchEntitlementOptions();
}
...
function formatScheduleLocalDateTime(dt){
  if(!dt||Number.isNaN(dt.getTime()))return '';
  return `${localDateKey(dt)} ${String(dt.getHours()).padStart(2,'0')}:${String(dt.getMinutes()).padStart(2,'0')}`;
}
function scheduleConfirmRuleMeta(scheduleSource,startTime=''){
  ...
  return {days,label:`提前${days}天确认`,dueText:fmtDt(formatScheduleLocalDateTime(due))};
}
...
function refreshScheduleSettlementMode(){
  const gift=document.getElementById('sch_giftLesson');
  const late=document.getElementById('sch_coachLateFree');
  const lateWrap=document.getElementById('sch_lateFieldsWrap');
  if(gift?.checked&&late?.checked)late.checked=false;
  if(lateWrap)lateWrap.style.display=late?.checked?'':'none';
}
...
function buildRepeatScheduleSeeds(baseData){
  ...
  const makeShift=(raw,offset)=>{
    ...
    return formatScheduleLocalDateTime(dt);
  };
...
async function refreshSchEntitlementOptions(){
  ...
  if(!sel||!hint)return;
  if(document.getElementById('sch_giftLesson')?.checked){
    sel.innerHTML='<option value="">— 赠送上课，本次不扣课 —</option>';
    hint.textContent='已标记赠送上课，本次不会扣减课包余额。';
    return;
  }
  const ids=parseArr(document.getElementById('sch_stuIds')?.value||'[]');
  ...
  try{
    const res=await apiCall('POST','/entitlements/recommend',{scheduleId:editId||'',studentIds:ids,courseType:document.getElementById('sch_courseType')?.value||'',coach:document.getElementById('sch_coach')?.value||'',coachId:document.getElementById('sch_coach')?.value||'',campus:document.getElementById('sch_campus')?.value||'',startTime:startRaw,endTime:endRaw,lessonCount:parseFloat(document.getElementById('sch_lc')?.value)||1,status:document.getElementById('sch_status')?.value||'已排课'});
    sel.innerHTML=(res.options||[]).filter(x=>x.selectable).map(x=>`<option value="${x.entitlementId}"${(document.getElementById('sch_entitlement').dataset.keep||document.getElementById('sch_entitlement').value||'')===x.entitlementId?' selected':''}>${esc(x.packageName)} · 剩余${lessonUnitsText(x.remainingLessons)}/${lessonUnitsText(x.totalLessons)} · ${esc(x.timeBand)||'全天'} · 到期${esc(x.validUntil)||'—'}</option>`).join('')||'<option value="">— 无可用课包 —</option>';
...
function scheduleSaveConfirmText(data,selectedEntitlement){
  return [
    ...
    `消课时数：${lessonUnitsText(data.lessonCount||0)}`,
    `扣减课包：${data.giftLesson?'赠送上课，本次不扣课':data.studentIds.length>1?'系统按参与学员自动扣课':(selectedEntitlement?selectedEntitlement.packageName:'未选择可用课包，本次不会扣减课包余额')}`,
    data.giftLesson?'赠送上课：本节不扣学员课时，原因请写在备注里':'',
...
async function saveSchedule(){
  ...
  const giftLesson=!!document.getElementById('sch_giftLesson')?.checked;
  const coachLateFree=!!document.getElementById('sch_coachLateFree')?.checked;
  if(giftLesson&&coachLateFree){toast('赠送上课和教练迟到免费不能同时勾选','warn');return;}
  const lateReason=document.getElementById('sch_lateReason')?.value.trim()||'';
  if(coachLateFree&&!lateReason){toast('请填写迟到原因','warn');return;}
  const data={startTime,endTime,classId,studentIds,expectedStudentIds:expectedBase,absentStudentIds,studentName:scheduleStudentTextByIds(studentIds).replace(/（[^）]*）/g,''),courseType:selectedCourseType,isTrial:selectedCourseType==='体验课',coach,coachId:coach,locationType,venue,campus:campusValue,externalVenueName:locationType==='external'?externalVenueName:'',externalCourtName:locationType==='external'?externalCourtName:'',externalNotes:locationType==='external'?externalNotes:'',lessonCount:lc,status,entitlementId:studentIds.length===1&&!giftLesson?selectedEntitlementId:'',packageName:studentIds.length===1&&!giftLesson?(selectedEntitlement?.packageName||''):'',purchaseId:studentIds.length===1&&!giftLesson?(selectedEntitlement?.purchaseId||''):'',timeBand:studentIds.length===1&&!giftLesson?(selectedEntitlement?.timeBand||''):'',cancelReason,notifyStatus:'',confirmStatus:'',scheduleSource:document.getElementById('sch_scheduleSource')?.value||'排课表',giftLesson,coachLateFree,lateMinutes:parseInt(document.getElementById('sch_lateMinutes')?.value)||0,lateReason,coachLateFieldFeeAmount:parseFloat(document.getElementById('sch_lateFieldFee')?.value)||0,coachLateHandledAt:coachLateFree?new Date().toISOString():'',coachLateHandledBy:coachLateFree?(currentUser?.name||''):'',notes:document.getElementById('sch_notes').value.trim()};
...
function openScheduleDetail(scheduleId){
  ...
  const giftSummary=s.giftLesson?`<div class="tms-section-header">赠送上课</div><div class="tms-form-row"><div class="tms-form-item"><label class="tms-form-label">处理结果</label><input class="finput tms-form-control" value="本节赠送，不扣学员课时" readonly></div></div></div>`:'';
