...
function scheduleSaveConfirmText(data,selectedEntitlement){
  const confirmMeta=scheduleConfirmRuleMeta(data.scheduleSource,data.startTime);
  return [
    '确认保存这节课？',
    ...
    `确认规则：${confirmMeta.label}`,
    `确认截止：${confirmMeta.dueText}`,
...
function openScheduleDetail(scheduleId){
  ...
  const confirmMeta=scheduleConfirmRuleMeta(s.scheduleSource||'排课表',s.startTime);
