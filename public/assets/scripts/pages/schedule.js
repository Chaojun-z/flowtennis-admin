...
function scheduleStudentRosterRows(){const map=new Map(),studentById=new Map(students.map(item=>[String(item.id||''),item]));const upsert=(row,base={})=>{const id=String(row.studentId||row.id||'').trim();if(!id)return;const merged={...base,...row};if(isHiddenStudentProfile(merged))return;const current=map.get(id)||{};map.set(id,{...current,...merged,id,name:merged.name||merged.displayName||current.name||'',phone:merged.phone||current.phone||'',campus:merged.campus||current.campus||'',primaryCoach:merged.primaryCoach||merged.owner||current.primaryCoach||''});};students.forEach(row=>upsert(row));[...(teachingStudentViews?.historicalStudents||[]),...(teachingStudentViews?.activeStudents||[]),...(teachingStudentViews?.searchableStudents||[])].forEach(row=>upsert(row,studentById.get(String(row.studentId||row.id||''))||{}));return [...map.values()];}
...
function selectScheduleStudent(studentId){
  ...
  const suggest=document.getElementById('sch_studentSuggest');if(suggest)suggest.innerHTML='';
  refreshSchEntitlementOptions();
}
function removeScheduleStudent(studentId){
  ...
  const entitlement=document.getElementById('sch_entitlement');if(entitlement)delete entitlement.dataset.keep;
...
function scheduleStudentSettlementPackageText(studentId,row={}){const rows=schStudentEntitlementOptionsByStudent.get(String(studentId||''))||[];const selected=scheduleSelectableEntitlementForStudent(studentId,row);return selected?scheduleEntitlementLabel(selected):scheduleEntitlementUnavailableReason(rows);}
function renderScheduleStudentSettlementRow(row={},student={}){const studentId=String(row.studentId||student?.id||'').trim();return scheduleStudentSettlementRowHtml({row:{...row,studentId},studentName:scheduleStudentDisplayName(student)||student.name||student.studentName||studentId,packageText:scheduleStudentSettlementPackageText(studentId,row),payMethodOptions:scheduleStudentSettlementPayMethodOptions()});}
...
async function saveScheduleStudentQuickCreate(){
  ...
  if(phone&&!validateCnPhone(phone)){toast('手机号格式不正确','warn');return;}
  const data=buildStudentQuickCreatePayload({name,phone,campus,source,notes});
  const duplicates=typeof getStudentDuplicateCandidates==='function'?getStudentDuplicateCandidates(data,''):[];
  if(duplicates.length){const summary=duplicates.map(s=>`${s.name}${s.phone?`（${s.phone}）`:''}`).join('、');if(!await appConfirm(`发现可能重复的学员：${summary}。是否继续保存？`,{title:'发现重复学员',confirmText:'继续保存'}))return;}
  const btn=document.getElementById('schQuickStudentSaveBtn');
  await runStandardMutation(btn,async()=>{const r=await apiCall('POST','/students',data);if(Array.isArray(students)&&!students.some(stu=>String(stu.id||'')===String(r.id||'')))students.unshift(r);closeScheduleStudentQuickCreateModal();selectScheduleStudent(r.id);return r;},{loadingText:'保存中…',errorPrefix:'保存失败',successText:'学员已创建并加入排课 ✓',closeOnSuccess:false});
}
function scheduleExternalVenueParts(s){const parts=String(s?.venue||'').split(' · ');return {name:s?.externalVenueName||parts[0]||'',court:s?.externalCourtName||parts.slice(1).join(' · ')||''};}
...
function refreshSchedulePaymentFields(){
  ...
  const packageRow=document.querySelector('.schedule-package-row'),packageItem=document.getElementById('sch_packageSettlementItem'),paymentRow=document.getElementById('sch_paymentRow'),payMethodItem=document.getElementById('sch_paymentMethodItem'),fieldFeePayMethodItem=document.getElementById('sch_fieldFeePayMethodItem');
  ...
  if(packageRow)packageRow.style.display=useStudentRows?'none':'';
...
function openScheduleModal(id,seed={}){
  (async()=>{
    ...
    const usePerStudentSettlementInitial=['小班课','专项课'].includes(normalizeCourseType(courseTypeForm.courseType||''));
    ...
    const packageField=`<div class="tms-form-row schedule-package-row" style="display:${usePerStudentSettlementInitial?'none':''}"><div class="tms-form-item full-width schedule-entitlement-item" id="sch_packageSettlementItem"><label class="tms-form-label">扣减课包</label><div id="sch_entitlementHost">${renderScheduleEntitlementDropdown([],rv(s,'entitlementId',''),rv(s,'packageName','')||'')}</div><div id="sch_studentEntitlementRows"></div><div id="sch_ent_hint" class="schedule-entitlement-alert"></div></div></div>`;
...
async function saveSchedule(){
  ...
  const rootSettlementType=useStudentSettlementRows?(studentSettlementRows.some(row=>row.settlementType==='package')?'package':'direct'):settlementType;const packageRows=useStudentSettlementRows?studentSettlementRows.filter(row=>row.settlementType==='package'):[],missingPackageBinding=rootSettlementType==='package'&&lc>0&&!coachLateFree&&(useStudentSettlementRows?packageRows.some(row=>!String(row.entitlementId||'').trim()):!String(selectedEntitlementId||'').trim());if(missingPackageBinding){toast('课包结算必须选择课包，未保存','warn');return;}
...
function resetScheduleSaveButton(){const btn=document.getElementById('scheduleSaveBtn');if(btn){btn.disabled=false;btn.textContent='保存修改';}}
