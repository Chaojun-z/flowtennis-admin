...
function scheduleEntitlementLabel(option){
  ...
  return [title,`${option.remainingLessons}/${option.totalLessons}${unit}`,scheduleEntitlementTimeBandText(option,title),scheduleEntitlementExpiryText(option)].filter(Boolean).join(' · ');
}
