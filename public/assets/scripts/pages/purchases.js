...
function purchasePackageSnapshotHtml(p){
  ...
  return `<div class="sec-ttl">购买时规则快照</div><div class="fgrid"><div class="fg"><div class="flabel">课程类型</div><div class="finput">${esc(renderCourtEmptyText(p.courseType))}</div></div><div class="fg"><div class="flabel">归属教练</div><div class="finput">${esc(renderCourtEmptyText(p.ownerCoach))}</div></div><div class="fg"><div class="flabel">可上课教练</div><div class="finput">${esc(coachText)}</div></div><div class="fg"><div class="flabel">课包课时</div><div class="finput">${parseInt(p.packageLessons)||0} 节</div></div><div class="fg"><div class="flabel">课包标价</div><div class="finput">¥${fmt(p.packagePrice)}</div></div><div class="fg"><div class="flabel">时段类型</div><div class="finput">${esc(p.packageTimeBand)||'全天'}</div></div><div class="fg"><div class="flabel">每日时段</div><div class="finput">${esc(windows)}</div></div><div class="fg"><div class="flabel">可用校区</div><div class="finput">${esc(campusText)}</div></div><div class="fg"><div class="flabel">使用开始</div><div class="finput">${esc(renderCourtEmptyText(p.usageStartDate))}</div></div><div class="fg"><div class="flabel">使用结束</div><div class="finput">${esc(renderCourtEmptyText(p.usageEndDate))}</div></div></div>`;
}
function purchaseLedgerHtml(purchaseId){
  ...
  return `<div class="finput" style="min-height:42px;white-space:normal;line-height:1.7">${rows.map(l=>`${(Number(l.lessonDelta)||0)>0?'退回':'扣减'} ${lessonQty(Math.abs(Number(l.lessonDelta)||0))} 节 · ${esc(renderCourtEmptyText(l.reason))} · ${renderCourtEmptyText(entitlementLedgerDisplayDate(l))}`).join('<br>')}</div>`;
}
