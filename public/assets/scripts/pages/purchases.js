...
function syncPurchasePackageMeta(prefix='pur',force=false){
  ...
  if(ownerCoachEl&&pkg?.ownerCoach&&(force||!ownerCoachEl.value)){
    ...
    setCourtDropdownValue(ownerCoachId,ownerCoach,ownerCoach);
  }
  if(force&&pkg){
...
function selectPurchaseStudent(studentId){
  ...
  if(stu?.primaryCoach){const primaryCoach=coachName(stu.primaryCoach);setCourtDropdownValue('pur_ownerCoach',primaryCoach,primaryCoach);}
