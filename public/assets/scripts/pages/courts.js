...
function renderMembershipOrdersAuditPage(){
  ...
  host.innerHTML=`<div class="tms-audit-note">此页面仅用于审计与追溯，不用于日常操作。</div><div class="tms-table-card" style="margin-bottom:0"><div class="tms-table-wrapper"><table class="tms-table"><thead><tr><th style="padding-left:20px;width:120px">支付日期</th><th style="width:170px">录入时间</th><th style="width:120px">订场用户</th><th style="width:150px">会员方案</th><th style="width:110px">充值</th><th style="width:110px">赠送金额</th><th style="width:90px">折扣</th><th style="width:120px">是否重置有效期</th><th style="width:360px">当次权益摘要</th><th style="width:100px">状态</th></tr></thead><tbody>${rows.map(o=>`<tr><td style="padding-left:20px">${renderCourtCellText(o.purchaseDate)}</td><td>${renderCourtCellText(formatMembershipLedgerTime(o.createdAt),false)}</td><td>${renderCourtCellText(o.courtName)}</td><td>${renderCourtCellText(o.membershipPlanName)}</td><td><div class="tms-cell-text">¥${fmt(o.rechargeAmount)}</div></td><td><div class="tms-cell-text">¥${fmt(o.bonusAmount)}</div></td><td>${renderCourtCellText(membershipDiscountText(o.discountRate),false)}</td><td>${renderCourtCellText(o.qualifiesRenewalReset===false?'否':'是',false)}</td><td><div class="tms-cell-text" style="white-space:normal;line-height:1.55;min-width:320px">${membershipOrderBenefitSummaryHtml(o)}</div></td><td>${renderCourtCellText(membershipStatusText(o.status),false)}</td></tr>`).join('')||'<tr><td colspan="10"><div class="empty"><p>暂无会员购买记录</p></div></td></tr>'}</tbody></table></div></div>`;
}
...
function renderCourtHistoryItems(hist){
  return hist.length?hist.map(h=>{
    ...
    const sign=type==='充值'?'+':'-';
    const meta=[h.payMethod,h.category,h.internalReason,h.note].filter(Boolean).join(' · ')||'—';
...
async function saveCourtFinanceRecord(){
  ...
  const h={id:uid(),date,type,category,payMethod:category==='内部占用'?'其他':payMethod,studentId,amount:category==='内部占用'?0:Math.abs(amt),note,startTime,endTime,venue,campus:recCampus,lessonCount,internalReason};
