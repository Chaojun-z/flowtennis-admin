...
function planEntitlementRows(p){
  const prod=planProduct(p);
  const base=entitlements.filter(e=>e.studentId===p?.studentId&&e.status==='active'&&lessonValue(e.remainingLessons)>0);
...
function planEntitlementSummary(p){
  ...
  const e=rows[0];
  return `${esc(standardPackageLabel(e,true)||e.packageName||'课包')}<div class="udesc">剩 ${lessonQty(e.remainingLessons)}/${lessonQty(e.totalLessons)}</div>`;
}
...
function openPlanDetail(planId){
  ...
  openStandardModal({title:'学习计划详情',bodyHtml:body,actionsHtml:footer,extraClass:'modal-wide'});
}
