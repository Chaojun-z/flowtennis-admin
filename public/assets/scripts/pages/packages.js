...
function renderPackages(){
  ...
  host.innerHTML=list.length?list.map(p=>{
    ...
    return `<div class="package-card-shell"><div class="showcase-card-body"><div class="showcase-card-header"><div class="showcase-card-title-group"><div class="showcase-card-title">${esc(p.name)}<span class="tms-tag ${productTypeTagClass(p.courseType)}">${esc(p.courseType||'—')}</span></div><div class="showcase-card-subtitle">关联产品: ${esc(p.productName||'—')}</div></div><span class="showcase-status-tag ${status==='inactive'?'is-off':'is-on'}">${status==='inactive'?'停用':'启用'}</span></div><div class="showcase-highlight"><span class="showcase-highlight-price">¥${fmt(p.price)}</span><span class="showcase-highlight-divider">/</span><span class="showcase-highlight-value">${p.lessons||0}<span class="showcase-highlight-unit">节</span></span><span class="showcase-highlight-divider">/</span><span class="showcase-highlight-value">${p.maxStudents||1}<span class="showcase-highlight-unit">人</span></span></div><div class="showcase-kv-list"><div class="showcase-kv-row"><div class="showcase-kv-label">归属教练</div><div class="showcase-kv-value">购买时选择</div></div><div class="showcase-kv-row"><div class="showcase-kv-label">销售归属校区</div><div class="showcase-kv-value">${esc(packageSaleCampusText(p))}</div></div><div class="showcase-kv-row"><div class="showcase-kv-label">可上课教练</div><div class="showcase-kv-value">${esc(coachText)}</div></div><div class="showcase-kv-row"><div class="showcase-kv-label">可用校区</div><div class="showcase-kv-value">${esc(campusText)}</div></div><div class="showcase-kv-row"><div class="showcase-kv-label">活动期</div><div class="showcase-kv-value is-mono">${esc(p.saleStartDate||'不限')} <span class="showcase-kv-sep">至</span> ${esc(p.saleEndDate||'不限')}</div></div><div class="showcase-kv-row"><div class="showcase-kv-label">使用期</div><div class="showcase-kv-value is-mono">${esc(p.usageStartDate||'不限')} <span class="showcase-kv-sep">至</span> ${esc(p.usageEndDate||'不限')}</div></div><div class="showcase-kv-row"><div class="showcase-kv-label">使用时段</div><div class="showcase-kv-value">${esc(timeWindow||'全天')}</div></div></div></div><div class="showcase-card-footer"><div class="showcase-card-actions"><button class="showcase-action-btn is-primary" onclick="focusPurchaseByPackage('${p.id}')">看订单</button></div><div class="showcase-card-actions"><button class="showcase-action-btn" onclick="openPackageModal('${p.id}')">编辑</button><button class="showcase-action-btn is-danger" onclick="confirmDel('${p.id}','${esc(p.name)}','package')">删除</button></div></div></div>`;
...
function packageSaleCampusId(rec){
  return String(rec?.saleCampusId||parseArr(rec?.campusIds)[0]||'').trim();
}
function packageSaleCampusText(rec){
  const id=packageSaleCampusId(rec);
  return id?cn(id):'未设置';
}
  ...
  document.getElementById('mBody').innerHTML=`
    ...
    <div class="form-section">
      ...
      <div class="fgrid cols-4">
        ...
        <div class="fg"><div class="flabel">销售归属校区 *</div>${renderCourtDropdownHtml('pkg_saleCampusId','销售归属校区',campuses.map(c=>({value:c.code||c.id,label:c.name})),packageSaleCampusId(p),true)}</div>
...
async function savePackage(){
  ...
  const saleCampusId=document.getElementById('pkg_saleCampusId')?.value||'';
  if(campuses.length&&!saleCampusId){toast('请选择销售归属校区','warn');return;}
  ...
  const data={name,productId,productName:product?.name||'',courseType,price:parseFloat(document.getElementById('pkg_price').value)||0,lessons:parseInt(document.getElementById('pkg_lessons').value)||0,validDays:parseInt(document.getElementById('pkg_validDays').value)||0,saleCampusId,saleStartDate,saleEndDate,usageStartDate,usageEndDate,timeBand,dailyTimeWindows,coachNames,coachIds:coachNames,campusIds,maxStudents:parseInt(document.getElementById('pkg_maxStudents').value)||1,status:document.getElementById('pkg_status').value,notes:document.getElementById('pkg_notes').value.trim()};
