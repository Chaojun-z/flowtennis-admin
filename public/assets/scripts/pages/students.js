...
function renderStudents(){
  ...
  document.getElementById('stuTbody').innerHTML=slice.length?slice.map(s=>{
    ...
    if(studentListViewMode()==='trial')return `<tr><td class="tms-sticky-l" style="padding-left:20px"><div class="tms-text-primary">${esc(s.name)}</div></td><td>${renderStandardBusinessTag(s.type,'customerType')}</td><td>${renderStandardBusinessTag(studentSourceText(s),'source')}</td><td>${renderStandardCellText(cn(s.campus))}</td><td>${renderStandardBusinessTag(studentTrialPathStatusText(s),'stage')}</td><td>${renderStandardCellText(purchaseDate,false)}</td><td>${renderStandardTooltipText(packageNameText,'tms-text-remark')}</td><td>${renderStandardCellText(coachText)}</td><td>${renderStandardTooltipText(noteText,'tms-text-remark')}</td><td class="tms-sticky-r tms-action-cell" style="width:150px;padding-right:20px"><span class="tms-action-link" onclick="openStudentDetail('${s.id}')">查看</span><span class="tms-action-link" onclick="openPurchaseModal('${s.id}')">课包</span></td></tr>`;
    return `<tr><td class="tms-sticky-l" style="padding-left:20px"><div class="tms-text-primary">${esc(s.name)}</div></td><td>${renderStandardCellText(s.phone)}</td><td>${renderStandardBusinessTag(s.type,'customerType')}</td><td>${renderStandardCellText(cn(s.campus))}</td><td>${renderStandardCellText(purchaseDate,false)}</td><td>${renderStandardCellText(studentCompletedLessonCount(s),false)}</td><td>${renderStandardCellText(coachText)}</td><td class="tms-tooltip-text" data-tooltip="${esc(packageText)}">${studentPackageLessonMiniBar(s)}</td><td>${renderStandardCellText(studentDealPathText(s),false)}</td><td>${renderStandardBusinessTag(studentSourceText(s),'source')}</td><td>${renderStandardTooltipText(noteText,'tms-text-remark')}</td><td class="tms-sticky-r tms-action-cell" style="width:150px;padding-right:20px"><span class="tms-action-link" onclick="openStudentDetail('${s.id}')">查看</span><span class="tms-action-link" onclick="openPurchaseModal('${s.id}')">课包</span></td></tr>`;
