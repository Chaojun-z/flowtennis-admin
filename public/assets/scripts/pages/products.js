...
function syncProductExperienceType(){
  ...
  syncProductSpecialCourseFields();
}
function syncProductSpecialCourseFields(){
  const type=document.getElementById('p_type')?.value||'';
  const item=document.getElementById('p_specialCourseItem');
  if(item)item.style.display=normalizeCourseType(type)==='专项课'?'':'none';
}
...
async function saveProduct(){
  ...
  const skillLevelMin=type==='专项课'?(document.getElementById('p_skillMin')?.value||''):'';
  const skillLevelMax=type==='专项课'?(document.getElementById('p_skillMax')?.value||skillLevelMin):'';
  const specialTopic=type==='专项课'?(document.getElementById('p_specialTopic')?.value||'').trim():'';
  const data={name,type,experienceType:type==='体验课'?normalizeExperienceType(document.getElementById('p_experienceType')?.value):'',skillLevelMin,skillLevelMax,specialTopic,courseDisplayName:name,maxStudents:parseInt(document.getElementById('p_max').value)||1,price:parseFloat(document.getElementById('p_price').value)||0,lessons:parseInt(document.getElementById('p_lessons').value)||0,notes:document.getElementById('p_notes').value.trim()};
