...
function renderProducts(){
  ...
  document.getElementById('productGrid').innerHTML=list.length?list.map(p=>{
    ...
    return `<div class="product-card-shell"><div class="showcase-card-body"><div class="showcase-card-header"><div class="showcase-card-title-group"><div class="showcase-card-title">${esc(p.name)}<span class="tms-tag ${productTypeTagClass(p.type)}">${esc(typeText||'—')}</span></div><div class="showcase-card-subtitle">用于创建班次和售卖课包的课程模板</div></div>${locked?'<span class="showcase-status-tag is-linked">已被引用</span>':''}</div><div class="showcase-highlight"><span class="showcase-highlight-value">${p.maxStudents||1}<span class="showcase-highlight-unit">人</span></span><span class="showcase-highlight-divider">/</span><span class="showcase-highlight-value">¥${fmt(p.price)}</span><span class="showcase-highlight-divider">/</span><span class="showcase-highlight-value">${p.lessons||0}<span class="showcase-highlight-unit">节</span></span></div><div class="showcase-kv-list"><div class="showcase-kv-row"><div class="showcase-kv-label">已建班次</div><div class="showcase-kv-value">${linkedClasses} 个</div></div><div class="showcase-kv-row"><div class="showcase-kv-label">已建课包</div><div class="showcase-kv-value">${linkedPackages} 个</div></div></div></div><div class="showcase-card-footer"><div class="showcase-card-actions"><button class="showcase-action-btn is-primary" onclick="openPackageModal(null,'${p.id}')">建课包</button></div><div class="showcase-card-actions"><button class="showcase-action-btn" onclick="openProductModal('${p.id}')">编辑</button><button class="showcase-action-btn is-danger" onclick="confirmDel('${p.id}','${esc(p.name)}','product')">删除</button></div></div></div>`;
