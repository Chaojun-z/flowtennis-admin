...
function syncClassFilterOptions(){
  ...
  wrapMap.forEach(([hostId,id,label,options,value])=>{
    ...
    if(host)host.innerHTML=renderStandardDropdownHtml(id,label,options,value,false,'renderClasses');
  });
}
function renderClasses(){
  ...
  let list=classes.filter(c=>{
    ...
    if(coachF&&coachName(c.coach)!==coachF)return false;
    ...
    return searchHit(q,c.className,c.classNo,c.coach,c.productName,courseTypeDisplayLabel(prod||{}),prod?.lessons,prod?.price,c.status,c.notes,c.opsNote,c.sourceType,cn(c.campus),stuText,tags.join(' '));
  });
  ...
  document.getElementById('clsPagerBtns').innerHTML=pages<=1?'':renderStandardPaginationButtonsHtml(clsPage,pages,'setClassPage');
  ...
  document.getElementById('clsTbody').innerHTML=slice.length?slice.map(c=>{
    ...
    const typeText=courseTypeDisplayLabel(prod||{type:c.sourceType});
    return `<tr><td style="padding-left:20px"><div class="tms-text-primary">${esc(displayName)}</div></td><td>${renderStandardCellText(c.classNo,false)}</td><td><div class="tms-text-primary">${esc(prod?.name||c.productName||'-')}</div></td><td><span class="tms-tag ${productTypeTagClass(typeText)}">${esc(typeText)}</span></td><td>${renderStandardCellText(c.startDate,false)}</td><td>${renderStandardCellText(c.endDate,false)}</td><td>${renderStandardCellText(cn(c.campus))}</td><td><div class="tms-text-remark" title="${names}">${names}</div></td><td>${renderStandardCellText(coachName(c.coach))}</td><td>${renderStandardCellText(days)}</td><td>${renderStandardCellText(last?.startTime?fmtDt(last.startTime):'-',false)}</td><td>${renderStandardCellText(next?.startTime?fmtDt(next.startTime):'-',false)}</td><td><div class="prog-wrap"><div class="prog-track"><div class="prog-fill ${pc}" style="width:${Math.max(0,Math.min(100,pct))}%"></div></div><span class="prog-txt">${lessonQty(ul)}/${lessonQty(tl)} 剩${lessonQty(rem)}</span></div></td><td><span class="tms-tag ${ss[displayStatus]||'tms-tag-tier-slate'}">${displayStatus}</span></td><td class="tms-sticky-r tms-action-cell" style="width:160px;padding-right:20px"><span class="tms-action-link" onclick="openClassDetail('${c.id}')">查看</span><span class="tms-action-link" onclick="openClassModal('${c.id}')">编辑</span><span class="tms-action-link" onclick="confirmDel('${c.id}','${esc(displayName)}','class')">删除</span></td></tr>`;
  }).join(''):'<tr><td colspan="15"><div class="empty"><div class="empty-ico">📋</div><p>暂无班次</p></div></td></tr>';
}
function setClassPage(value){
  clsPage=Math.max(1,parseInt(value,10)||1);
  renderClasses();
}
function openClassDetail(id){
  ...
  openStandardModal({title:'查看班次',bodyHtml:body,actionsHtml:footer,extraClass:'modal-wide'});
}
function openClassModal(id){
  ...
  const po=products.map(p=>`<option value="${p.id}"${rv(c,'productId')===p.id?' selected':''}>${esc(p.name)}（${courseTypeDisplayLabel(p)}，${p.lessons}节）</option>`).join('');
  const co=activeCoachNames().map(x=>`<option value="${x}"${coachName(rv(c,'coach'))===x?' selected':''}>${x}</option>`).join('');
  ...
  const productOptions=[{value:'',label:'— 选择 —'},...products.map(p=>({value:p.id,label:`${p.name}（${courseTypeDisplayLabel(p)}，${p.lessons}节）`}))];
  ...
  openStandardModal({title:id?'编辑班次':'新建班次',bodyHtml:body,actionsHtml:footer,extraClass:'modal-wide'});
  if(locked){
