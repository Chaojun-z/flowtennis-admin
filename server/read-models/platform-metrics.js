...
function courseRowPackageLessonTotal(row = {}) {
  const value = Number(row.totalLessons || row.packageLessons);
  return Number.isFinite(value) && value > 0 ? value : 0;
}

function courseRowIsOneTimePaidProduct(row = {}) {
  const label = text([
    row.courseType,
  ...
  return total === 1 || /单次|按次|1\s*(?:次|课时|节)|199\s*元|199元/.test(label);
}
...
function formalPurchaseRows(data = {}) {
  const rows = new Map();
  (data.purchases || [])
    .filter(row => courseRowIsFormalPackageProduct(row) && coursePaymentAmount(row) > 0)
    .forEach(row => rows.set(coursePurchaseKey(row), row));
  (data.entitlements || [])
    .forEach(row => {
      const purchase = (data.purchases || []).find(item => text(item.id) === text(row.purchaseId)) || {};
      const merged = { ...purchase, ...row };
      if (courseRowIsFormalPackageProduct(merged) && coursePaymentAmount(merged) > 0 && !rows.has(coursePurchaseKey(merged))) rows.set(coursePurchaseKey(merged), merged);
    });
  return [...rows.values()];
}
...
function activeFormalPackageStudentCount(data = {}) {
  ...
  (data.entitlements || []).forEach(row => {
    ...
    if (!courseRowIsFormalPackageProduct(row)) return;
...
function formalPackageRecognizedAmount(data = {}) {
  ...
  return money((data.entitlementLedger || []).reduce((sum, row) => {
    ...
    if (!courseRowIsFormalPackageProduct({ ...purchase, ...entitlement })) return sum;
...
function reconcileTeachingPackageFields(packageFields = {}, completedLessons = 0, directFormalCompleted = 0) {
  ...
  if (packageCompleted < (Number(currentConsumed) || 0)
    && ((Number(packageFields.detailPackageBalanceRemaining) || 0) > 0 || packageCompleted < total - 1)) return packageFields;
    ...
    .forEach(row => {
      ...
      displayStudentIds.forEach(studentId => {
        ...
        push(studentId, {
          ...
          packageRecordKey: packageMeta.packageRecordKey,
          sortTime,
          time: displayTime,
          packageName: packageMeta.packageName || text(row.packageName || row.className || row.courseName || row.standardCourseType || row.courseType),
          totalLessons: packageMeta.totalLessons,
          paidAmount: packageAmount(packageMeta.entitlement, packageMeta.purchase, ['finalAmount', 'amountPaid', 'actualAmount', 'paidAmount', 'amount']),
...
function buildTeachingStudentListFieldMap(data = {}, options = {}) {
  ...
  [...new Set([...summaryFieldMap.keys(), ...packageFieldMap.keys(), ...trialPackageFieldMap.keys(), ...coursePaidByStudent.keys(), ...lessonDetailMap.keys(), ...trialLessonDetailMap.keys(), ...benefitDetailMap.keys(), ...feedbackMap.keys()])].forEach(studentId => {
    ...
    const packageConsumedLimit = rawPackageConsumedLimit === null
      ? null
      : (hasFreshLessonRows && ledgerPackageCompleted > 0 ? ledgerPackageCompleted : Math.max(rawPackageConsumedLimit, ledgerPackageCompleted));
...
function teachingStudentViewRow(row = {}, listFields = {}) {
  ...
  const detailLessonRecordRows = hasOwn(listFields, 'detailLessonRecordRows')
    ? (Array.isArray(listFields.detailLessonRecordRows) ? listFields.detailLessonRecordRows : [])
    : (Array.isArray(row.detailLessonRecordRows) ? row.detailLessonRecordRows : []);
  const searchText = [
    row.searchText,
    row.displayName,
    row.name,
  ...
  return {
    ...
    completedLessons: round((hasOwn(listFields, 'completedLessons') ? listFields.completedLessons : row.completedLessons) || 0, 1),
    detailLessonRecordRows,
    detailRecentLessonDate: text(hasOwn(listFields, 'detailRecentLessonDate') ? listFields.detailRecentLessonDate : row.detailRecentLessonDate),
    cumulativeCoursePaidAmount: money((hasOwn(listFields, 'cumulativeCoursePaidAmount') ? listFields.cumulativeCoursePaidAmount : row.cumulativeCoursePaidAmount) || 0),
    cumulativeCoursePaidText: text(hasOwn(listFields, 'cumulativeCoursePaidText') ? listFields.cumulativeCoursePaidText : row.cumulativeCoursePaidText),
...
function teachingScheduleElapsedCompletionSignal(row = {}) {
  ...
  return /课包|扣减|划扣|核销|消课|single|单次|gift|赠送|免费|free/i.test(marker);
}
...
function teachingScheduleLessonFact(row = {}, now = new Date()) {
  ...
  if (status === '已排课') return !!dateOnly(row.endTime) && happened && teachingScheduleElapsedCompletionSignal(row);
  if (['待上课', '待确认', '预约', '已预约'].includes(status)) return false;
  return happened;
}
...
function teachingStudentPackageStatusLabel(row = {}) {
  if (row.hasTeachingSummarySnapshot && text(row.packageStatusLabel) === '使用他人课包') return '使用他人课包';
...
function teachingStudentSummarySnapshotRow(row = {}, now = new Date().toISOString()) {
  ...
  return {
    ...
    wechatName: text(row.wechatName),
    ...
    campusName: text(row.campusName),
    campusIds: Array.isArray(row.campusIds) ? row.campusIds : parseArr(row.campusIds),
    primaryCoach: text(row.primaryCoach),
    sourceLeadId: text(row.sourceLeadId),
    hasStudentProfile: !!row.hasStudentProfile,
    notes: text(row.notes),
    profileNote: text(row.profileNote),
    studentStage: text(row.studentStage),
    trialStatus: text(row.trialStatus),
    courseDealPath: text(row.courseDealPath),
    trialAtRaw: text(row.trialAtRaw),
    trialBookedAt: text(row.trialBookedAt),
    trialAttendedAt: text(row.trialAttendedAt),
    courseFirstPurchaseAt: text(row.courseFirstPurchaseAt),
    conversionAt: text(row.conversionAt),
    ...
    leadDate: text(row.leadDate),
    leadDateSource: text(row.leadDateSource),
