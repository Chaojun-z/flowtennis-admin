...
function parseSharedPackageStudentCell(raw){
  ...
  return {
    ...
    lessonIndex,
    sharedPackageAttendeeName:attendee,
    sharedPackageOwnerName:packageOwner,
...
function applyConfirmedFeishuCellCorrection(row={}){
  const date=cleanText(row.date);
  const start=cleanText(row.time?.start||row.startClock);
  const end=cleanText(row.time?.end||row.endClock);
  const coachKey=normalizeNameKey(row.coachName);
  const studentKey=normalizeStudentNameKey(row.studentText);
  const compactStudentText=cleanText(row.studentText).replace(/\s+/g,'').toLowerCase();
  const courtKey=normalizeNameKey(row.courtText);
  if(date==='2026-08-08'&&start==='15:00'&&end==='16:00'&&coachKey===normalizeNameKey('siren')&&studentKey===normalizeStudentNameKey('朝珺木子')&&courtKey===normalizeNameKey('1号')){
    return {
      ...row,
      coachName:'朝珺教练',
      studentText:'木子',
      ...
      confirmedCorrectionNote:'用户确认：朝珺给木子上课，借用 1 号场，与 Siren 小土豆的姐姐课程并行',
      confirmedPaymentLocked:true,
      confirmedPayment:{settlementType:'direct',payMethod:'',paidAmount:0,confirmedPaymentNote:'用户确认：木子借用 1 号场，只补排课事实，不自动扣课包'}
    };
  }
  if(date==='2026-08-16'&&start==='10:00'&&end==='11:30'&&coachKey===normalizeNameKey('岳克舟')&&compactStudentText==='jerry、jerry、艾斯'&&courtKey===normalizeNameKey('1号')){
    return {
      ...row,
      studentText:'Jerry、Jerry 老婆、艾斯',
      confirmedCorrectionNote:'用户确认：Jerry、Jerry 老婆、艾斯三人上课',
      confirmedPaymentLocked:true,
      confirmedPayment:{settlementType:'direct',payMethod:'微信',paidAmount:594,confirmedPaymentNote:'用户确认：3 人每人支付 198 元，微信合计收款 594 元'}
    };
  }
  if(date==='2026-08-18'&&start==='11:00'&&end==='12:00'&&coachKey===normalizeNameKey('岳克舟')&&studentKey===normalizeStudentNameKey('王先生')&&courtKey===normalizeNameKey('3号')){
    return {...row,courtText:'1号'};
  }
  if(date==='2026-08-18'&&start==='11:00'&&end==='12:00'&&coachKey===normalizeNameKey('杨')&&studentKey===normalizeStudentNameKey('甄女士')&&courtKey===normalizeNameKey('3号')){
    return {...row,courtText:'2号'};
  }
...
function findSpecialCoursePackage(packages=[],candidate={}){
  if(candidate.course?.courseType!=='专项课')return null;
  const amount=specialCourseAutoPackageAmount(candidate);
  ...
  return (packages||[]).find(row=>{
    ...
    if(rowPrice!==amount)return false;
    if(row.courseType&&row.courseType!=='专项课')return false;
    return specialCourseEntitlementMatches(row,candidate);
  })||null;
}
...
function parseFeishuScheduleRows({values=[],merges=[],sheetId='',sheetTitle=''}={}){
  ...
  return grouped.map(row=>{
    const correctedRow=applyConfirmedFeishuCellCorrection(row);
    const students=parseStudentCell(correctedRow.studentText);
    const course=applyConfirmedCourseCorrection(correctedRow,normalizeCourseType(correctedRow.courseText),students);
    const venue=normalizeVenueName(correctedRow.venueText);
    const venueName=normalizeCourtName(correctedRow.courtText);
    const durationMinutes=minutesBetween(row.startClock,row.endClock);
    const sourceKey=[
      correctedRow.sheetId,correctedRow.date,correctedRow.startClock,correctedRow.endClock,normalizeNameKey(correctedRow.coachName),
      normalizeNameKey(correctedRow.studentText),normalizeNameKey(correctedRow.courseText),normalizeNameKey(correctedRow.venueText),normalizeNameKey(correctedRow.courtText)
    ].join('|');
    return {
      ...correctedRow,
      sourceKey,
      fingerprint:sha256(JSON.stringify({row:correctedRow,course,students,venue,venueName,durationMinutes})),
      startTime:`${correctedRow.date} ${correctedRow.startClock}`,
      endTime:`${correctedRow.date} ${correctedRow.endClock}`,
      durationMinutes,
      lessonCount:durationMinutes/60,
      course,
      studentNames:students.names,
      lessonIndex:students.lessonIndex,
      sharedPackageAttendeeName:students.sharedPackageAttendeeName||'',
      sharedPackageOwnerName:students.sharedPackageOwnerName||'',
      sharedPackageNote:students.sharedPackageNote||'',
      allowLinkedVenueConflict:!!correctedRow.allowLinkedVenueConflict,
      linkedScheduleGroupId:correctedRow.linkedScheduleGroupId||'',
      confirmedCorrectionNote:correctedRow.confirmedCorrectionNote||'',
      campus:venue.campus,
      locationType:venue.locationType,
      venue:venueName,
      sourceCell:`R${correctedRow.startRowIndex+1}C${correctedRow.colIndex+1}`
    };
  });
}
...
function venueConflictForNewCandidate(candidate={},schedules=[]){
  if(candidate.allowLinkedVenueConflict)return null;
  const fields=scheduleCandidateFields(candidate);
  return (schedules||[]).find(row=>{
    if(!activeSchedule(row))return false;
    if(String(row.venue||'')!==String(fields.venue||''))return false;
...
function campusKeysMatch(left,right,options={}){
  const a=normalizeCampusKey(left);
  const b=normalizeCampusKey(right);
  if(a===b)return true;
  return !!options.allowMissingCampus&&(!a||!b);
}
...
function sameSlotSamePeopleScheduleMatch(candidate,schedules=[],options={}){
  const fields=scheduleCandidateFields(candidate);
  const studentSet=new Set(fields.studentIds.map(String));
  return (schedules||[]).find(row=>{
    if(!activeSchedule(row)||!sameTime(fields,row))return false;
...
function attachSharedPackageStudent(candidate,ctx={}){
  if(!candidate.sharedPackageAttendeeName)return null;
  const owner=candidate.resolvedStudents?.[0]||null;
  if(!owner?.id)return {...candidate,errors:[...candidate.errors,'共享课包没有找到课包所有人']};
  const attendeeName=resolveFeishuStudentAlias(candidate.sharedPackageAttendeeName);
  const matches=studentMatchesByName(ctx.students,attendeeName);
...
function confirmedYangZitianPackageOwner(candidate={}){
  if(candidate.course?.isTrial)return false;
  if(candidate.course?.courseType!=='私教课')return false;
  const keys=candidateStudentNameKeys(candidate);
  return keys.includes(normalizeStudentNameKey('杨梓天'));
}

function attachConfirmedYangZitianPackage(candidate,ctx={}){
  if(!confirmedYangZitianPackageOwner(candidate))return null;
  const owner=uniqueByName(ctx.students,'宝红')||studentMatchesByName(ctx.students,'宝红')[0];
  if(!owner?.id)return {...candidate,errors:[...candidate.errors,'杨梓天妈妈宝红学员档案不存在']};
  const selected=selectEntitlementIgnoringLessonIndex(owner,candidate,ctx.entitlements,ctx.recommendEntitlements,ctx.schedules);
  if(!selected)return {...candidate,errors:[...candidate.errors,'宝红没有可用于杨梓天排课的可扣课包']};
  return {
    ...candidate,
    scheduleStudents:[{...owner,name:'杨梓天'}],
    selectedEntitlements:[selected],
    sharedPackageNote:'杨梓天使用宝红课包'
  };
}
...
function attachSchedulableStudents(candidate,ctx={}){
  ...
  if(shared)return shared;
  const yangZitian=attachConfirmedYangZitianPackage(candidate,ctx);
  if(yangZitian)return yangZitian;
  ...
  const scheduleStudents=candidate.resolvedStudents.filter(student=>{
    ...
    if(selected){
      ...
      selectedStudentIds.add(String(student.id||''));
      return true;
    }
    return false;
  });
  if(candidate.course.courseType==='专项课'&&scheduleStudents.length<candidate.resolvedStudents.length){
    const pkg=findSpecialCoursePackage(ctx.packages,candidate);
    if(pkg){
      return {
        ...candidate,
        scheduleStudents:candidate.resolvedStudents.slice(),
...
function buildDryRunPlan({feishuCourses=[],syncRows=[],schedules=[],students=[],coaches=[],users=[],entitlements=[],packages=[],recommendEntitlements=null,nowKey='',scannedDateRanges=[]}={}){
  ...
  for(const raw of orderedFeishuCourses){
    ...
    const sameSlotSamePeople=sameSlotSamePeopleScheduleMatch(candidate,planningSchedules);
    if(sameSlotSamePeople){
      markRepresentedSchedule(sameSlotSamePeople);
      actions.push({type:'bind_existing',sourceKey:candidate.sourceKey,candidate,schedule:sameSlotSamePeople,sameSlotSamePeopleMatch:true,supersededSyncRows:supersededSyncRowsFor(sameSlotSamePeople,candidate.sourceKey)});
      continue;
    }
...
function buildScheduleBody(candidate,extra={}){
  ...
  const sharedAuth=candidate.sharedPackageAuthorization||extra.sharedPackageAuthorization||{};
  ...
  return {
    ...
    packageOwnerStudentId:sharedAuth.ownerStudentId||sharedAuth.packageOwnerStudentId||'',
    packageOwnerStudentName:sharedAuth.ownerStudentName||sharedAuth.packageOwnerStudentName||'',
    authorizedStudentId:sharedAuth.authorizedStudentId||sharedAuth.usedByStudentId||'',
    authorizedStudentName:sharedAuth.authorizedStudentName||sharedAuth.usedByStudentName||'',
    usedByStudentId:sharedAuth.authorizedStudentId||sharedAuth.usedByStudentId||'',
    usedByStudentName:sharedAuth.authorizedStudentName||sharedAuth.usedByStudentName||'',
    ...
    allowLinkedVenueConflict:!!candidate.allowLinkedVenueConflict,
    linkedScheduleGroupId:candidate.linkedScheduleGroupId||'',
    ...
    notes:[candidate.sharedPackageNote,confirmedPayment.confirmedPaymentNote,candidate.lessonIndexAuditNote,candidate.confirmedCorrectionNote,`飞书排课表同步 ${candidate.sheetTitle||candidate.sheetId||''} ${candidate.sourceCell||''}`.trim()].filter(Boolean).join('；')
  };
}
...
function formatCandidateCardLine(candidate={},suffix=''){
  ...
  const venue=cleanText(candidate.venue||formatVenueText(candidate));
  return [date&&time?`${date} ${time}`:date||time,venue,coach,student,course,suffix].filter(Boolean).join('｜');
}
...
function notificationProblemText(reason=''){
  const text=cleanText(reason);
  if(/历史排课缺少系统绑定/.test(text))return '系统里没有找到唯一可绑定排课';
  if(/历史排课修改/.test(text)){
    if(/目标时间或场地已有排课/.test(text))return '系统排课和飞书不一致，且目标时间或场地已有排课';
    return '系统排课和飞书不一致';
  }
  if(/课时编号|系统课包进度/.test(text))return '飞书课时编号和系统课包进度不一致';
  if(/无法唯一识别.*学员/.test(text))return '学员无法唯一识别';
  if(/没有可自动扣课/.test(text))return '没有可扣课包或已确认收费规则';
  if(/体验课无法判断/.test(text))return '体验课无法判断成人/青少年';
  if(/缺少课程类型|缺少场馆|缺少场地号|缺少学员/.test(text))return text;
  if(/小班课至少/.test(text))return '当前规则不允许这类小班课单人开课';
  return text||'需要人工确认';
}
...
function buildNotificationCard(result={}){
  ...
  if(needItems.length){
    ...
    const needLines=needItems.slice(0,8).map((item,index)=>{
      ...
      return `${index+1}. ${formatCandidateCardLine(candidate)}\n问题：${notificationProblemText(action.reason)}\n需要你确认：${operatorActionText(action.reason)}`;
    });
...
async function ensureSpecialCourseEntitlements(candidate,{entitlements=[],purchasePackage}={}){
  const students=Array.isArray(candidate.scheduleStudents)?candidate.scheduleStudents:[];
  ...
  for(const student of students){
    ...
    const existing=(entitlements||[]).find(row=>{
      if(String(row.studentId||'')!==studentId)return false;
      if(row.status&&row.status!=='active')return false;
      if(!specialCourseEntitlementMatches(row,candidate))return false;
      return Number(row.remainingLessons||0)>=candidateEntitlementLessonCount(candidate);
    });
    ...
    if(typeof purchasePackage!=='function')throw new Error('专项课包购买接口不可用');
    const purchase=await purchasePackage({
      studentId,
      packageId:pkg.id,
      purchaseDate:String(candidate.startTime||'').slice(0,10),
      ...
      payMethod:'微信',
      operator:'飞书同步',
      businessKey:['feishu-special-package',candidate.sourceKey,studentId,pkg.id].map(cleanText).join('|'),
      sourceBusinessKey:['feishu-special-package',candidate.sourceKey,studentId,pkg.id].map(cleanText).join('|'),
      sourceType:'feishu-schedule-sync',
      sourceKey:candidate.sourceKey||''
    });
    if(!purchase?.entitlement?.id)throw new Error('专项课包购买后没有生成可扣课包');
    selected.push(purchase.entitlement);
    byStudent.set(studentId,purchase.entitlement);
    purchases.push(purchase.purchase);
  }
  return {selectedEntitlements:selected,purchases};
}

async function ensureSharedPackageAuthorization(candidate,ctx={}){
  const auth=candidate.sharedPackageAuthorization||null;
  if(!auth?.entitlementId||!auth?.ownerStudentId||!auth?.authorizedStudentId)return candidate;
  const existing=(ctx.authorizations||[]).find(row=>
    String(row.entitlementId||'')===String(auth.entitlementId)&&
    String(row.ownerStudentId||row.packageOwnerStudentId||'')===String(auth.ownerStudentId)&&
    String(row.authorizedStudentId||row.usedByStudentId||'')===String(auth.authorizedStudentId)&&
    String(row.status||'active')==='active'
  );
  if(existing)return {...candidate,sharedPackageAuthorization:{...auth,authorizationId:existing.id}};
  if(typeof ctx.put!=='function'||!ctx.T_ENTITLEMENT_AUTHORIZATIONS)throw new Error('课包授权表不可用');
  const now=new Date().toISOString();
  const row={
    id:`feishu-auth-${sha256([auth.entitlementId,auth.ownerStudentId,auth.authorizedStudentId].join('|')).slice(0,24)}`,
    entitlementId:auth.entitlementId,
    purchaseId:auth.purchaseId||'',
    packageName:auth.packageName||'',
    ownerStudentId:auth.ownerStudentId,
    ownerStudentName:auth.ownerStudentName||'',
  ...
  try{
    await ctx.put(ctx.T_ENTITLEMENT_AUTHORIZATIONS,row.id,row);
  }catch(error){
    if(!/OTSObjectNotExist|Requested table does not exist/i.test(String(error?.message||error||'')))throw error;
    if(typeof ctx.mkTable==='function')await ctx.mkTable(ctx.T_ENTITLEMENT_AUTHORIZATIONS);
    await ctx.put(ctx.T_ENTITLEMENT_AUTHORIZATIONS,row.id,row);
  }
  if(Array.isArray(ctx.authorizations))ctx.authorizations.push(row);
...
function createFeishuScheduleSyncRoutes(deps={}){
  const {
    ...
    T_SCHEDULE,T_STUDENTS,T_COACHES,T_USERS,T_PACKAGES,T_ENTITLEMENTS,T_LEADS,T_ENTITLEMENT_AUTHORIZATIONS,T_FEISHU_SCHEDULE_SYNC,T_FEISHU_SCHEDULE_TASKS
  }=deps;
  ...
  async function runSync({dryRun=true,startDate='',endDate='',includeHistorical=false,historyApplyMode='',historyTrialMode='',notifyDryRun=false,suppressNotification=false,scanAllSheets=false}={}){
    ...
    const [feishuSheet,schedules,students,coaches,users,packages,entitlements,leads,authorizations]=await Promise.all([
      ...
      T_ENTITLEMENT_AUTHORIZATIONS?getCachedScan(T_ENTITLEMENT_AUTHORIZATIONS,{fresh:true}).catch(()=>[]):Promise.resolve([])
    ]);
