...
assert.match(apiSource, /async function prewarmStudentTeachingSummaryCache\(\)\{[\s\S]*readReadyStudentTeachingSummaryListRows\(\{tableName:T_STUDENT_TEACHING_SUMMARY,getCachedRow,getCachedScan,scanByIdPrefix,verifyChecksum:true\}\)[\s\S]*\}/, 'customer center first screen should prewarm the ready list bundle and summary-table light-field fallback');
