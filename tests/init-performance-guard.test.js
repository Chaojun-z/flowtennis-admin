...
assert.match(apiSource, /async function prewarmStudentTeachingSummaryCache\(\)\{[\s\S]*readReadyStudentTeachingSummaryListRows\(\{tableName:T_STUDENT_TEACHING_SUMMARY,getCachedRow,getCachedScan,scanByIdPrefix,put,verifyChecksum:true\}\)[\s\S]*\}/, 'customer center first screen should prewarm the ready list bundle and summary-table light-field fallback');
assert.doesNotMatch(fnBody(apiSource, 'prewarmStudentTeachingSummaryCache'), /rowsGzipBase64/, 'customer center first screen prewarm must not read or unzip the old large summary bundle');
