...
assert.match(fnBody('renderMemberships'), /const rows=serverPaging\?membershipBaseRows\(\):getMembershipRows\(\);[\s\S]*const stats=serverPaging&&courtAccountListViewData\?\.summary\?\.membershipSummary[\s\S]*: FlowTennisPlatformDataStandards\.currentMembershipSummary\(rows\);[\s\S]*renderMembershipStats\(stats\);/, 'membership summary should use the server-side filtered summary for paged lists and local rows only as fallback');
...
assert.doesNotMatch(fnBody('renderMembershipStats'), /courtAccountListViewData\?\.summary\?\.membershipFinanceSummary/, 'membership stats renderer should receive the prepared current-filter summary instead of reading global data');
