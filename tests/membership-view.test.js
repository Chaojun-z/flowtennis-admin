...
assert.match(fnBody('renderCourts'), /openCourtMembershipPanel\('\$\{u\.id\}'\)">查看[\s\S]*openCourtFinanceModal\('\$\{u\.id\}'\)">订场/, 'court legacy row actions should only expose view and booking drawer entries');
assert.doesNotMatch(fnBody('renderCourts'), /openCourtModal\('\$\{u\.id\}'\)">编辑/, 'court legacy row actions should remove edit copy');
...
assert.match(fnBody('membershipOrdersDrawerHtml'), /membershipOrderHasCustomAdjustment\(order\)[\s\S]*membershipPlanName[\s\S]*tms-tag[\s\S]*membership-order-title/, 'membership order custom tag should sit next to the plan name only when customized');
assert.match(fnBody('membershipOrdersDrawerHtml'), /membershipOrderBenefitLinesHtml\(order\)/, 'membership order tab should use per-line benefit summary');
...
assert.match(fnBody('membershipLedgerDrawerHtml'), /membershipBenefitLedger[\s\S]*l\.membershipAccountId===account\.id&&l\.action!=='grant'&&\(!membershipDrawerBenefitCode\|\|l\.benefitCode===membershipDrawerBenefitCode\)/, 'benefit history should hide grant rows because purchase records already carry the issuance source');
