...
assert.match(fnBody('renderMemberships'), /const rows=getMembershipRows\(\);[\s\S]*const stats=courtAccountListViewData\?\.summary\?\.membershipFinanceSummary\|\|FlowTennisPlatformDataStandards\.currentMembershipSummary\(rows\);[\s\S]*renderMembershipStats\(stats\);/, 'membership summary should use the backend filtered summary when the list is server-paged');
