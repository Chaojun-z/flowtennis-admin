...
assert.strictEqual(audit.warningCount, 1, 'finance audit should count warning anomalies after date clues auto-fix the historical import dates');
assert.strictEqual(audit.pendingCount, 1, 'finance audit should only keep unresolved actionable items after auto-fixing import dates');
assert.strictEqual(audit.fixedCount, 5, 'finance audit should count both campus and date auto-fixes');
...
assert.strictEqual(audit.actionItems[0].type, '历史导入零金额', 'finance audit should keep unresolved import zero-amount rows in action items');
