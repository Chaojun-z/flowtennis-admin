...
assert.match(leadsRoutesSource, /cachedResult=\{sorted:sortLeadListRows\(filtered,query\),summary:buildLeadListSummary\(filtered\),filters:buildLeadListFilterMeta\(visibleRows,filterState\)\};[\s\S]*const payload=paging\?\{\.\.\.buildLeadListPage\(cachedResult\.sorted,paging\),summary:cachedResult\.summary,filters:cachedResult\.filters\}:cachedResult\.sorted;[\s\S]*writeLeadPagedResponseCache\(responseCacheKey,payload\);[\s\S]*return sendJson\(res,payload\)/, 'lead list API should sort, summarize, count filters before caching and returning paged metadata');
