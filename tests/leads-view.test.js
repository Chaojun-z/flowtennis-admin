...
assert.match(standardSource, /微信名[\s\S]*线索时间[\s\S]*线索渠道[\s\S]*咨询需求[\s\S]*水平[\s\S]*基本信息[\s\S]*线索阶段[\s\S]*跟进优先级[\s\S]*跟进人[\s\S]*体验课时间[\s\S]*转化教练[\s\S]*未转化原因[\s\S]*操作/, 'leads table should expose the requested reordered columns');
...
assert.match(leadsSource, /function leadBasicInfoReadonlyHtml\(lead\)[\s\S]*leadDetailFieldHtml\('微信名',leadWechatText\(lead\)\)[\s\S]*leadDetailFieldHtml\('电话',lead\?\.phone\|\|'-'\)[\s\S]*leadDetailFieldHtml\('水平',leadLevelText\(lead\)\)[\s\S]*leadDetailFieldHtml\('线索时间',lead\?\.leadDate\|\|'-'\)[\s\S]*leadDetailFieldHtml\('线索来源',leadSourceText\(lead\)\|\|'-'\)[\s\S]*leadDetailFieldHtml\('所属校区',leadCampusText\(lead\)\)[\s\S]*leadDetailFieldHtml\('咨询需求',leadConsultText\(lead\)\|\|'-'\)[\s\S]*leadDetailFieldHtml\('意向类型',lead\?\.intentLevel\|\|'-'\)[\s\S]*leadDetailFieldHtml\('跟进优先级',leadPriorityText\(lead\)\)[\s\S]*leadDetailFieldHtml\('跟进人',lead\?\.owner\|\|'-'\)[\s\S]*leadDetailBlockHtml\('基本信息',esc\(leadProfileText\(lead\)\),\{hideEmpty:true\}\)/, 'lead detail basic tab should keep the current basic fields and normalize source/consult labels');
...
assert.match(leadsSource, /const body=`<div class="tms-form-row lead-form-row-4">[\s\S]*<label class="tms-form-label">微信名<\/label><input class="finput tms-form-control" id="lead_wechatName"[\s\S]*<label class="tms-form-label">电话<\/label><input class="finput tms-form-control" id="lead_phone"[\s\S]*<label class="tms-form-label">水平<\/label>\$\{leadLevelControlHtml\(lead\)\}[\s\S]*<label class="tms-form-label">线索时间<\/label>[\s\S]*<label class="tms-form-label">线索来源<\/label>[\s\S]*<label class="tms-form-label">所属校区<\/label>[\s\S]*<label class="tms-form-label">咨询需求<\/label>[\s\S]*<label class="tms-form-label">意向类型<\/label>[\s\S]*<label class="tms-form-label">跟进优先级<\/label>[\s\S]*leadPriorityOptions\(\)[\s\S]*<label class="tms-form-label">跟进人<\/label>[\s\S]*<label class="tms-form-label">基本信息<\/label>/, 'lead create and edit modal should use the requested field order without a section title');
...
assert.match(leadsSource, /function leadConsultOptions\(\)[\s\S]*FlowTennisBusinessTaxonomy\.optionList\('leadConsultTypes'\)/, 'lead consult options should use the global business dictionary');
...
assert.match(leadsSource, /function leadConsultText\(lead\)[\s\S]*FlowTennisBusinessTaxonomy\.normalizeLeadConsultType\(lead\?\.consultType\)/, 'lead consult display should normalize legacy consult values');
...
assert.match(fnBody('renderLeads'), /leadDateOnly\(lead\?\.leadDate,lead\)[\s\S]*renderLeadTag\(leadSourceText\(lead\),'source'\)[\s\S]*renderLeadTag\(leadConsultText\(lead\),'consult'\)[\s\S]*leadLevelText\(lead\)[\s\S]*leadProfileText\(lead\)[\s\S]*renderLeadTag\(leadStageText\(lead\),'stage'\)/, 'lead list rows should show normalized source, normalized consult, level, basic info, and stage in the table order');
...
assert.match(standardSource, /label:'咨询需求'[\s\S]*style:'width:160px'/, 'lead consult column should use the requested 160px width');
