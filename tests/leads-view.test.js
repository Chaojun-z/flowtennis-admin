...
assert.match(standardSource, /姓名[\s\S]*线索时间[\s\S]*来源[\s\S]*类型[\s\S]*需求产品[\s\S]*水平[\s\S]*基本信息[\s\S]*线索阶段[\s\S]*跟进优先级[\s\S]*跟进人[\s\S]*体验课时间[\s\S]*成交教练[\s\S]*流失原因[\s\S]*操作/, 'leads table should expose the requested reordered columns');
...
assert.match(leadsSource, /线索数[\s\S]*全盘最终成交[\s\S]*预约体验客户[\s\S]*体验课实到人数[\s\S]*体验后课程成交[\s\S]*高意向蓄水池/, 'lead stats should expose the unified lifecycle funnel metrics');
assert.match(leadsSource, /总成交人数 \/ 有效线索数[\s\S]*预约体验客户 \/ 有效线索数[\s\S]*已体验人数 \/ 预约体验客户[\s\S]*课程成交人数 \/ 已体验人数[\s\S]*已体验待成交 \/ 已体验人数/, 'lead stats should explain the unified formulas');
...
assert.match(leadsSource, /function leadBasicInfoReadonlyHtml\(lead\)[\s\S]*leadDetailFieldHtml\('姓名',leadWechatText\(lead\)\)[\s\S]*leadDetailFieldHtml\('电话',lead\?\.phone\|\|'-'\)[\s\S]*leadDetailFieldHtml\('水平',leadLevelText\(lead\)\)[\s\S]*leadDetailFieldHtml\('线索时间',leadDateDisplayText\(lead\)\)[\s\S]*leadDetailFieldHtml\('来源',leadSourceText\(lead\)\|\|'-'\)[\s\S]*leadDetailFieldHtml\('所属校区',leadCampusText\(lead\)\)[\s\S]*leadDetailFieldHtml\('类型',leadCustomerTypeText\(lead\)\|\|'-'\)[\s\S]*leadDetailFieldHtml\('需求产品',leadDemandProductText\(lead\)\|\|'-'\)[\s\S]*leadDetailFieldHtml\('意向类型',lead\?\.intentLevel\|\|'-'\)[\s\S]*leadDetailFieldHtml\('跟进优先级',leadPriorityText\(lead\)\)[\s\S]*leadDetailFieldHtml\('跟进人',lead\?\.owner\|\|'-'\)[\s\S]*leadDetailBlockHtml\('基本信息',esc\(leadProfileText\(lead\)\),\{hideEmpty:true\}\)/, 'lead detail basic tab should keep the current basic fields and normalize labels');
...
assert.match(leadsSource, /const body=`<div class="tms-form-row lead-form-row-4">[\s\S]*<label class="tms-form-label">姓名<\/label><input class="finput tms-form-control" id="lead_wechatName"[\s\S]*<label class="tms-form-label">电话<\/label><input class="finput tms-form-control" id="lead_phone"[\s\S]*<label class="tms-form-label">水平<\/label>\$\{leadLevelControlHtml\(lead\)\}[\s\S]*<label class="tms-form-label">线索时间<\/label>[\s\S]*<label class="tms-form-label">来源<\/label>[\s\S]*<label class="tms-form-label">所属校区<\/label>[\s\S]*<label class="tms-form-label">类型<\/label>[\s\S]*<label class="tms-form-label">需求产品<\/label>[\s\S]*<label class="tms-form-label">意向类型<\/label>[\s\S]*<label class="tms-form-label">跟进优先级<\/label>[\s\S]*leadPriorityOptions\(\)[\s\S]*<label class="tms-form-label">跟进人<\/label>[\s\S]*<label class="tms-form-label">基本信息<\/label>/, 'lead create and edit modal should use the requested field order without a section title');
