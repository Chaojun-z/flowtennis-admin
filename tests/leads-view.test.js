...
assert.match(standardSource, /微信名[\s\S]*线索时间[\s\S]*线索渠道[\s\S]*咨询需求[\s\S]*水平[\s\S]*基本信息[\s\S]*跟进优先级[\s\S]*跟进人[\s\S]*体验课时间[\s\S]*线索阶段[\s\S]*转化教练[\s\S]*未转化原因[\s\S]*操作/, 'leads table should expose the requested reordered columns');
...
assert.match(leadsSource, /function leadBasicInfoReadonlyHtml\(lead\)[\s\S]*leadDetailFieldHtml\('微信名',leadWechatText\(lead\)\)[\s\S]*leadDetailFieldHtml\('电话',lead\?\.phone\|\|'-'\)[\s\S]*leadDetailFieldHtml\('水平',leadLevelText\(lead\)\)[\s\S]*leadDetailFieldHtml\('线索时间',lead\?\.leadDate\|\|'-'\)[\s\S]*leadDetailFieldHtml\('线索来源',lead\?\.source\|\|'-'\)[\s\S]*leadDetailFieldHtml\('所属校区',leadCampusText\(lead\)\)[\s\S]*leadDetailFieldHtml\('咨询需求',lead\?\.consultType\|\|'-'\)[\s\S]*leadDetailFieldHtml\('意向类型',lead\?\.intentLevel\|\|'-'\)[\s\S]*leadDetailFieldHtml\('跟进优先级',leadPriorityText\(lead\)\)[\s\S]*leadDetailFieldHtml\('跟进人',lead\?\.owner\|\|'-'\)[\s\S]*leadDetailBlockHtml\('基本信息',esc\(leadProfileText\(lead\)\),\{hideEmpty:true\}\)/, 'lead detail basic tab should keep the current basic fields');
...
assert.match(fnBody('renderLeads'), /leadDateOnly\(lead\?\.leadDate,lead\)[\s\S]*renderLeadTag\(lead\?\.source,'source'\)[\s\S]*renderLeadTag\(lead\?\.consultType,'consult'\)[\s\S]*leadLevelText\(lead\)[\s\S]*leadProfileText\(lead\)/, 'lead list rows should show source, consult, level, and basic info in the table order');
