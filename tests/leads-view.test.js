...
assert.match(leadsSource, /function leadReadonlyCardHtml\(/, 'lead detail should expose a shared readonly card helper');
...
assert.match(leadDetailSource, /setCourtModalFrame\('线索详情'/, 'lead detail modal title should be lead detail');
assert.match(leadDetailSource, /leadDetailSectionHtml\('基础信息',leadReadonlyCardHtml\(`[\s\S]*leadDetailFieldHtml\('微信名',leadWechatText\(lead\)\)[\s\S]*leadDetailFieldHtml\('电话',lead\?\.phone\|\|'-'\)[\s\S]*leadDetailFieldHtml\('线索时间',lead\?\.leadDate\|\|'-'\)[\s\S]*leadDetailFieldHtml\('线索来源',lead\?\.source\|\|'-'\)[\s\S]*leadDetailFieldHtml\('所属校区',leadCampusText\(lead\)\)[\s\S]*leadDetailFieldHtml\('咨询需求',lead\?\.consultType\|\|'-'\)[\s\S]*leadDetailFieldHtml\('意向类型',lead\?\.intentLevel\|\|'-'\)[\s\S]*leadDetailFieldHtml\('跟进人',lead\?\.owner\|\|'-'\)[\s\S]*leadDetailBlockHtml\('基本信息',esc\(leadProfileText\(lead\)\),\{hideEmpty:true\}\)[\s\S]*`,'lead-readonly-card lead-readonly-card-4'\)/, 'lead detail basic info should use the requested four-column readonly card');
assert.doesNotMatch(leadDetailSource, /leadDetailSectionHtml\('当前跟进'/, 'lead detail should remove the separate current follow-up section');
assert.doesNotMatch(leadDetailSource, /leadDetailSectionHtml\('转化关系'/, 'lead detail should remove the separate conversion relation section');
assert.match(leadDetailSource, /leadDetailSectionHtml\('跟进时间线'/, 'lead detail should keep the timeline section');
...
assert.match(leadsSource, /function leadTimelineHtml\(lead\)[\s\S]*lead-timeline-edit[^>]*onclick="openLeadFollowupModal\('\$\{lead\.id\}','\$\{item\.id\}'\)"/, 'lead timeline edit action should use the shared hover-to-edit button');
assert.doesNotMatch(leadsSource, /lead-timeline-item"><span>\$\{esc\(leadTimelineLineText\(item\)\)\}<\/span><span class="tms-action-link"/, 'lead timeline should not show edit as always-visible text');
...
assert.match(leadDetailSource, /<button class="tms-btn tms-btn-default" onclick="convertLeadToStudent\('\$\{lead\.id\}'\)">转学员<\/button><button class="tms-btn tms-btn-default" onclick="convertLeadToCourt\('\$\{lead\.id\}'\)">转订场<\/button><button class="tms-btn tms-btn-primary" onclick="openLeadModal\('\$\{lead\.id\}'\)">编辑<\/button>/, 'lead detail footer should only show convert student, convert court, and highlighted edit');
...
assert.match(css, /\.modal\.modal-court \.lead-timeline-list\{[^}]*position:relative[^}]*padding-left:22px/, 'lead timeline should reserve space for the shared vertical axis');
assert.match(css, /\.modal\.modal-court \.lead-timeline-list::before\{[^}]*width:2px[^}]*background:#E6E1DC/, 'lead timeline should use the standard subtle vertical axis');
assert.match(css, /\.modal\.modal-court \.lead-timeline-item\{[^}]*position:relative[^}]*opacity:.6[^}]*transition:opacity \.2s ease/, 'lead timeline history rows should stay visually quiet until hover');
assert.match(css, /\.modal\.modal-court \.lead-timeline-item::before\{[^}]*width:12px[^}]*height:12px[^}]*background:#E6E1DC[^}]*box-shadow:0 0 0 4px #F5F2F0/, 'lead timeline dots should cut the axis with the modal-surface ring');
assert.match(css, /\.modal\.modal-court \.lead-timeline-item:first-child::before\{background:#D19356\}/, 'latest lead timeline dot should use the primary color');
assert.match(css, /\.modal\.modal-court \.lead-timeline-edit\{[^}]*position:absolute[^}]*top:0[^}]*right:0[^}]*opacity:0[^}]*padding:6px[^}]*border-radius:6px[^}]*background:#FFFFFF[^}]*border:1px solid #E6E1DC[^}]*box-shadow:0 1px 2px rgba\(0,0,0,\.05\)/, 'lead timeline edit button should be a hidden hover-to-edit control');
assert.match(css, /\.modal\.modal-court \.lead-timeline-item:hover \.lead-timeline-edit\{opacity:1\}/, 'lead timeline row hover should reveal the edit button');
assert.match(css, /\.modal\.modal-court \.lead-timeline-edit:hover\{[^}]*color:#D19356/, 'lead timeline edit button hover should turn the icon primary');
