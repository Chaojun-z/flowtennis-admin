...
assert.match(html, /<th style="width:110px;padding-left:20px">日期<\/th><th style="width:110px">时间段<\/th>/, 'precheck table should split date and time range into separate columns');
...
assert.match(html, /会员资料只同步到资料层，不需要人工处理/, 'member profile note should explain why profiles are not booking confirmations');
assert.match(html, /用户名[\s\S]*手机号[\s\S]*订场方式[\s\S]*操作账号[\s\S]*备注[\s\S]*金额/, 'sync center should expose operator-facing third party fields');
...
assert.match(html, /本表只显示需要运营确认的订场记录/, 'precheck tab should not show auto-import rows as readonly noise');
...
assert.match(html, /同步记录[\s\S]*预检确认[\s\S]*写入回滚[\s\S]*异常处理/, 'sync center should expose four business table tabs');
assert.match(html, /写入结果[\s\S]*失败原因/, 'sync center should show import results and failure reasons');
assert.match(html, /第三方变更待处理[\s\S]*运营要做什么[\s\S]*异常待处理/, 'sync center should expose business-facing change and alert handling');
