...
assert.match(html, /renderCourtDropdownHtml\('nrType','交易类型'/, 'court finance modal first dropdown should be transaction type');
assert.match(html, /renderCourtDropdownHtml\('nrCategory','业务类型'/, 'court finance modal second dropdown should be field business subtype');
assert.match(html, /renderCourtDropdownHtml\('nrPayMethod','支付方式'/, 'court finance modal third dropdown should be payment method');
assert.doesNotMatch(html, /renderCourtDropdownHtml\('nrType','类型'|renderCourtDropdownHtml\('nrCategory','项目'|renderCourtDropdownHtml\('nrPayMethod','支付'/, 'court finance modal should not keep old dropdown labels');
