...
assert.match(html, /function courtPayMethodOptions\([\s\S]*FlowTennisBusinessTaxonomy\.optionList\('payMethods'\)/, 'court finance payment dropdown should use the global payment dictionary');
...
assert.match(html, /COURT_FINANCE_TRANSACTION_TYPES=FlowTennisBusinessTaxonomy\.TRANSACTION_TYPES/, 'court finance modal should use the global transaction dictionary');
assert.match(html, /COURT_FINANCE_BUSINESS_TYPES=FlowTennisBusinessTaxonomy\.COURT_FINANCE_BUSINESS_TYPES/, 'court finance modal should use the global field business dictionary');
