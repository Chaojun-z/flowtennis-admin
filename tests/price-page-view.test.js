...
assert.match(html, /同步默认马坡价格/, 'price page should expose default Mabao price sync');
assert.match(html, /新增价格/, 'price page should expose one generic create button');
assert.match(html, /tms-btn-ghost" onclick="importDefaultMabaoPrices\(\)">同步默认马坡价格/, 'price sync button should match student page secondary action style');
...
assert.match(html, /style="width:360px">名称/, 'price table should further expand the name column for readability');
assert.ok(styles.includes('padding-left:5px;padding-right:5px'), 'price table should further tighten cell padding to reduce column gaps');
assert.ok(styles.includes('#page-prices .price-table{min-width:1160px;table-layout:fixed}'), 'price table should shrink overall width so more columns fit on one screen');
