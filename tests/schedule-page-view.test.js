...
assert.match(indexHtml, /state\.js\?v=20260920-schedule-helper-v1/, 'state script version should force a fresh browser load after schedule renderer recovery asset updates');
assert.match(indexHtml, /schedule\.js\?v=20260920-student-ids-v1/, 'schedule script version should force a fresh browser load after repeat scheduling fixes');
...
function scheduleVmContext(overrides = {}){
  ...
  vm.runInContext(scheduleHelperSource, context);
...
assert.doesNotMatch(fnBody('applySchEntitlementOptions'), /maxRemain/, 'schedule package picker should keep the backend recommended order instead of selecting the largest remaining package');
assert.match(fnBody('applySchEntitlementOptions'), /selectableOptions\.find\(x=>preferredId&&x\.entitlementId===preferredId\)\|\|selectableOptions\[0\]/, 'schedule package picker should default to the first backend recommendation when there is no preferred package');
...
assert.match(source, /schedule:\['students','courts','coaches','coachProposals','classes','feedbacks','entitlements','entitlementLedger','lifecycleMetricsPage','financePage','courtAccountListViewPage'\]/, 'schedule page should refresh detail/search and membership balance datasets in the background after first paint');
...
assert.match(source, /function refreshCoachOpsWorkbenchAfterScheduleMutation\(/, 'schedule saves should refresh the active coach schedule read model');
assert.match(fnBody('refreshCoachOpsWorkbenchAfterScheduleMutation'), /currentPage==='coachschedule'\?'coachSchedulePage':currentPage==='coachops'\?'workbenchPage'/, 'schedule saves should refresh the lightweight calendar read model on the calendar page');
assert.match(fnBody('renderAfterScheduleMutation'), /renderSchedule\(\);renderCoachOps\(\);renderMySchedule\(\);refreshCoachOpsWorkbenchAfterScheduleMutation\(\);/, 'schedule saves should rerender and then refresh coach ops from the active read model source');
...
assert.match(styles, /\.schedule-entitlement-item \.tms-dropdown-menu\{[^}]*width:492px/, 'schedule package dropdown menu should be wider than the input field');
...
assert.match(fnBody('openScheduleModal'), /sch_settlementType[\s\S]*sch_fieldFeeMode[\s\S]*schedule-payment-row[\s\S]*sch_payMethod[\s\S]*sch_paidAmount[\s\S]*sch_fieldFeePayMethod[\s\S]*sch_fieldFeeAmount/, 'schedule modal should split settlement, field fee mode, lesson pay method, lesson fee, field fee pay method, and field fee');
