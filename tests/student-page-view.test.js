...
assert.match(fnBody('studentTableColumns'), /return \[\s*\n    \{label:'姓名'[\s\S]*label:'类型',style:'width:72px'[\s\S]*label:'来源'[\s\S]*label:'校区'[\s\S]*label:'成交路径'[\s\S]*studentSortHeader\('packagePurchaseDate','课包购买时间'\)[\s\S]*label:'课包',style:'width:260px'[\s\S]*label:'负责教练'[\s\S]*studentSortHeader\('packageLessons','课包余额'\)[\s\S]*studentSortHeader\('completedLessons','累计上课'\)[\s\S]*label:'备注',style:'width:280px'/, 'official student table should use the requested column order, wider type column, package name, package balance label, and wider note column');
