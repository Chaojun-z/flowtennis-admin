...
assert.match(fnBody('saveStudent'), /if\(phone&&!validateCnPhone\(phone\)\)/, 'student save should allow empty phone but still validate filled phone');
...
assert.match(fnBody('studentFilteredSummaryForMode'), /FlowTennisPlatformDataStandards\.currentStudentSummary\(base,studentListViewMode\(\)\)/, 'student filtered top stats should use the shared summary helper over the full filtered unified row set');
assert.match(fnBody('studentPageStats'), /studentHasActiveSearchOrFilter\(\)[\s\S]*studentFilteredSummaryForMode\(base\)/, 'student local search and toolbar filters must make top cards follow the same full filtered row set as the list');
...
assert.match(fnBody('goPage'), /document\.body\.classList\.toggle\('is-active-students-page',pg==='package-students'\)/, 'active student page should toggle a page-only body class for scoped toolbar visibility');
assert.match(css, /body\.is-active-students-page #page-students \.tms-toolbar-right \.tms-export-action,\s*body\.is-active-students-page #page-students \.tms-toolbar-right \.tms-btn-primary\{display:none!important\}/, 'active student page should hide the export and add-student toolbar entries for every account');
assert.doesNotMatch(css, /(?:^|\n)#page-students \.tms-toolbar-right \.tms-export-action,[\s\S]*?display:none!important/, 'student toolbar actions should not be hidden unconditionally across historical students');
...
assert.match(fnBody('studentUnifiedPackageBalanceHtml'), /studentPackageLessonMeta\(stu\)/, 'student list package balance should use the same total-package helper as the drawer');
assert.match(fnBody('studentSortValue'), /if\(key==='packageLessons'\)return studentPackageRemainingLessons\(stu\);/, 'student package sort should use the same total-package helper as the drawer');
...
assert.match(source, /ensureStudentDetailData\(id,\{force:false\}\)/, 'student detail should load fast published detail facts by student id instead of scanning raw aggregates');
...
assert.match(source, /function studentLessonRecordDetailSectionText\([\s\S]*if\(row\.lessonSectionText\)return row\.lessonSectionText;[\s\S]*const formatSection=value=>unit==='次'[\s\S]*const startText=formatSection\(startNo\)[\s\S]*return `\[第/, 'backend unified student lesson detail rows should show current package count labels for small class lessons');
assert.match(fnBody('studentLessonRecordHtml'), /const title=studentLessonRecordTitleText\(item,rows,index\);[\s\S]*const sourceText=studentLessonRecordSourceText\(item,stu,rows,index\);/, 'backend unified student lesson detail rows should render title and source through dedicated helpers');
assert.match(fnBody('studentLessonRecordHtml'), /student-lesson-relation[\s\S]*student-lesson-title[\s\S]*student-lesson-meta[\s\S]*student-lesson-source[\s\S]*\$\{relation\}/, 'student lesson records should use the agreed three-line structure with optional relation text');
...
assert.match(source, /function studentCumulativeLessonSectionText\([\s\S]*const entitlementId=String\(row\?\.entitlementId\|\|ent\?\.id\|\|row\?\.purchaseId\|\|ent\?\.purchaseId\|\|''\)\.trim\(\)[\s\S]*studentEntitlementLedgerRows\(\{id:studentId\}\)[\s\S]*String\(item\.entitlementId\|\|''\)\.trim\(\)===entitlementId[\s\S]*function studentLessonRecordSectionText\([\s\S]*const cumulativeText=studentCumulativeLessonSectionText\(row,ent\);[\s\S]*return cumulativeText\|\|'';/, 'student lesson record section numbers should reset within the current package only');
...
assert.match(fnBody('studentLessonRecordTitleText'), /pendingStudentLessonSequenceText[\s\S]*studentLessonSequenceText[\s\S]*className\|\|item\.packageName\|\|item\.courseDisplayName\|\|item\.courseType[\s\S]*pendingSequenceText\|\|studentSequenceText\|\|fallbackSection/, 'student lesson record title should show pending estimated cumulative sequence before falling back to current-package count');
assert.match(fnBody('studentLessonRecordSourceText'), /lessonSourceText[\s\S]*课包扣课[\s\S]*赠送课｜免费[\s\S]*单次付费[\s\S]*历史记录｜未关联订单/, 'student lesson record source should use unified backend text and clear fallbacks for package, gift, direct-pay, and historical rows');
...
assert.match(source, /\{key:'purchases'[\s\S]*columns:\[\{label:'姓名',style:'width:120px;padding-left:20px'\},\{style:'width:100px',html:'<button class="tms-sort-header" data-purchase-sort="purchaseDate" onclick="cyclePurchaseSort\(\)">报名时间[\s\S]*\{label:'课程类型',style:'width:110px'\},\{label:'用户类型',style:'width:64px'\},\{label:'上课人数',style:'width:86px'\},\{label:'在期状态',style:'width:108px'\},\{label:'付费状态',style:'width:74px'\},\{label:'报名课时',style:'width:74px'\},\{label:'报名费用',style:'width:84px'\},\{label:'归属教练',style:'width:90px'\},\{label:'赠送课时',style:'width:74px'\},\{label:'课包余额',style:'width:92px'\}/, 'purchase record table should follow the new column order');
...
assert.match(source, /function onPurchaseSearchChange\(\)[\s\S]*purPage=standardListFirstPage\(\)[\s\S]*renderPurchases\(\)/, 'purchase search should reset pagination through the standard list flow');
assert.match(source, /function purchaseSelectedPackageFilter\(\)[\s\S]*purPackageFilterValue\|\|''/, 'purchase package drilldown should survive navigation before the page shell is rendered');
assert.match(source, /function onPurchaseCourseTypeFilterChange\(\)[\s\S]*purCourseTypeFilterValue=document\.getElementById\('purCourseTypeFilter'\)\?\.value\|\|''[\s\S]*renderPurchases\(\)/, 'purchase course type filter should update the page state');
assert.match(source, /function onPurchaseInPeriodFilterChange\(\)[\s\S]*purInPeriodFilterValue=document\.getElementById\('purInPeriodFilter'\)\?\.value\|\|''[\s\S]*renderPurchases\(\)/, 'purchase in-period filter should update the page state');
assert.match(source, /function onPurchasePayStatusFilterChange\(\)[\s\S]*purPayStatusFilterValue=document\.getElementById\('purPayStatusFilter'\)\?\.value\|\|''[\s\S]*renderPurchases\(\)/, 'purchase paid-status filter should update the page state');
