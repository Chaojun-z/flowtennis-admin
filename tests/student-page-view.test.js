...
assert.match(source, /stuDealPathFilterHost[\s\S]*onStudentFilterChange/, 'official student deal path filter should reset pagination before rendering');
assert.match(fnBody('renderStudentToolbarFilters'), /key:'type'[\s\S]*key:'source'[\s\S]*key:'dealPath'[\s\S]*key:'coach'/, 'student filters should render in the requested type, source, deal path, coach order');
assert.match(fnBody('renderStudentToolbarFilters'), /thirdFilterIsTrial=studentListViewMode\(\)==='trial'[\s\S]*emptyDisplay:thirdFilterIsTrial\?'体验状态':'成交路径'/, 'student third filter should be trial status for normal students and deal path for official students');
...
assert.match(source, /studentListViewMode\(\)==='trial'[\s\S]*普通学员[\s\S]*课程服务学员[\s\S]*体验路径学员[\s\S]*体验路径未成交[\s\S]*体验路径成交[\s\S]*直接成交学员/, 'trial student top stats should show course-chain funnel cards');
...
assert.match(source, /data-student-sort="packagePurchaseDate"[\s\S]*课包购买时间/, 'student table should sort by package purchase date');
...
assert.match(source, /function studentTableColumns\(\)[\s\S]*studentListViewMode\(\)==='trial'[\s\S]*label:'姓名',className:'tms-sticky-l',style:'width:130px;padding-left:20px'[\s\S]*label:'来源',style:'width:110px'[\s\S]*label:'类型',style:'width:90px'[\s\S]*label:'校区'[\s\S]*label:'体验状态'[\s\S]*studentSortHeader\('packagePurchaseDate','课包购买时间'\)[\s\S]*label:'课包'[\s\S]*label:'负责教练'[\s\S]*label:'备注'/, 'normal student table should use the requested column order and shared identity widths');
...
assert.match(fnBody('studentTableColumns'), /return \[\s*\n    \{label:'姓名',className:'tms-sticky-l',style:'width:130px;padding-left:20px'\}[\s\S]*label:'来源',style:'width:110px'[\s\S]*label:'类型',style:'width:90px'[\s\S]*label:'校区'[\s\S]*label:'成交路径'[\s\S]*studentSortHeader\('packagePurchaseDate','课包购买时间'\)[\s\S]*label:'课包',style:'width:260px'[\s\S]*label:'负责教练'[\s\S]*studentSortHeader\('packageLessons','课包余额'\)[\s\S]*studentSortHeader\('completedLessons','累计上课'\)[\s\S]*label:'备注',style:'width:280px'/, 'official student table should use the requested column order, shared identity widths, package name, package balance label, and wider note column');
...
assert.match(source, /style:'width:110px',html:'<button class="tms-sort-header" data-student-sort="packagePurchaseDate"[\s\S]*课包购买时间[\s\S]*label:'课包'/, 'normal student table should place package purchase date before package name');
...
assert.match(source, /filterHostIds:\['stuTypeFilterHost','stuSourceFilterHost','stuTrialStatusFilterHost','stuDealPathFilterHost','stuCoachFilterHost'\]/, 'student third filter should sit after source in the toolbar');
assert.match(source, /renderStudentToolbarFilters[\s\S]*withLinkedFilterCounts\(\[[\s\S]*key:'type'[\s\S]*key:'source'[\s\S]*key:'dealPath'[\s\S]*key:'coach'/, 'student toolbar filters should use linked count labels');
...
assert.match(source, /emptyDisplay:thirdFilterIsTrial\?'体验状态':'成交路径'[\s\S]*thirdFilterIsTrial\?studentTrialStatusOptions\(\):studentDealPathOptions\(\)[\s\S]*thirdFilterIsTrial\?studentTrialPathStatusText\(s\)===value:studentDealPathText\(s\)===value/, 'student third filter should switch between normalized trial status and official deal path');
...
assert.match(fnBody('getFilteredStudents'), /const dealPathFilter=document\.getElementById\('stuDealPathFilter'\)\?\.value\|\|''[\s\S]*if\(dealPathFilter&&studentDealPathText\(s\)!==dealPathFilter\)return false/, 'official student deal path filter should apply to the student list');
...
assert.match(fnBody('renderStudents'), /renderStandardCellText\(studentSourceText\(s\),false\)[\s\S]*renderStandardCellText\(studentDealPathText\(s\),false\)[\s\S]*renderStandardTooltipText\(noteText,'tms-text-remark tms-text-remark-1 student-note-cell'\)/, 'package student list should render source and deal path as plain text with one-line notes');
