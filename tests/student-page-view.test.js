...
assert.match(source, /function studentActiveEntitlementRows\([\s\S]*entitlementStatusText\(e\)==='正常'[\s\S]*lessonValue\(e\.remainingLessons\)>0/, 'student lesson/package summary should only show currently usable remaining packages');
...
assert.match(source, /function studentLessonRecordSectionText\([\s\S]*padStart\(2,'0'\)[\s\S]*第\$\{pad\(startNo\)\}-\$\{pad\(endNo\)\}节/, 'student lesson record rows should show lesson section numbers');
assert.match(source, /function studentLessonRecordPackageHtml\([\s\S]*<strong>扣\$\{lessonQty\(Math\.abs\(Number\(row\.lessonDelta\)\|\|0\)\)\}节（\$\{lessonQty\(balance\.remaining\)\}\/\$\{lessonQty\(balance\.total\)\}）<\/strong>/, 'student lesson record package rows should bold deduct lessons and balance');
