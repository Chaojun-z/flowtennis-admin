...
assert.match(fnBody('studentPageStats'), /historicalTrialAttendedCount[\s\S]*historicalFormalAttendedCount[\s\S]*historicalTrialWithoutFormalCount[\s\S]*historicalFormalLesson30Count[\s\S]*activeFormalLesson30Count[\s\S]*activeFormalLesson90Count[\s\S]*activePackageBalanceCount[\s\S]*activePackageLowCount/, 'student top stats should read explicit backend card fields');
...
assert.match(fnBody('studentTableColumns'), /label:'姓名'[\s\S]*label:'来源'[\s\S]*label:'类型'[\s\S]*label:'校区'[\s\S]*label:'活跃状态'[\s\S]*label:'付费方式'[\s\S]*label:'课包状态'[\s\S]*label:'课包余额'[\s\S]*label:'历史课时'[\s\S]*label:'学员状态'[\s\S]*label:'负责教练'[\s\S]*label:'备注'/, 'student table should use the new active-label column order with package balance');
...
assert.match(fnBody('renderStudents'), /studentActivityStatusText\(s\)[\s\S]*studentPaymentModeText\(s\)[\s\S]*studentPackageStatusText\(s\)[\s\S]*studentUnifiedPackageBalanceHtml\(s\)[\s\S]*studentLessonVolumeText\(s\)[\s\S]*studentLifecycleStatusText\(s\)/, 'student table rows should render the new label columns and backend package balance');
assert.doesNotMatch(fnBody('renderStudents'), /studentPackageListHtml\(s\)|studentPackageListTooltip\(s\)|studentPackageLessonMiniBar\(s\)|studentCompletedLessonCount\(s\)|studentUnifiedPackageListHtml\(s\)|studentUnifiedCompletedLessonCount\(s\)/, 'student table rows should not render old package-name or cumulative-lesson columns');
