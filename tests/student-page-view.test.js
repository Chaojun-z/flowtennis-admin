...
assert.match(source, /stuPackageStatusFilterHost[\s\S]*onStudentFilterChange/, 'student package status filter should reset pagination before rendering');
assert.match(source, /stuPaymentModeFilterHost[\s\S]*onStudentFilterChange/, 'student payment mode filter should reset pagination before rendering');
assert.match(source, /stuActivityStatusFilterHost[\s\S]*onStudentFilterChange/, 'student activity status filter should reset pagination before rendering');
assert.match(source, /stuLessonVolumeFilterHost[\s\S]*onStudentFilterChange/, 'student lesson volume filter should reset pagination before rendering');
assert.match(source, /stuLifecycleStatusFilterHost[\s\S]*onStudentFilterChange/, 'student lifecycle status filter should reset pagination before rendering');
...
assert.match(fnBody('renderStudentToolbarFilters'), /key:'type'[\s\S]*key:'source'[\s\S]*key:'packageStatus'[\s\S]*key:'paymentMode'[\s\S]*key:'activityStatus'[\s\S]*key:'lessonVolume'[\s\S]*key:'lifecycleStatus'[\s\S]*key:'coach'/, 'student filters should render type, source, tag filters, coach order');
...
assert.match(source, /studentListViewMode\(\)==='trial'[\s\S]*历史学员[\s\S]*累计来上过课[\s\S]*近30天活跃[\s\S]*课包有余额[\s\S]*稳定单次付费[\s\S]*从未正式上课/, 'historical student top stats should show active tag cards');
...
assert.match(fnBody('studentPageStats'), /studentStandardSummaryForMode\(\)/, 'student stats should summarize package facts from the backend standard summary');
...
assert.match(source, /label:'历史课时'/, 'student table should show lesson volume');
assert.match(source, /label:'学员状态'/, 'student table should show lifecycle status');
assert.match(fnBody('studentTableColumns'), /label:'姓名'[\s\S]*label:'来源'[\s\S]*label:'类型'[\s\S]*label:'校区'[\s\S]*label:'活跃状态'[\s\S]*label:'付费方式'[\s\S]*label:'课包状态'[\s\S]*label:'历史课时'[\s\S]*label:'学员状态'[\s\S]*label:'负责教练'[\s\S]*label:'备注'/, 'student table should use the new active-label column order');
...
assert.match(fnBody('renderStudents'), /studentLessonVolumeText\(s\)/, 'student list should render historical lesson volume through the new label column');
...
assert.match(fnBody('renderStudents'), /studentActivityStatusText\(s\)[\s\S]*studentPaymentModeText\(s\)[\s\S]*studentPackageStatusText\(s\)[\s\S]*studentLessonVolumeText\(s\)[\s\S]*studentLifecycleStatusText\(s\)/, 'student table rows should render the new label columns');
assert.doesNotMatch(fnBody('renderStudents'), /studentPackageListHtml\(s\)|studentPackageListTooltip\(s\)|studentPackageLessonMiniBar\(s\)|studentCompletedLessonCount\(s\)|studentUnifiedPackageListHtml\(s\)|studentUnifiedPackageBalanceHtml\(s\)|studentUnifiedCompletedLessonCount\(s\)/, 'student table rows should not render the old package-detail columns');
...
assert.match(source, /filterHostIds:\['stuTypeFilterHost','stuSourceFilterHost','stuPackageStatusFilterHost','stuPaymentModeFilterHost','stuActivityStatusFilterHost','stuLessonVolumeFilterHost','stuLifecycleStatusFilterHost','stuCoachFilterHost'\]/, 'student toolbar should expose all student label filters');
assert.match(source, /renderStudentToolbarFilters[\s\S]*withLinkedFilterCounts\(\[[\s\S]*key:'type'[\s\S]*key:'source'[\s\S]*key:'packageStatus'[\s\S]*key:'paymentMode'[\s\S]*key:'activityStatus'[\s\S]*key:'lessonVolume'[\s\S]*key:'lifecycleStatus'[\s\S]*key:'coach'/, 'student toolbar filters should use linked count labels for the new student labels');
...
assert.match(fnBody('getFilteredStudents'), /const packageStatusFilter=document\.getElementById\('stuPackageStatusFilter'\)\?\.value\|\|''[\s\S]*if\(packageStatusFilter&&studentPackageStatusText\(s\)!==packageStatusFilter\)return false/, 'student package status filter should apply to the student list');
assert.match(fnBody('getFilteredStudents'), /const paymentModeFilter=document\.getElementById\('stuPaymentModeFilter'\)\?\.value\|\|''[\s\S]*if\(paymentModeFilter&&studentPaymentModeText\(s\)!==paymentModeFilter\)return false/, 'student payment mode filter should apply to the student list');
assert.match(fnBody('getFilteredStudents'), /const activityStatusFilter=document\.getElementById\('stuActivityStatusFilter'\)\?\.value\|\|''[\s\S]*if\(activityStatusFilter&&studentActivityStatusText\(s\)!==activityStatusFilter\)return false/, 'student activity status filter should apply to the student list');
assert.match(fnBody('getFilteredStudents'), /const lessonVolumeFilter=document\.getElementById\('stuLessonVolumeFilter'\)\?\.value\|\|''[\s\S]*if\(lessonVolumeFilter&&studentLessonVolumeText\(s\)!==lessonVolumeFilter\)return false/, 'student lesson volume filter should apply to the student list');
assert.match(fnBody('getFilteredStudents'), /const lifecycleStatusFilter=document\.getElementById\('stuLifecycleStatusFilter'\)\?\.value\|\|''[\s\S]*if\(lifecycleStatusFilter&&studentLifecycleStatusText\(s\)!==lifecycleStatusFilter\)return false/, 'student lifecycle status filter should apply to the student list');
assert.match(fnBody('studentHasActiveSearchOrFilter'), /stuPackageStatusFilter[\s\S]*stuPaymentModeFilter[\s\S]*stuActivityStatusFilter[\s\S]*stuLessonVolumeFilter[\s\S]*stuLifecycleStatusFilter/, 'student empty state should treat the new student label filters as active filters');
