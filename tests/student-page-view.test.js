...
assert.match(source, /function studentEntitlementSummaryHtml\([\s\S]*detailPackageOrderRows[\s\S]*student-package-card[\s\S]*报名[\s\S]*应付\$\{fmt\(systemAmount\)\} · 实付\$\{fmt\(paidAmount\)\}[\s\S]*\$\{esc\(renderStandardEmptyText\(ownerCoach\)\)\}[\s\S]*<strong>已扣 \$\{lessonQty\(used\)\}\$\{unit\}（\$\{lessonQty\(remaining\)\}\/\$\{lessonQty\(total\)\}\$\{unit\}）<\/strong>/, 'student package purchase records should show backend unified signup, price, owner, and bold used balance with the package unit');
