...
assert.match(source, /function studentLessonRecordPackageText\([\s\S]*studentLedgerBalanceAfter\(row,ent\)[\s\S]*扣\$\{lessonQty\(Math\.abs\(Number\(row\.lessonDelta\)\|\|0\)\)\}节[\s\S]*standardPackageLabel/, 'student lesson record package rows should show deduct lessons, per-lesson balance, and package name');
...
assert.match(source, /function studentLessonRecordChargeHtml\([\s\S]*免费送课[\s\S]*扣\$\{lessonQty\(Math\.abs\(Number\(row\.lessonDelta\)\|\|0\)\)\}节/, 'student lesson records should show free lessons without deducting package balance');
assert.match(source, /function studentLessonRecordChargeHtml\([\s\S]*<strong>扣\$\{lessonQty\(Math\.abs\(Number\(row\.lessonDelta\)\|\|0\)\)\}节（\$\{lessonQty\(balance\?\.remaining\)\}\/\$\{lessonQty\(balance\?\.total\)\}）<\/strong>/, 'student lesson record package rows should bold deduct lessons and balance');
