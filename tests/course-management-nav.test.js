...
assert.match(html, /package-card-created">创建 \$\{esc\(packageCreatedDate\(p\)\)\}<\/div>/, 'package cards should keep created time as weak footer metadata');
...
assert.match(fnBody('openPackageModal'), /基础信息[\s\S]*课程类型[\s\S]*上课人数[\s\S]*课时[\s\S]*上课时间[\s\S]*时段类型[\s\S]*可用时段[\s\S]*活动时间[\s\S]*可用时间[\s\S]*教练和场地[\s\S]*归属教练[\s\S]*可上课教练[\s\S]*可用校区/, 'package modal should follow the standardized package creation layout');
