...
assert.match(purchaseShellConfig, /columns:\[\{label:'姓名',style:'width:120px;padding-left:20px'\},\{style:'width:100px',html:'<button class="tms-sort-header" data-purchase-sort="purchaseDate" onclick="cyclePurchaseSort\(\)">报名时间/, 'purchase table should put sortable purchase date in the second column');
assert.match(fnBody('renderPurchases'), /<td class="purchase-name-cell" style="padding-left:20px">[\s\S]*<td>\$\{renderStandardCellText\(p\.purchaseDate,false\)\}<\/td><td>\$\{renderPurchaseTag\(purchaseCourseTypeText\(p\)/, 'purchase rows should render name, purchase date, then course type');
assert.match(fnBody('renderPurchases'), /purchase-name-cell[\s\S]*renderStandardCellText\(p\.studentName,false\)/, 'purchase name column should expose a dedicated bold styling hook');
assert.match(pagesCss, /#page-purchases \.purchase-name-cell \.tms-cell-text\{[^}]*font-weight:700/, 'purchase name column should be bold');
assert.match(fnBody('getFilteredPurchases'), /purchaseSortDir==='asc'\?1:-1/, 'purchase list should support ascending and descending purchase date sorting');
assert.match(html, /function updatePurchaseSortHeaders\([\s\S]*data-purchase-sort[\s\S]*classList\.toggle\('asc'[\s\S]*classList\.toggle\('desc'/, 'purchase sort header should use the existing sort icon active classes');
...
assert.match(fnBody('purchaseCourseTypeOptions'), /purchaseFilterRows\(\)/, 'purchase course type counts should use the unified purchase rows');
assert.match(fnBody('purchaseInPeriodStatusOptions'), /purchaseFilterRows\(\)/, 'purchase in-period counts should use the unified purchase rows');
assert.match(fnBody('purchasePayStatusOptions'), /purchaseFilterRows\(\)/, 'purchase paid-status counts should use the unified purchase rows');
assert.match(fnBody('purchaseCourseTypeTagClass'), /'私教课'[\s\S]*tms-tag-course-private[\s\S]*'体验课 \/ 私教体验课'[\s\S]*tms-tag-tier-blue[\s\S]*'陪打'[\s\S]*tms-tag-course-partner/, 'purchase course type tags should use distinct colors across options');
assert.match(fnBody('purchaseInPeriodStatusTagClass'), /'在期'[\s\S]*tms-tag-green[\s\S]*'已用完-续课'[\s\S]*tms-tag-tier-gold[\s\S]*'已用完-未续课'[\s\S]*tms-tag-red/, 'purchase in-period tags should use green, gold, and red states');
assert.match(fnBody('purchasePaidStatusTagClass'), /'首次'[\s\S]*tms-tag-tier-blue[\s\S]*'续报'[\s\S]*tms-tag-tier-teal/, 'purchase paid-status tags should use different colors for first and renewal purchases');
...
assert.match(purchaseShellConfig, /label:'姓名',style:'width:120px;padding-left:20px'[\s\S]*data-purchase-sort="purchaseDate"[\s\S]*报名时间[\s\S]*label:'课程类型',style:'width:110px'[\s\S]*label:'用户类型',style:'width:64px'[\s\S]*label:'上课人数',style:'width:86px'[\s\S]*label:'在期状态',style:'width:108px'[\s\S]*label:'付费状态',style:'width:74px'[\s\S]*label:'报名课时',style:'width:74px'[\s\S]*label:'报名费用',style:'width:84px'[\s\S]*label:'归属教练',style:'width:90px'[\s\S]*label:'赠送课时',style:'width:74px'[\s\S]*label:'课包余额',style:'width:92px'/, 'purchase table should follow the new purchase record columns');
assert.match(fnBody('renderPurchases'), /renderPurchaseTag\(purchaseCourseTypeText\(p\),purchaseCourseTypeTagClass\(purchaseCourseTypeText\(p\)\)\)[\s\S]*renderPurchaseTag\(purchaseUserTypeText\(p\),purchaseUserTypeTagClass\(purchaseUserTypeText\(p\)\)\)[\s\S]*renderPurchaseTag\(balanceStatus,balanceTagClass\)[\s\S]*renderPurchaseTag\(purchasePaidStatusText\(p\),purchasePaidStatusTagClass\(purchasePaidStatusText\(p\)\)\)/, 'purchase rows should render course type, user type, in-period, and paid status as tags');
