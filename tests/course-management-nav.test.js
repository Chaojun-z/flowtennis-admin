...
assert.match(html, /function packageBoardColumnKey\([\s\S]*packageStoredBoardColumnKey\(p\)[\s\S]*packageIsChaojunOwned\(p\)[\s\S]*chaojun[\s\S]*小班体验课[\s\S]*小班课[\s\S]*私教课[\s\S]*other/, 'package board should classify Chaojun packages before the normal teaching columns and keep an other fallback');
...
assert.doesNotMatch(fnBody('dropPackageCard'), /packageBoardColumnKey\(dragged\)!==packageBoardColumnKey\(target\)[\s\S]*return/, 'package drag sorting should support moving between board columns');
assert.match(fnBody('dropPackageCard'), /targetColumnKey[\s\S]*savePackageBoardMove\(ordered,targetColumnKey\)/, 'package card drops should send the target board column into the save flow');
assert.match(fnBody('savePackageBoardMove'), /boardColumn=targetColumnKey[\s\S]*savePackageOrder\(orderedIds,columnById\)/, 'package drag sorting should persist the target board column');
assert.match(fnBody('packageBoardCardHtml'), /ondrop="dropPackageCard\(event,'\$\{p\.id\}'\)"/, 'package cards should accept package drops');
assert.match(fnBody('renderPackages'), /ondrop="dropPackageToColumn\(event,'\$\{esc\(col\.key\)\}'\)"/, 'empty package columns should accept package drops');
assert.match(html, /async function dropPackageToColumn\(/, 'package board should support dropping into an empty column');
assert.match(apiSource, /path==='\/packages\/order'&&method==='PUT'[\s\S]*boardColumnById[\s\S]*boardColumn/, 'package order endpoint should persist board column placement');
