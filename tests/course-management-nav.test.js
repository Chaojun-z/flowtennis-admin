...
assert.match(fnBody('saveSchedule'), /fieldFeeEnabled&&isStoredValuePayMethod\(fieldFeePayMethod\)[\s\S]*场地费不能用储值卡扣款/, 'schedule field fee should reject stored-value payment methods before saving');
...
assert.match(fnBody('openScheduleModal'), /schedule-payment-row[\s\S]*支付方式[\s\S]*课时费[\s\S]*场地费/, 'schedule payment row should show pay method, lesson fee, and field fee in one compact row');
...
assert.match(pagesCss, /schedule-payment-row[\s\S]*grid-template-columns:repeat\(3,156px\)/, 'schedule payment row should use compact equal-width fields');
