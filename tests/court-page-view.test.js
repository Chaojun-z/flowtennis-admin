...
assert.match(html, /最近订场[\s\S]*累计订场[\s\S]*累计消费金额[\s\S]*会员状态[\s\S]*会员类型[\s\S]*会员余额[\s\S]*会员到期/, 'court table should use the standardized booking account columns');
...
assert.match(html, /<th class="tms-sortable" onclick="setCourtSort\('lastBookingDate'\)"[\s\S]*?>最近订场/, 'court table should support sorting by latest booking date');
assert.match(html, /<th class="tms-sortable" onclick="setCourtSort\('bookingCount'\)"[\s\S]*?>累计订场/, 'court table should support sorting by booking count');
assert.match(html, /<th class="tms-sortable" onclick="setCourtSort\('bookingAmount'\)"[\s\S]*?>累计消费金额/, 'court table should support sorting by booking amount');
assert.match(html, /<th class="tms-sortable" onclick="setCourtSort\('balance'\)"[\s\S]*?>会员余额/, 'court table should keep balance sorting');
