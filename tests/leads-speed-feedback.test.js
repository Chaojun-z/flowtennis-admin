...
function fnBody(name){
  const starts = [`function ${name}(`, `async function ${name}(`]
    .map(pattern => leadsSource.indexOf(pattern))
    .filter(index => index !== -1);
  assert.ok(starts.length, `${name} should exist`);
  const start = Math.min(...starts);
  const nextFunction = leadsSource.indexOf('\nfunction ', start + 1);
  const nextAsync = leadsSource.indexOf('\nasync function ', start + 1);
  const candidates = [nextFunction, nextAsync].filter(index => index !== -1);
  const next = candidates.length ? Math.min(...candidates) : leadsSource.length;
  return leadsSource.slice(start, next);
}
...
assert.match(html, /assets\/scripts\/pages\/leads\.js\?v=20260807-lead-speed-feedback-v1/, 'index should bump the lead script asset version');
