...
assert.match(operationsSnapshotWorkflowSource, /--write --view coach --common-scopes --shard-count 6 --shard-index \$\{\{ matrix\.shard \}\}[\s\S]*--write --view coach --daily-scopes --skip-default-scope --daily-from 2026-09-01 --daily-to 2026-12-31 --shard-count 6 --shard-index \$\{\{ matrix\.shard \}\}/, 'operations snapshot workflow must prebuild common date scopes and daily coach scopes in bounded shards');
...
assert.match(operationsSnapshotRunnerSource, /listCampusesWithDefaults\(\)[\s\S]*buildCommonScopeArgs\(\{ \.\.\.args, shardCount: 1, shardIndex: 0 \}, new Date\(\), campusRows\)[\s\S]*buildDailyScopeArgs\(args, dailyBounds \|\| \{\}, campusRows\)[\s\S]*rebuildScope\(\{ user, scope: buildScope\(scopeArgs\), reason: args\.commonScopes \? 'script-common-scope' : 'script-default' \}\)/, 'operations snapshot runner must prebuild the default, campus, common date scopes and coach daily scopes');
