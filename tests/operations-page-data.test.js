...
assert.match(operationsSourceModelSource, /OPERATIONS_PURCHASE_FIELDS[\s\S]*'paidAmount'[\s\S]*'receivedAmount'[\s\S]*'refundAmount'[\s\S]*'refundDate'[\s\S]*'refundStatus'[\s\S]*'coachPriceName'[\s\S]*'coachPriceSnapshot'/, 'operations source model should read refund fields as well as coach ownership and receipt fallback fields');
...
assert.match(operationsSnapshotRunnerSource, /dailyMonthScopes: argv\.includes\('--daily-month-scopes'\)/, 'operations snapshot runner should support prebuilding coach daily month packs for 1s arbitrary date filters');
...
assert.match(operationsSnapshotRunnerSource, /function buildDailyMonthScopeArgs\(/, 'operations snapshot runner should build coach month packs so custom date ranges read few rows');
...
assert.match(operationsSnapshotWorkflowSource, /--write --view coach --common-scopes --shard-count 6 --shard-index \$\{\{ matrix\.shard \}\}[\s\S]*--write --view coach --daily-month-scopes --skip-default-scope --daily-from 2026-08-01 --daily-to 2026-12-31 --shard-count 6 --shard-index \$\{\{ matrix\.shard \}\}/, 'operations snapshot workflow must prebuild common date scopes and coach month packs in bounded shards');
assert.match(operationsSnapshotWorkflowSource, /Process queued snapshot rebuilds[\s\S]*matrix\.shard == 0[\s\S]*--write --process-queued --limit 5 --skip-default-scope/, 'operations snapshot workflow must consume durable queued rebuilds for uncommon historical ranges');
...
assert.match(operationsSnapshotRunnerSource, /scanFirstRows:\s*\(table, options = \{\}\) => storage\.getCachedScan\([\s\S]*?scope\?\.view === 'weekly-report'/, 'offline snapshot rebuilds must read the complete source tables instead of failing at the production first-page limit');
assert.match(operationsSnapshotRunnerSource, /listCampusesWithDefaults\(\)[\s\S]*buildCommonScopeArgs\(\{ \.\.\.args, shardCount: 1, shardIndex: 0 \}, new Date\(\), campusRows\)[\s\S]*buildDailyScopeArgs\(args, dailyBounds \|\| \{\}, campusRows\)[\s\S]*buildDailyMonthScopeArgs\(args, dailyBounds \|\| \{\}, campusRows\)[\s\S]*rebuildScope\(\{ user, scope: buildScope\(scopeArgs\), reason: args\.commonScopes \? 'script-common-scope' : 'script-default' \}\)/, 'operations snapshot runner must prebuild the default, campus, common date scopes and coach month packs');
