...
assert.match(operationsPageSource, /getScheduleListRows[\s\S]*getOperationsBaseRows\(\{[\s\S]*getScheduleListRows/, 'operations page-data should pass the schedule list reader into the shared operations source');
...
assert.match(operationsSnapshotRunnerSource, /processQueuedRebuilds\(\{ limit: args\.limit \}\)/, 'operations snapshot runner must process durable queued tasks');
