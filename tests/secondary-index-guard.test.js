...
assert.match(purchaseEntitlementRoutesSource, /if\(path==='\/entitlements\/recommend'&&method==='POST'\)\{[\s\S]*const recommendationStudentIds=[\s\S]*getIndexedActiveEntitlementsForStudents\(recommendationStudentIds\)[\s\S]*getCachedScan\(T_COACHES\)\.catch\(\(\)=>\[\]\)[\s\S]*getCachedScan\(T_USERS\)\.catch\(\(\)=>\[\]\)/, '课包推荐必须优先走学员活跃课包索引');
...
assert.doesNotMatch(apiSource, /if\(!needsFallback\)return indexedRows;/, '课包推荐不能因为索引返回了部分结果就跳过主表合并，避免学员多个课包只显示一个');
