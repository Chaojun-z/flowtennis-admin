...
assert.match(body, /总订场用户[\s\S]*会员用户[\s\S]*客群次数对比[\s\S]*订场总金额[\s\S]*散客消费/, '订场顶部应显示新的 5 张卡片标题');
assert.match(body, /会员用户 \/ 总订场用户占比[\s\S]*散客次数占比 vs 会员次数占比[\s\S]*散客消费金额 \/ 订场总金额占比/, '订场顶部应显示新的说明文案');
...
assert.doesNotMatch(body, /fmt\(totalReceived\)/, '订场顶部不能把账户累计实收当成订场收入展示');
assert.match(body, /fmt\(bookingAmount\)/, '订场顶部应展示订场结构化金额');
assert.match(body, /courtStatInlinePercent\(\{part:guestBookingAmount,total:bookingAmount\}\)/, '散客消费卡片应按订场金额计算占比');
