...
const applyLessonDeltaBody = apiSource.match(/async function applyLessonDelta\(classId,delta,studentIds=\[\]\)\{([\s\S]*?)\n\}/)?.[1] || '';
assert.match(applyLessonDeltaBody, /return null;/, 'deprecated class lesson delta should stay disabled');
assert.doesNotMatch(applyLessonDeltaBody, /T_CLASSES|getCachedRow/, 'deprecated class lesson delta should not read class rows');
...
assert.match(purchaseEntitlementRoutesSource, /if\(path==='\/entitlements\/recommend'&&method==='POST'\)\{[\s\S]*const recommendationStudentIds=[\s\S]*getIndexedActiveEntitlementsForStudents\(recommendationStudentIds\)[\s\S]*getCachedScan\(T_COACHES\)\.catch\(\(\)=>\[\]\)[\s\S]*getCachedScan\(T_USERS\)\.catch\(\(\)=>\[\]\)/, 'entitlement recommend should prefer active entitlement index');
...
assert.match(scheduleSaveValidationSource, /async function validateScheduleSave\(nextRec\)\{[\s\S]*timed\('load schedule conflict candidates'[\s\S]*scheduleConflictIndexReady\(\)[\s\S]*loadScheduleConflictIndexRows\(nextRec\)[\s\S]*getCachedScan\(T_SCHEDULE\)[\s\S]*timed\('scan courts for schedule conflict check',\(\)=>withTimeout\(getCachedScan\(T_COURTS\)\.catch\(\(\)=>\[\]\),2500,\[\]\)\)/, 'schedule save validation should prefer ready conflict indexes and fall back to cached schedule scan before backfill');
...
assert.match(apiSource, /const storedAuthUser=.*getCachedRow\(T_USERS,user\.id\)\.catch\(\(\)=>null\);/, 'authenticated requests should reuse cached user row lookup');
...
assert.match(packageRoutesSource, /if\(path==='\/packages'\)\{[\s\S]*if\(method==='GET'\)\{[\s\S]*const rows=await getCachedScan\(T_PACKAGES\)\.catch\(\(\)=>\[\]\);[\s\S]*return sendJson\(res,filterLoadAllForUser\(\{packages:rows\},user\)\.packages\);/, 'packages list should use cached scan');
...
assert.doesNotMatch(corePageDataSource, /\/page-data\/plans/, 'deprecated plans page aggregate endpoint should stay removed');
...
assert.match(residualPageDataSource, /require\('\.\/finance-page\.js'\)/, 'finance page aggregate endpoint should live outside api/index.js');
...
assert.match(corePageDataSource, /if\(path==='\/page-data\/memberships'&&method==='GET'\)\{[\s\S]*Promise\.all\(\[[\s\S]*listCampusesWithDefaults\(\)[\s\S]*cappedScan\(T_STUDENTS\)[\s\S]*getCachedScan\(T_COURTS\)[\s\S]*cappedScan\(T_MEMBERSHIP_ACCOUNTS\)[\s\S]*cappedScan\(T_MEMBERSHIP_ORDERS\)[\s\S]*cappedScan\(T_MEMBERSHIP_BENEFIT_LEDGER\)[\s\S]*cappedScan\(T_MEMBERSHIP_ACCOUNT_EVENTS\)[\s\S]*cappedScan\(T_MEMBERSHIP_PLANS\)[\s\S]*cappedScan\(T_COACHES\)[\s\S]*\]\)/, 'memberships page aggregate endpoint should use the full cached court account set before matching membership accounts');
...
assert.match(residualPageDataSource, /const forceFresh=query\?\.get\('fresh'\)==='1'\|\|query\?\.get\('forceFresh'\)==='1';[\s\S]*const params=\{[\s\S]*sampleIds:ids,[\s\S]*sample,[\s\S]*forceFresh,[\s\S]*includeDetails:ids\.length>0,[\s\S]*page:query\?\.get\('page'\)\|\|''[\s\S]*if\(!view\)view=await loadCourtAccountListView\(params\)/, 'court account read model should support fresh reads, paging, and facts fallback after membership benefit mutations');
