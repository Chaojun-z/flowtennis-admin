...
assert.match(residualPageDataSource, /const forceFresh=query\?\.get\('fresh'\)==='1'\|\|query\?\.get\('forceFresh'\)==='1';[\s\S]*loadCourtAccountListView\(\{[\s\S]*sampleIds:ids,[\s\S]*sample,[\s\S]*forceFresh,[\s\S]*includeDetails:ids\.length>0,[\s\S]*page:query\?\.get\('page'\)\|\|''/, 'court account read model should support fresh reads and paging after membership benefit mutations');
