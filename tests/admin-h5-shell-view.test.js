...
assert.match(appSource, /function renderCourtAccountListView\([\s\S]*const isMobileList=document\.body\.classList\.contains\('admin-mobile'\)[\s\S]*const slice=sortedList;/, 'court H5 page should render the current server-paged list instead of a desktop page slice');
assert.match(appSource, /function renderMemberships\([\s\S]*const isMobileList=document\.body\.classList\.contains\('admin-mobile'\)[\s\S]*const slice=sortedRows;/, 'membership H5 page should render the current server-paged list instead of a desktop page slice');
