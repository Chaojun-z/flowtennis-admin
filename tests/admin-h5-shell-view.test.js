...
assert.match(appSource, /function renderCourtAccountListView\([\s\S]*const serverPaging=courtAccountListViewData\?\.pagination\|\|null[\s\S]*slice=serverPaging\?sortedList:\(isMobileList\?sortedList/, 'court H5 page should render server-side page rows without a desktop slice');
assert.match(appSource, /function renderMemberships\([\s\S]*const serverPaging=courtAccountListViewData\?\.pagination\|\|null[\s\S]*slice=serverPaging\?sortedRows:\(isMobileList\?sortedRows/, 'membership H5 page should render server-side page rows without a desktop slice');
