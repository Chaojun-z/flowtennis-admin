...
assert.strictEqual(taxonomy.normalizeLeadDemandProduct('青少年小班课（训练营）'), '小班课');
...
assert.match(dictionaryDoc, /`dealType`[\s\S]*成交类型[\s\S]*课程\+订场\+订场会员/, 'data dictionary should define standard deal type');
assert.match(dictionaryDoc, /`customerType`[\s\S]*类型[\s\S]*成人[\s\S]*青少年/, 'data dictionary should define standard customer type');
assert.match(dictionaryDoc, /`demandProduct`[\s\S]*需求产品[\s\S]*私教课[\s\S]*会员[\s\S]*其他/, 'data dictionary should define standard demand product');
