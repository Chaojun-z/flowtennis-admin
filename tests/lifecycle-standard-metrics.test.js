...
assert.deepStrictEqual(
  ...
  '在期学员必须按课包有余额或近90天已完成正式课活跃返回，不能把单纯已排课当成已上课'
);
assert.strictEqual(
...
assert.strictEqual(
  ...
  5,
  '历史学员上过正式课必须只按已完成正式课事实统计，不能把单纯已排课算成上课'
);
assert.strictEqual(
...
const siblingScheduleOwnerData = {
  ...
  purchases: [{
    id: 'purchase-william-owner',
    studentId: 'student-william',
    packageName: '家庭共享私教课',
    courseType: '私教课',
    packageLessons: 10,
    amountPaid: 4500,
  ...
  entitlements: [{
    id: 'ent-william-owner',
    studentId: 'student-william',
    purchaseId: 'purchase-william-owner',
    packageName: '家庭共享私教课',
    courseType: '私教课',
    totalLessons: 10,
    remainingLessons: 9,
  ...
  entitlementLedger: [{
    id: 'ledger-brother-uses-william-package',
    entitlementId: 'ent-william-owner',
    purchaseId: 'purchase-william-owner',
    studentId: '',
    scheduleId: 'schedule-brother-2',
    lessonDelta: -1,
...
assert.deepStrictEqual(
  ...
  [
    ['2026-08-01 16:00-17:00', '林铭教练', '4号场', '使用 William（时节） 的课包'],
    ['2026-07-19 16:00-17:00', '刘润扬教练', '2号场', undefined]
  ],
  '学员抽屉上课记录必须和该学员自己的排课事实一致，不能混入同名前缀或家庭共享课包的其他学员课程'
);
assert.strictEqual(
  siblingOwnerRow?.completedLessons,
  0,
  '课包被授权他人使用时，课包所有人的累计上课不能增加'
);
assert.deepStrictEqual(
  siblingOwnerRow?.detailLessonRecordRows.map(row => [row.time, row.lessonRelationText, row.isPackageOwnerLedger, row.countAsCompletedLesson]),
...
assert.strictEqual(
  hardStandard.views.historicalStudents.find(row => row.studentId === 'hard-blank-formal')?.paymentModeLabel,
  '-',
  '没有课包且只有未完成排课的记录，不能兜底标成单次付费学员'
);
assert.strictEqual(
