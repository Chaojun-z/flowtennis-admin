...
assert.match(scheduleWxml, /week-task-package[\s\S]*item\.packageText/, 'native workbench weekly todo card should render package progress');
assert.match(scheduleWxml, /week-task-location[\s\S]*item\.shortLocation[\s\S]*week-task-separator[\s\S]*item\.student/, 'native workbench weekly todo card should render location before student');
...
assert.match(scheduleJs, /closeFeedback\(\)[\s\S]*restoreTimetableScroll\(\)/, 'closing feedback should restore the timetable position');
...
assert.match(scheduleWxss, /\.week-task-card\s*\{[\s\S]*width:\s*361px;[\s\S]*height:\s*148px;/, 'dashboard week todo card should leave room for package progress');
...
assert.match(scheduleWxss, /\.week-task-package\s*\{[\s\S]*font-size:\s*12px;[\s\S]*transform:\s*translateY\(-7px\);/, 'dashboard week todo package progress should fit as a compact extra row');
assert.match(scheduleWxss, /\.week-task-divider\s*\{[\s\S]*width:\s*325px;[\s\S]*height:\s*1px;[\s\S]*margin-top:\s*6px;[\s\S]*transform:\s*translateY\(-5px\);/, 'dashboard week todo divider should match the new package row spacing');
...
assert.match(scheduleWxss, /\.week-task-actions\s*\{[\s\S]*transform:\s*translateY\(-6px\);/, 'dashboard week todo actions should sit below the new package row');
