...
assert.match(scheduleWxml, /student-row[\s\S]*student-name[\s\S]*student-course-label/, 'student list first row should show name and course label');
assert.doesNotMatch(scheduleWxml, /student-avatar/, 'student list should not render avatars');
assert.doesNotMatch(scheduleWxml, /累计上课（节\/课时）/, 'student list should no longer render cumulative lesson text');
assert.doesNotMatch(scheduleWxml, /student-meta-separator">·<\/text>/, 'student list should not separate cumulative and package text after cumulative is removed');
assert.match(scheduleWxml, /student-progress[\s\S]*student-progress-fill[\s\S]*item\.packagePercent[\s\S]*item\.packageText/, 'student list should render package balance with progress');
assert.match(scheduleWxml, /最近上课：\{\{item\.lastClassText\}\}/, 'student list should render latest lesson as a dedicated row');
assert.match(scheduleWxml, /studentStats\.totalCount[\s\S]*当前学员[\s\S]*studentStats\.weekActiveCount[\s\S]*本周上课[\s\S]*studentStats\.monthActiveCount[\s\S]*本月上课/, 'student summary should use total, weekly active and monthly active counts');
assert.match(scheduleWxml, /studentTabs[\s\S]*onStudentTabTap/, 'student page should render filter tabs with counts');
assert.match(scheduleWxml, /studentSearchKeyword[\s\S]*bindinput="onStudentSearchInput"[\s\S]*搜索学员/, 'student page should support student search');
...
assert.match(scheduleWxml, /今日课程[\s\S]*本周课时[\s\S]*本月课时/, 'native workbench should keep the three primary metrics in the requested order');
assert.doesNotMatch(scheduleWxml, /本月反馈[\s\S]*体验课转化/, 'native workbench should remove the lower three metrics from the top card');
assert.match(scheduleWxml, /dashboard-hero/, 'native workbench should render the recreated SVG hero header');
assert.match(scheduleWxml, /dashboard-grid/, 'native workbench should render the three-column metric grid');
...
assert.doesNotMatch(scheduleWxml, /week-task-package/, 'native workbench weekly todo card should not render package progress as a separate row');
assert.match(scheduleWxml, /week-task-location[\s\S]*item\.shortLocation[\s\S]*week-task-separator[\s\S]*item\.student[\s\S]*week-task-separator[\s\S]*课包 \{\{item\.packageText\}\}/, 'native workbench weekly todo card should render location, student and package progress on one row');
...
assert.match(scheduleWxml, /bindscroll="onTimetableScroll"/, 'native timetable should record the current manual scroll position');
...
const openDetailBody = (scheduleJs.match(/openDetail\(event\)\s*\{[\s\S]*?\n  \},\n\n  openStudentDetail/) || [''])[0];
const closeFeedbackBody = (scheduleJs.match(/closeFeedback\(\)\s*\{[\s\S]*?\n  \},\n\n  openProposal/) || [''])[0];
const buildStudentCardsBody = (scheduleJs.match(/function buildStudentCards\(students = \[\]\)[\s\S]*?\n\}/) || [''])[0];
...
assert.doesNotMatch(scheduleJs, /function buildStudentStats\(students = \[\], schedule = \[\]\)/, 'mini program must not calculate student summary stats from local schedules');
assert.doesNotMatch(scheduleJs, /function studentRosterTabKey/, 'mini program must not privately classify active, trial, or ended students');
assert.doesNotMatch(scheduleJs, /function validStudentScheduleIdsForWindow/, 'mini program must not privately count weekly or monthly attended students');
assert.doesNotMatch(scheduleJs, /function studentPackageProgress/, 'mini program must not parse package progress locally');
assert.doesNotMatch(scheduleJs, /function studentCourseLabel/, 'mini program must not guess course labels locally');
assert.match(scheduleJs, /function buildStudentTabs[\s\S]*label: '全部'[\s\S]*label: '在课'[\s\S]*label: '体验'[\s\S]*label: '已结课'[\s\S]*label: '代课'/, 'student tabs should use backend counts including the substitute tab');
assert.match(scheduleJs, /data\.studentRoster[\s\S]*studentRosterItems/, 'mini program student page should read the backend unified student roster projection');
assert.match(scheduleJs, /function filterStudentCards\(students = \[\], tab = 'all', keyword = ''\)[\s\S]*studentMatchesTab[\s\S]*studentMatchesSearch/, 'student tabs and search should filter the prepared student cards');
assert.match(scheduleJs, /onStudentTabTap\(event\)[\s\S]*studentsFilteredList:\s*filterStudentCards/, 'student tab tap should refresh the visible student list');
assert.match(scheduleJs, /onStudentSearchInput\(event\)[\s\S]*studentsFilteredList:\s*filterStudentCards/, 'student search input should refresh the visible student list');
...
assert.doesNotMatch(scheduleJs, /function mergeTeachingStudentViews/, 'mini program students should not merge web views locally after backend roster projection is available');
assert.match(scheduleJs, /openDetail\(event\)[\s\S]*preserveTimetableScroll\(\)/, 'opening a timetable detail should preserve the current timetable scroll position before feedback entry');
assert.match(scheduleJs, /openFeedbackById\(event\)[\s\S]*preserveTimetableScroll\(\)/, 'opening feedback from a timetable course should preserve the current timetable scroll position');
assert.match(scheduleJs, /openDetail\(event\)[\s\S]*detailReturnTarget:\s*this\.data\.showStudentDetail \? 'student' : ''[\s\S]*showDetail:\s*true/, 'opening schedule detail from student detail should remember the previous student sheet');
assert.doesNotMatch(openDetailBody, /showStudentDetail:\s*false/, 'opening schedule detail should not close the student detail sheet underneath');
assert.match(scheduleJs, /closeDetail\(\)[\s\S]*showDetail:\s*false[\s\S]*detailReturnTarget:\s*''/, 'closing schedule detail should only close the schedule detail layer');
assert.match(closeFeedbackBody, /feedbackReturnTarget === 'detail'[\s\S]*showDetail:\s*returnToDetail \? true : false[\s\S]*restoreTimetableScroll\(\)/, 'closing feedback should return to the previous detail layer and restore timetable position');
assert.match(scheduleWxml, /feedback-sheet \{\{feedbackSheetClass\}\}[\s\S]*课后反馈[\s\S]*bindtap="closeFeedback"[\s\S]*bindtap="closeFeedback"/, 'feedback sheet close and cancel buttons should restore the timetable position');
...
assert.match(scheduleJs, /function studentDetailLessonRecordsFromUnifiedRows[\s\S]*Array\.isArray\(row\.metaParts\)/, 'student detail should preserve backend normalized lesson meta parts including venue, coach and course type');
assert.match(scheduleJs, /function studentDetailLessonRecordsFromUnifiedRows[\s\S]*\/待\|进行\/\.test\(status\)[\s\S]*feedbackStatusText/, 'student detail should not show pending-feedback tags for future pending lessons');
...
assert.match(scheduleJs, /function studentDetailLessonRecordsFromUnifiedRows[\s\S]*backendMetaParts\.length \? backendMetaParts : \[[\s\S]*courseType[\s\S]*\]\.filter\(Boolean\)/, 'student detail lesson record meta should prefer backend meta and otherwise include course type after venue and coach');
assert.match(scheduleJs, /feedbackStatusText:\s*hasFeedback \|\| pending \? '' : '未反馈'/, 'student detail lesson records should mark missing feedback only after lessons end');
...
assert.match(scheduleJs, /const requestParams = coachStatsRequestParamsFromState\(this\.data\);[\s\S]*loadCoachStats\(requestParams\)/, 'stats tab should load backend coach stats instead of local class data');
assert.match(scheduleJs, /function coachStatsRequestParamsFromState[\s\S]*startDate: view === 'all' \? startDate : ''[\s\S]*endDate: view === 'all' \? endDate : ''/, 'all-time stats should send the custom date range to the backend');
...
assert.match(scheduleJs, /onCoachStatsViewTap\(event\)[\s\S]*const cacheKey = coachStatsCacheKey\(requestParams\);[\s\S]*this\._coachStatsActiveRequestKey = cacheKey;[\s\S]*const cached/, 'stats tab should invalidate older requests before rendering a newly selected cached tab');
assert.match(scheduleJs, /prevCoachStatsRange\(\)[\s\S]*this\._coachStatsActiveRequestKey = coachStatsCacheKey\(coachStatsRequestParamsFromState\(this\.data,\s*\{ offset: coachStatsOffset \}\)\)/, 'stats previous range should invalidate older async responses before setData finishes');
assert.match(scheduleJs, /nextCoachStatsRange\(\)[\s\S]*this\._coachStatsActiveRequestKey = coachStatsCacheKey\(coachStatsRequestParamsFromState\(this\.data,\s*\{ offset: coachStatsOffset \}\)\)/, 'stats next range should invalidate older async responses before setData finishes');
...
assert.match(scheduleJs, /function trendLabel[\s\S]*view === 'all'[\s\S]*`\$\{month\}月`/, 'all-time trend labels should explicitly include month text');
...
assert.match(scheduleJs, /function coachStatsAllTrendBucketMode[\s\S]*days <= 31[\s\S]*return 'day'[\s\S]*days <= 730[\s\S]*return 'month'[\s\S]*return 'year'/, 'all-time trend should switch between date, month and year buckets by actual range');
assert.match(scheduleJs, /this\._coachStatsActiveRequestKey = cacheKey[\s\S]*if \(this\._coachStatsActiveRequestKey !== cacheKey\) return;[\s\S]*this\.setData\(\{[\s\S]*\.\.\.adapted/, 'stats tab should ignore stale async responses after tab or range changes');
assert.match(scheduleJs, /const preview = buildLocalCoachStatsData\(\{[\s\S]*view: requestParams\.view[\s\S]*offset: requestParams\.offset[\s\S]*if \(this\._coachStatsActiveRequestKey !== cacheKey\) return;[\s\S]*this\.setData\(\{[\s\S]*\.\.\.preview/, 'stats local preview should not overwrite a newer selected tab or range');
assert.match(scheduleJs, /if \(requestParams\.view === 'all'\)/, 'all-time cache aliases should use the original request view instead of the current tab after async returns');
...
assert.match(scheduleJs, /lastScheduleId:\s*firstNonEmpty\(student\.lastScheduleId\)/, 'mini program students should read their latest class id from backend roster projection');
...
assert.match(apiServerJs, /pages\/schedule\/schedule\?tab=timetable/, 'course reminders should deep link directly to the native timetable tab');
assert.match(apiServerJs, /&scheduleId=\$\{id\}/, 'course reminder timetable deep link should keep the schedule id');
assert.match(apiServerJs, /pagepath:'pages\/schedule\/schedule\?tab=timetable'/, 'daily digest should deep link directly to the native timetable tab');
...
assert.match(scheduleWxss, /\.mini-metrics\s*\{[\s\S]*height:\s*98px;[\s\S]*padding:\s*16px;[\s\S]*gap:\s*6px;/, 'dashboard metric shell should shrink to one row after removing secondary metrics');
...
assert.doesNotMatch(scheduleWxss, /conversion-row|conversion-value|conversion-unit/, 'dashboard should remove conversion-specific metric styles with the removed metric');
...
assert.match(scheduleWxss, /\.week-task-card\s*\{[\s\S]*width:\s*361px;[\s\S]*height:\s*130px;/, 'dashboard week todo card should return to the compact single-line package layout');
...
assert.doesNotMatch(scheduleWxss, /\.week-task-package\s*\{/, 'dashboard week todo package progress should not need a separate row style');
assert.match(scheduleWxss, /\.week-task-divider\s*\{[\s\S]*width:\s*325px;[\s\S]*height:\s*1px;[\s\S]*margin-top:\s*8px;[\s\S]*transform:\s*translateY\(-6px\);/, 'dashboard week todo divider should use the original compact spacing');
...
assert.match(scheduleWxss, /\.week-task-actions\s*\{[\s\S]*transform:\s*translateY\(-8px\);/, 'dashboard week todo actions should use the compact card vertical offset');
...
assert.match(scheduleWxss, /\.stats-summary-card\s*\{[\s\S]*height:\s*168px;/i, 'stats summary card should keep enough bottom spacing for the total and highlights');
...
assert.match(scheduleJs, /function shortDateText[\s\S]*`\$\{month\}\.\$\{String\(day\)\.padStart\(2,\s*'0'\)\}`/, 'date trend labels should use one compact M.DD format across week, month and all-day views');
assert.match(scheduleJs, /if \(\/\^\\d\{4\}-\\d\{2\}-\\d\{2\}\$\/\.test\(raw\)\) \{\s*return shortDateText\(raw\);\s*\}/, 'all-time day buckets should reuse the same date label format as week and month tabs');
...
assert.match(scheduleJs, /const isScrollable = safeView === 'all' \|\| safeView === 'month';[\s\S]*coachStatsTrendScrollable: isScrollable/, 'all-time and month trends should use controlled scrolling when the x-axis is dense');
assert.match(scheduleJs, /safeView === 'all'[\s\S]*count \* 62[\s\S]*safeView === 'month'[\s\S]*count \* 28/, 'all-time and month trends should reserve enough width for explicit bottom labels');
assert.match(scheduleJs, /function trendValueText[\s\S]*if \(!n\) return '';[\s\S]*formatCoachStatsNumber\(n\)/, 'all stats trends should show non-zero lesson values above bars');
assert.match(scheduleWxml, /stats-trend-track[\s\S]*stats-trend-axis[\s\S]*stats-trend-value[\s\S]*item\.lessonUnitsText[\s\S]*item\.label/, 'stats trend should place lesson values above time labels in a fixed bottom axis area');
assert.match(scheduleWxss, /\.stats-trend-scroll\s*\{[\s\S]*height:\s*186px;/, 'stats trend scroll view should reserve enough fixed height for the bottom time and value rows');
assert.match(scheduleWxss, /\.stats-trend-track\s*\{[\s\S]*flex:\s*0 0 112px;[\s\S]*height:\s*112px;/, 'stats trend bars should use a fixed-height track so values do not change column length');
assert.match(scheduleWxss, /\.stats-trend-axis\s*\{[\s\S]*height:\s*44px;/, 'stats trend bottom axis should fit both the time label and lesson value');
assert.match(scheduleWxss, /\.stats-trend-month\s+\.stats-trend-item\s*\{[\s\S]*width:\s*24px;[\s\S]*flex:\s*0 0 24px;/, 'month trend items should be wide enough so consecutive values do not run together');
assert.doesNotMatch(scheduleWxss, /\.stats-trend-all\s+\.stats-trend-label\s*\{[\s\S]*font-size:/, 'all-time trend labels should use the same bottom font size as the other tabs');
...
assert.match(cssBlock(scheduleWxss, '.stats-trend-all .stats-trend-value'), /display:\s*block;/, 'all-time trend should show lesson values above time labels');
assert.doesNotMatch(cssBlock(scheduleWxss, '.stats-trend-month .stats-trend-value'), /display:\s*none;/, 'month trend should use the same value-above-bar structure as other tabs');
