...
assert.match(scheduleWxml, /wx:if="\{\{item\.showPackage\}\}" class="student-meta">当前课包余额：<text class="student-strong">\{\{item\.packageText\}\}<\/text>/, 'student list should render only the current package balance');
...
assert.match(scheduleJs, /function buildStudentStats\(students = \[\], coachName = '', entitlements = \[\]\)[\s\S]*studentOwnedByCoach\(item, entitlements, coachName\)[\s\S]*return \{ ownerCount, substituteCount \}/, 'student stats should use the same package ownership rule as student cards');
