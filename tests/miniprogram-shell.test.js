...
assert.match(scheduleJs, /function buildStudentStats\(students = \[\], schedule = \[\]\)[\s\S]*weekActiveCount[\s\S]*monthActiveCount[\s\S]*return \{ totalCount, weekActiveCount, monthActiveCount, activeCount, trialCount, endedCount \}/, 'student stats should use total, weekly active, monthly active and tab counts');
assert.match(scheduleJs, /function buildStudentTabs[\s\S]*label: '全部'[\s\S]*label: '在课'[\s\S]*label: '体验'[\s\S]*label: '已结课'/, 'student tabs should use the requested four labels');
...
assert.match(scheduleJs, /function mergeTeachingStudentViews[\s\S]*historicalStudents[\s\S]*completedLessons[\s\S]*packageBalanceText[\s\S]*detailRecentLessonDate/, 'mini program students should merge the same historical student fields used by the web list');
...
assert.match(scheduleJs, /function studentOwnedByCoach[\s\S]*entitlements[\s\S]*ownerCoach[\s\S]*primaryCoach/, 'student ownership should include package ownership, not only the student profile coach');
...
assert.match(scheduleJs, /buildStudentCards\(studentsRaw, data\.entitlements \|\| \[\], studentSchedule, coachName, data\.entitlementLedger \|\| \[\]\)/, 'student cards should consume merged students, entitlements and ledger-backed student schedules');
