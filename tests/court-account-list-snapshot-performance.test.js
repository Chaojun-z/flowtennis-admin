...
const { meta, bundle } = buildSnapshotRows(rows, { versionId: 'perf-snapshot' });
...
(async () => {
  ...
  for (const [label, options] of scenarios) {
    ...
    for (let i = 0; i < 3; i += 1) {
      ...
      times.push(elapsed);
      assert.ok(elapsed < 1000, `${label} 5000 条快照包筛选排序统计分页应在 1s 内完成，实际 ${elapsed}ms`);
      ...
      assert.strictEqual(view.meta.source, 'court-account-list-snapshot', `${label} 应走快照包`);
    }
    report.push(`${label}: ${times.join('ms / ')}ms`);
  }
  assert.strictEqual(scanCalled, false, '快照读取链路不得调用 scan');
  console.log(`court account list snapshot performance tests passed\n${report.join('\n')}`);
})().catch((err) => {
  console.error(err);
  process.exit(1);
});
