...
assert.match(source, /leads:\['campuses','leads'\]/, 'leads page should only block on the datasets needed for first paint');
assert.doesNotMatch(source, /leads:\['campuses','leads','customerCenterPage'\]/, 'leads page must not block first paint on the customer center read model');
...
assert.match(source, /schedule:\['campuses','schedule'\]/, 'schedule page should load only campuses and its lightweight schedule snapshot before first paint');
...
assert.match(source, /const PAGE_DATA_BACKGROUND_REQUIREMENTS=\{[\s\S]*students:\['classes','schedule','courts'\][\s\S]*'package-students':\['classes','schedule','courts'\][\s\S]*'trial-students':\['classes','schedule','courts'\][\s\S]*leads:\[\][\s\S]*packages:\[\][\s\S]*purchases:\[\][\s\S]*schedule:\['students','courts','coaches','coachProposals','classes','feedbacks','entitlements','entitlementLedger','lifecycleMetricsPage','financePage','courtAccountListViewPage'\][\s\S]*coachschedule:\['entitlements','entitlementLedger'\][\s\S]*finance:\['financePage'\][\s\S]*courts:\['courtsPage'\][\s\S]*memberships:\[\][\s\S]*workbench:\['workbenchPage'\][\s\S]*postfeedback:\['workbenchPage'\][\s\S]*mystudents:\['workbenchPage'\][\s\S]*myclasses:\['students','classes'\]/, 'schedule background data should include the unified membership read model for stored-value balance lookup');
assert.match(fnBodyFrom(source, 'courtAccountListViewQueryParams'), /currentPage==='schedule'[\s\S]*accountType:'会员账户'[\s\S]*pageSize:5000/, 'schedule stored-value payment should load the membership-account read model, not the raw court-account list defaults');
...
assert.match(source, /function teachingSummaryNotReadyErrorFromPayload\(data\)/, '200 not-ready teaching summary payloads should be converted into controlled frontend errors');
...
assert.match(source, /studentTeachingSummaryUnavailable[\s\S]*STUDENT_TEACHING_SUMMARY_NOT_READY/, 'frontend must not treat summary-unavailable 200 payloads as empty list success');
...
assert.match(source, /if\(name==='leads'\)\{[\s\S]*studentTeachingSummaryUnavailable[\s\S]*repairTeachingSummaryAndReload\('leads'\)/, 'leads page should repair stale teaching summary list bundles instead of silently keeping zero trial stats');
...
assert.match(corePagesSource, /createStudentRosterIndexReader/, 'customer center first-screen route should receive the isolated student roster index reader');
...
assert.match(studentRosterReaderSource, /readReadyStudentTeachingSummaryListRows\(\{[\s\S]*tableName,[\s\S]*getCachedRow,[\s\S]*getCachedScan,[\s\S]*scanByIdPrefix,[\s\S]*put,[\s\S]*verifyChecksum: true[\s\S]*\}\)/, 'student roster index reader must read the ready list bundle and use summary-table light-field fallback when the bundle has not been published yet');
assert.doesNotMatch(studentRosterReaderSource, /readReadyStudentTeachingSummaryRows\(/, 'student roster index reader must not scan active-version summary rows for the first screen');
...
assert.match(studentRosterReaderSource, /function buildCustomerCenterPagePayload\([\s\S]*const listPage = buildCustomerCenterListPage\(teachingStudentViews, query\)/, 'customer center list endpoint should expose server-side paged student views through the shared payload builder');
assert.match(studentRosterReaderSource, /function buildCustomerCenterListPage\(teachingStudentViews = \{\}, query\)[\s\S]*const filteredRows = studentRows\.filter[\s\S]*textSearchHit\(q, row\.name, row\.displayName, row\.studentName, row\.wechatName, row\.nickName, row\.nickname, row\.phone\)[\s\S]*buildListPage\(sortSummaryRowsForQuery\(filteredRows, query\), paging\)[\s\S]*facets: buildStudentListFacets\(filteredRows\)[\s\S]*rows: page\.rows\.map\(projectCustomerCenterStudentRow\)/, 'customer center cached views should apply server-side search and sorting before pagination, and expose full-set filter counts');
assert.doesNotMatch(studentRosterReaderSource, /searchableRows = q && Array\.isArray\(teachingStudentViews\.searchableStudents\)/, 'customer center paged search must stay inside the current student view');
assert.match(studentRosterReaderSource, /const filteredRows = studentRows\.filter\(row => textSearchHit\(q, row\.name, row\.displayName, row\.studentName, row\.wechatName, row\.nickName, row\.nickname, row\.phone\)\)/, 'customer center paged search should only match student identity fields');
assert.doesNotMatch(studentRosterReaderSource, /textSearchHit\(q,[^)]*row\.(searchText|primaryCoach|owner|notes|profileNote|source|campus|activityStatusLabel|studentStatusLabel)/, 'customer center paged search must not match owner, coach, notes, campus, status, or cached broad search text');
...
assert.match(corePagesSource, /path==='\/page-data\/customer-center-list\/rebuild-summary'&&method==='POST'[\s\S]*buildStudentTeachingSummaryBundleRow\(rows,batchId\)[\s\S]*await put\(T_STUDENT_TEACHING_SUMMARY,bundle\.id,bundle\)[\s\S]*buildStudentTeachingSummaryListBundleRow\(rows,batchId\)[\s\S]*await put\(T_STUDENT_TEACHING_SUMMARY,listBundle\.id,listBundle\)/, 'student teaching summary manual rebuild must publish the full bundle and the point-read list bundle used by first-screen pages');
...
assert.match(publishSummaryBundleRouteSource, /buildStudentTeachingSummaryListBundleRow\(publishedRows,activeVersion\)[\s\S]*await put\(T_STUDENT_TEACHING_SUMMARY,listBundle\.id,listBundle\)/, 'student teaching summary bundle repair must also write the point-read list bundle');
