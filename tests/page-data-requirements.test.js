...
assert.match(source, /'package-students':\['campuses','coaches','customerCenterPage'\]/, 'formal student page should use coaches and the customer center list read model before first render');
assert.match(source, /'trial-students':\['campuses','coaches','customerCenterPage'\]/, 'normal student page should use coaches and the customer center list read model before first render');
...
assert.match(source, /schedule:\['campuses','students','courts','schedule','coaches','coachProposals','lifecycleMetricsPage'\]/, 'schedule page should load its first-screen datasets without blocking on feedback, entitlement, or ledger datasets');
assert.match(source, /coachschedule:\['workbenchPage'\]/, 'coach schedule calendar should block on the backend unified coach schedule view');
...
assert.match(source, /,packageCenterPage:\(\{fresh=false\}=\{\}\)=>apiCall\('GET',packageCenterPageDataUrl\(\{fresh,page:/, 'package center first-screen pages should use the paged lightweight list endpoint');
...
assert.match(source, /function customerCenterPageDataUrl\(\{fresh=false,view='',page=1,pageSize=15\}=\{\}\)\{[\s\S]*fresh\?appendPageDataQuery\(next,\{fresh:1,_ts:Date\.now\(\)\}\):next;/, 'customer center list endpoint url should support forced fresh paged reads after schedule and package mutations');
...
assert.match(source, /,courtAccountListViewPage:\(\{fresh=false,page=1,pageSize=15,accountType=''\}=\{\}\)=>apiCall\('GET',courtAccountListViewPageDataUrl\(\{fresh,page,pageSize,accountType\}\)\)/, 'membership pages should use the paged scoped unified court account read model endpoint');
...
assert.match(source, /leads:\(\{fresh=false\}=\{\}\)=>apiCall\('GET',leadListPageDataUrl\(\{fresh,page:leadPage,pageSize:leadPageSize\}\)\)/, 'leads page should load its primary list dataset from paged /leads');
...
assert.match(source, /if\(name==='packageCenterPage'\)\{[\s\S]*if\(data\.listPage\)mergePackageCenterListPage\(data\);[\s\S]*if\(Array\.isArray\(data\.purchases\)\)setDatasetValue\('purchases',data\.purchases\);[\s\S]*if\(Array\.isArray\(data\.packages\)\)setDatasetValue\('packages',data\.packages\);[\s\S]*if\(Array\.isArray\(data\.entitlements\)\)setDatasetValue\('entitlements',data\.entitlements\);/, 'package center list loader should hydrate paged package datasets without clearing omitted arrays');
...
assert.match(source, /if\(name==='customerCenterPage'\)\{[\s\S]*if\(data\.listPage\)mergeCustomerCenterListPage\(data\);[\s\S]*if\(Array\.isArray\(data\.customerLifecycleRows\)\)setDatasetValue\('customerLifecycleRows',data\.customerLifecycleRows,\{persist:false\}\);/, 'customer center list loader should hydrate paged lifecycle rows and teaching views without clearing omitted arrays');
...
assert.match(source, /function markLearningDataStale\(\)\{[\s\S]*'leads','customerCenterPage','lifecycleMetricsPage','packageCenterPage','purchasesPage','workbenchPage'[\s\S]*financeOverviewData=null;[\s\S]*financePrepaidView=\{rows:\[\],summary:\{\}\};/, 'schedule and package mutations should invalidate list, lifecycle, workbench, and finance read-model caches together');
assert.match(source, /const DATASETS_WITH_REQUEST_KEYS=new Set\(\['leads','operationsPage','packageCenterPage','customerCenterPage','lifecycleMetricsPage','financePage','courtAccountListViewPage'\]\);/, 'scoped page-data datasets should be keyed by their current request url');
...
assert.match(corePagesSource, /path==='\/page-data\/customer-center-list'&&method==='GET'[\s\S]*const listPage=paging&&view\?\{view,\.\.\.buildListPage\(filteredStudentRows,paging\)\}/, 'customer center list endpoint should expose server-side paged student views');
