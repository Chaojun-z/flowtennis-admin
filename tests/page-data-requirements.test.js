...
assert.match(studentRosterReaderSource, /readReadyStudentTeachingSummaryListRows\(\{[\s\S]*tableName,[\s\S]*getCachedRow,[\s\S]*verifyChecksum: true[\s\S]*\}\)/, 'student roster index reader must point-read the ready active-version list bundle from the configured roster index table');
