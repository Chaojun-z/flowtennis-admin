...
assert.match(studentRosterReaderSource, /readReadyStudentTeachingSummaryListRows\(\{[\s\S]*tableName,[\s\S]*getCachedRow,[\s\S]*getCachedScan,[\s\S]*scanByIdPrefix,[\s\S]*verifyChecksum: true[\s\S]*\}\)/, 'student roster index reader must read the ready list bundle and use summary-table light-field fallback when the bundle has not been published yet');
