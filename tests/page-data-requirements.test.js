...
assert.match(studentRosterReaderSource, /function buildCustomerCenterListPage\(teachingStudentViews = \{\}, query\)[\s\S]*buildListPage\(studentRows\.filter[\s\S]*rows: page\.rows\.map\(projectCustomerCenterStudentRow\)/, 'customer center cached views should still apply server-side search before pagination');
...
assert.match(studentRosterReaderSource, /buildListPage\(studentRows\.filter\(row => textSearchHit\(q, row\.name, row\.displayName, row\.studentName, row\.wechatName, row\.nickName, row\.nickname, row\.phone\)\), paging\)/, 'customer center paged search should only match student identity fields');
