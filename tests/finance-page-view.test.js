...
assert.match(source,/交易日期[\s\S]*姓名[\s\S]*校区[\s\S]*交易类型[\s\S]*业务类型[\s\S]*支付方式[\s\S]*交易金额[\s\S]*备注[\s\S]*financeLedgerTbody/,'ledger table should use the new transaction columns');
assert.match(source,/全部交易类型/,'ledger should expose new transaction type filter');
