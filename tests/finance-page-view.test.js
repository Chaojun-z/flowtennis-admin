...
assert.match(source,/总实收[\s\S]*总核销确收[\s\S]*会员储值[\s\S]*散客订场[\s\S]*散客单次课程[\s\S]*课包专项/,'ledger summary should use the requested owner-facing finance metrics');
assert.match(source,/总核销金额 \/ 总实收占比[\s\S]*会员实收 vs 会员已核销[\s\S]*课包实收 vs 课包已核销/,'ledger summary cards should show requested captions');
...
assert.match(source,/成交笔数[\s\S]*实收合计[\s\S]*会员储值[\s\S]*散客订场[\s\S]*散客课程[\s\S]*课包收入/,'revenue stats should show requested income card labels');
assert.match(source,/流水条数[\s\S]*确收合计[\s\S]*散客订场核销[\s\S]*课包消课核销[\s\S]*会员耗卡核销[\s\S]*散客课程核销/,'recognized stats should show requested recognized card labels');
