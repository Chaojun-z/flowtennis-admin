...
assert.doesNotMatch(source,/function financeDeferredRowsFromUnifiedLedger\(/,'prepaid balance should not build pending rows in the frontend');
assert.match(functionSource(source,'financePrepaidRows'), /financePrepaidUnifiedRows\(\)/, 'prepaid balance rows should read backend unified prepaid view');
...
assert.match(functionSource(source,'renderFinancePrepaidBalance'), /financePrepaidUnifiedSummary\(\)/, 'prepaid balance stats should use backend unified prepaid summary');
assert.doesNotMatch(functionSource(source,'renderFinancePrepaidBalance'), /financeLessonDeferredRows|financeStoredValueRows|reduce\(|financeDeferredRowsFromUnifiedLedger/, 'prepaid balance stats should not call legacy pending calculators or summarize locally');
