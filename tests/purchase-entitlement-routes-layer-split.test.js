...
const routesPath = path.join(repoRoot, 'server/purchase-entitlement-routes.js');

assert.ok(fs.existsSync(routesPath), 'server/purchase-entitlement-routes.js should own purchase and entitlement write routes');

const routesSource = fs.readFileSync(routesPath, 'utf8');

assert.match(apiSource, /require\('\.\.\/server\/purchase-entitlement-routes'\)/, 'api/index.js should import purchase entitlement routes');
