...
assert.match(html, /weekly-template-nav[\s\S]*Dashboard[\s\S]*Revenue[\s\S]*Private Course[\s\S]*Court Usage/, 'top navigation should mirror the template segmented menu and jump to report sections');
assert.match(html, /class="hero-overview-card"[\s\S]*class="hero-overview-item"[\s\S]*总收入[\s\S]*总场地利用率[\s\S]*总私教课人数/, 'hero lifetime metrics should use the template large right-side KPI card');
assert.match(html, /class="weekly-chip-row"[\s\S]*本周收入[\s\S]*本周已入账[\s\S]*本周场地利用率[\s\S]*本周教练课时[\s\S]*本周线索数/, 'weekly summary metrics should use the template second-row chip cards');
assert.match(html, /USER RETENTION MATRIX[\s\S]*核心客群生命周期存留分析[\s\S]*起始批次[\s\S]*<span>W1<\/span>[\s\S]*<span>W5<\/span>[\s\S]*08\.27 Cohort[\s\S]*\* 最近 5 周场地利用率矩阵/, 'daily court utilization should copy the template retention matrix structure');
...
assert.match(html, /data-edit-key="section.revenue.title"[\s\S]*data-edit-key="section.course.title"[\s\S]*data-edit-key="course.totalPeople.label"[\s\S]*data-edit-key="court.matrix.title"[\s\S]*data-edit-key="conversion.source.0.source"/, 'weekly report should make section titles, metric labels, charts and table cells editable');
