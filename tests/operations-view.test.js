...
assert.match(operationsSource, /function operationsBuildConversionTrendRows[\s\S]*operationsTrendToday\(\)[\s\S]*row\._trendDate <= today/, 'conversion trend rows should exclude future lead dates before drawing sparklines');
...
assert.match(stylesSource, /operations-conversion-kpi-sparkline \.operations-kpi-dot\{opacity:1/, 'conversion KPI sparklines should show visible point markers');
...
assert.match(operationsSource, /function operationsCoachSparklineValues[\s\S]*if \(source\.length <= 31\) return source;[\s\S]*targetCount = 24[\s\S]*Math\.round\(index \* step\)/, 'KPI sparklines should keep week/month daily points and only compress dense all-time trends');
...
assert.match(operationsSource, /operationsTrendChangeText[\s\S]*first[\s\S]*last[\s\S]*change/, 'KPI cards should show a visible change value from first point to last point');
...
assert.match(operationsSource, /operationsTrendChangeText[\s\S]*return operationsKpiSignedValueText\(change, key\)/, 'KPI change values should only show the compact numeric delta');
