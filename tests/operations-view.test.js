...
assert.match(chartsSource, /operations-funnel-stats[\s\S]*上一步转化 \$\{fmt\(stepRate\)\}%/, 'progress funnel should show only the previous-step conversion rate on the left side');
...
assert.match(operationsSource, /function operationsCourtTrendValues[\s\S]*if \(!operationsShouldShowTrend\(\)\) return \[\];[\s\S]*return values\.length >= 2 \? values : \[\]/, 'court KPI cards should draw only real multi-point trends and hide one-day or missing trends');
...
assert.match(chartsSource, /operations-funnel-stats[\s\S]*上一步转化 \$\{fmt\(stepRate\)\}%/, 'conversion funnel rows should focus on previous-step conversion rate');
...
assert.match(stylesSource, /operations-court-kpi\{[^}]*min-height:136px[^}]*display:flex;flex-direction:column/, 'court KPI cards should use the same card layout as coach KPI cards');
...
assert.match(chartsSource, /buildOperationsCoachMatrixChartOption[\s\S]*grid: \{ left: 8, right: 8[\s\S]*containLabel: true[\s\S]*axisLabel: \{ formatter: value => operationsCoachRevenueAxisLabel\(value\), color: '#A19080', fontSize: 11, margin: 8 \}/, 'coach matrix should keep the full chart inside the tighter content area without y-axis overlap');
assert.match(chartsSource, /buildOperationsCoachCapabilityChartOption[\s\S]*grid: \{ left: 8, right: 8[\s\S]*containLabel: true[\s\S]*yAxis: \{ type: 'value'[\s\S]*axisLabel: \{ formatter: '\{value\}%', color: '#A19080', fontSize: 11, margin: 8 \}/, 'coach capability matrix should keep the full chart inside the tighter content area without y-axis overlap');
...
assert.match(chartsSource, /function operationsCoachMatrixSymbolSize[\s\S]*Math\.max\(14[\s\S]*Math\.min\(46/, 'coach matrix bubbles should stay more compact');
assert.match(chartsSource, /function operationsCoachCapabilitySymbolSize[\s\S]*Math\.max\(14[\s\S]*Math\.min\(44/, 'coach capability bubbles should stay more compact');
...
assert.match(chartsSource, /buildOperationsCoachParetoChartOption[\s\S]*yAxis: \{ type: 'category'[\s\S]*data: source\.map\(row => operationsCoachShortName\(row\.coach\)\)[\s\S]*xAxis: \[/, 'coach contribution chart should use horizontal bars so names do not crowd the bottom axis');
assert.match(chartsSource, /xAxis: \[[\s\S]*\{ type: 'value'[\s\S]*\{ type: 'value', min: 0, max: 100/, 'coach contribution chart should keep amount and share axes for the horizontal layout');
