...
assert.match(operationsSource, /renderConversionRetentionModule[\s\S]*留存趋势[\s\S]*课包留存趋势[\s\S]*订场留存趋势[\s\S]*订场会员留存趋势[\s\S]*operations-retention-grid/, 'conversion page should render three peer visual retention cards');
...
assert.match(operationsSource, /renderConversionAttributeModule[\s\S]*人群画像[\s\S]*成交画像图[\s\S]*留存画像图/, 'conversion page should render people profiles as two visual ranking cards');
assert.match(operationsSource, /operationsPersonaBars[\s\S]*operations-persona-bar/, 'student personas should use horizontal visual bars instead of a card wall');
