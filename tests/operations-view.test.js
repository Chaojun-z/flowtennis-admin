...
assert.match(coachDashboardSource, /教练课时详细统计[\s\S]*教学课时\/人数[\s\S]*体验课转化[\s\S]*课程结构（课时）[\s\S]*课程反馈（课次）/, 'coach detail table should use the compact coach metric columns');
...
assert.match(operationsSource, /function operationsCoachUsedHoursCell[\s\S]*teachingStudentCount[\s\S]*\$\{operationsCoachHourText\(teachingHours\)\} \/ \$\{fmt\(teachingStudentCount\)\}[\s\S]*operationsCoachDetailChangeText\(row\.usedHoursComparison\)/, 'coach detail table should show exact teaching hours and unique student count with spaced slash formatting');
...
assert.match(operationsRuntime.operationsCoachUsedHoursCell({ teachingHours: 5.25, teachingStudentCount: 8, usedHoursComparison: { mode: 'previous_period', changeValue: -4.25 } }), /5\.25 \/ 8[\s\S]*-4\.25 课时/, 'coach detail workload cell should keep exact decimal lesson hours and compact change copy');
