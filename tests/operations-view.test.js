...
assert.match(operationsSource, /function operationsCoachTrialConversionText[\s\S]*trialConverted[\s\S]*trialBase[\s\S]*trialConversionRate[\s\S]*\$\{fmt\(converted\)\}\/\$\{fmt\(total\)\} \$\{fmt\(rate\)\}%/, 'coach detail table should render trial conversion as converted over total with rate');
