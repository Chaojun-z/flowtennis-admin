...
assert.match(coachDashboardSource, /教练课时详细统计[\s\S]*课时\/上课人次[\s\S]*体验课转化[\s\S]*课程结构（课时）[\s\S]*课程反馈（课次）/, 'coach detail table should use the compact coach metric columns');
assert.doesNotMatch(coachDashboardSource, /校区结构（课时）/, 'coach detail table should not show campus structure anymore');
assert.match(coachDashboardSource, /<colgroup>[\s\S]*width:14%[\s\S]*width:16%[\s\S]*width:12%[\s\S]*width:42%[\s\S]*width:16%[\s\S]*<\/colgroup>/, 'coach detail table should use five proportional non-pixel columns that give course structure more room');
...
assert.match(operationsSource, /function operationsCoachUsedHoursCell[\s\S]*teachingAttendanceCount[\s\S]*teachingStudentCount[\s\S]*\$\{operationsCoachHourText\(teachingHours\)\} \/ \$\{fmt\(teachingAttendanceCount\)\}[\s\S]*operationsCoachDetailChangeText\(row\.usedHoursComparison\)/, 'coach detail table should show exact teaching hours and attendance count with spaced slash formatting');
...
assert.match(operationsRuntime.operationsCoachUsedHoursCell({ teachingHours: 5.25, teachingAttendanceCount: 9, teachingStudentCount: 8, usedHoursComparison: { mode: 'previous_period', changeValue: -4.25 } }), /5\.25 \/ 9[\s\S]*-4\.25 课时/, 'coach detail workload cell should keep exact decimal lesson hours and compact attendance copy');
...
assert.strictEqual(operationsRuntime.operationsCoachCourseMixText({ courseMix: [{ type: '私教课', hours: 21.25 }, { type: '专项课', hours: 1.25 }, { type: '陪打', hours: 5 }] }), '私教课 21.25 | 专项课 1.25 | 陪打 5', 'coach course mix should keep exact decimals and include special courses');
