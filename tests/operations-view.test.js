...
assert.match(functionBody(stateSource, 'currentOperationsPageView'), /typeof operationsActiveTab!==['"]undefined['"][\s\S]*operationsActiveTab===['"]coach['"]/, 'coach operations first load should use the runtime active tab before relying on localStorage');
assert.match(functionBody(stateSource, 'currentOperationsPageView'), /querySelector\(['"]\.sb-item\[data-nav-page="operations"\]\[data-operations-tab="coach"\]\.active['"]\)/, 'coach operations first load should still detect the active coach menu when localStorage is unavailable');
...
assert.match(coachDashboardSource, /教练课时详细统计[\s\S]*课时数\/环比课时数[\s\S]*体验课转化率[\s\S]*课程结构/, 'coach detail table should use the requested coach lesson detail columns');
assert.doesNotMatch(coachDashboardSource, /校区结构（课时）/, 'coach detail table should not show campus structure anymore');
assert.match(coachDashboardSource, /<colgroup>[\s\S]*width:18%[\s\S]*width:20%[\s\S]*width:18%[\s\S]*width:44%[\s\S]*<\/colgroup>/, 'coach detail table should use four proportional non-pixel columns that give course structure more room');
...
assert.doesNotMatch(coachDashboardSource, /课程反馈（课次）|已反馈|未反馈|时间段/, 'coach detail table should remove feedback and time-band distribution columns');
assert.match(operationsSource, /function operationsCoachHourText[\s\S]*String\(number\)[\s\S]*function operationsCoachUsedHoursCell/, 'coach detail hour values should not use the global rounded formatter');
assert.match(operationsSource, /function operationsCoachPreviousHoursText[\s\S]*previousValue[\s\S]*function operationsCoachUsedHoursCell[\s\S]*teachingHours[\s\S]*usedHoursComparison[\s\S]*\$\{operationsCoachHourText\(teachingHours\)\} \/ \$\{operationsCoachPreviousHoursText/, 'coach detail table should show current lesson hours over previous-period lesson hours');
assert.doesNotMatch(operationsSource, /function operationsCoachUsedHoursCell[\s\S]*teachingAttendanceCount[\s\S]*function operationsCoachTrialConversionText/, 'coach detail workload cell should not show attendance count');
assert.doesNotMatch(operationsSource, /const icon = change > 0 \? '↑' : '↓'/, 'coach detail lesson-hour change should not use raw text arrows');
assert.strictEqual(operationsRuntime.operationsCoachUsedHoursCell({ teachingHours: 5.25, teachingAttendanceCount: 9, teachingStudentCount: 8, usedHoursComparison: { mode: 'previous_period', previousValue: 4.25, changeValue: 1 } }).replace(/<[^>]+>/g, ''), '5.25 / 4.25', 'coach detail workload cell should keep exact decimal lesson hours and previous-period hours');
assert.strictEqual(operationsRuntime.operationsCoachUsedHoursCell({ teachingHours: 5, teachingStudentCount: 8, usedHoursComparison: { mode: 'none' } }).replace(/<[^>]+>/g, ''), '5 / -', 'coach detail workload cell should show dash when previous-period comparison is unavailable');
assert.strictEqual(operationsRuntime.operationsCoachTrialConversionText({ trialConverted: 5, trialBase: 8, trialConversionRate: 62.5 }), '62.5% 5/8', 'coach trial conversion should render as rate plus converted over attended people');
assert.strictEqual(operationsRuntime.operationsCoachTrialConversionText({ trialConverted: 0, trialBase: 0, trialConversionRate: 0 }), '-', 'coach trial conversion should stay empty when there are no trial attendees');
assert.strictEqual(operationsRuntime.operationsCoachCourseMixText({ courseMix: [{ type: '私教课', hours: 21.25 }, { type: '专项课', hours: 1.25 }, { type: '陪打', hours: 5 }] }), '私教课 21.25｜体验课 0｜小班课 0｜专项课 1.25｜陪打 5', 'coach course mix should show all requested course categories in a fixed order');
...
assert.match(coachDashboardSource, /operationsCoachDetailTooltipText\(operationsCoachCourseMixText\(row\)\)/, 'coach detail course structure should show full text on hover');
assert.doesNotMatch(coachDashboardSource, /operationsCoachCampusDistributionText/, 'coach detail should remove the campus distribution renderer');
...
assert.doesNotMatch(operationsSource, /operationsCoachDetailTrendIcon/, 'coach detail workload cell should not keep the old trend icon renderer');
assert.doesNotMatch(stylesSource, /operations-coach-detail-change|operations-coach-detail-icon/, 'coach detail table should remove old trend icon styles');
assert.doesNotMatch(stylesSource, /operations-coach-feedback/, 'coach detail table should remove old feedback-only styles');
...
assert.match(operationsSource, /operationsCoachChartHeader\('课程结构占比'[\s\S]*color: '#D97706'[\s\S]*color: '#0F766E'[\s\S]*color: '#E7E5E4'[\s\S]*color: '#2563EB'[\s\S]*color: '#7C3AED'/, 'coach course mix legend should match the approved stacked-bar colors');
...
assert.match(chartsSource, /function operationsCoachCourseColor[\s\S]*if \(type === '体验课'\) return '#D97706'[\s\S]*if \(type === '私教课'\) return '#0F766E'[\s\S]*if \(type === '小班课'\) return '#E7E5E4'[\s\S]*if \(type === '专项课'\) return '#2563EB'[\s\S]*if \(type === '陪打'\) return '#7C3AED'/, 'coach course mix should use the approved course colors');
