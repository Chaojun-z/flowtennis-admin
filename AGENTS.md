...
1. 顺义马坡周报、周度经营报表、周报重新生成接口属于限时生成链路，目标是读取已准备好的轻量事实字段或快照后快速组装，不得在生成过程中临时重建完整页面读模型。
2. 周报生成链路禁止直接或间接调用线索池、历史学员、在期学员、工作台等完整读模型来“顺手复用口径”，包括但不限于：
   - `buildCustomerLifecycleRows`
   - `buildTeachingStudentViews`
   - 任何会扫描 `ft_schedule`、`ft_purchases`、`ft_entitlements`、`ft_entitlement_ledger`、`ft_feedbacks` 后再拼完整学员列表的页面级读模型
3. 周报新增指标时，必须优先使用 `weeklyReportRaw` 已带出的轻字段、财务快照、经营快照或明确的预聚合结果；如果字段不够，应补充 raw 轻字段或预聚合来源，不得把页面级读模型塞进周报生成主流程。
4. 周报里需要引用“在期学员”“课包有余额”“私教课包在期人数”等口径时，只能在周报生成链路做与该指标直接相关的线性筛选，并按周报周期结束日判断有效期；不得为了一个数字重算完整在期学员页面。
5. 任何周报新增或修复指标，只要触达大表、课包、权益、学员生命周期、8 周趋势或重新生成接口，必须补反向性能测试。测试至少覆盖：
   - 大样本下仍走快路径
   - 重新引入完整读模型调用时测试失败
   - `weeklyReportRaw` 缺少关键轻字段时测试失败
   - 当前周、上周、8 周趋势重复计算不会把单个指标放大成多次重建
6. 删除、绕开、放宽周报性能锁测试，视为破坏周报生成性能治理；不得以“先保证口径”“后面再优化”为理由合并。

### 1.3.8 全平台核心列表读模型上线与防复发硬规则
...
5. 数据比对、异常排查、修复后复现这三类数据任务，默认走“数据问题快检”：
   - 先只看时间窗、批次号、单号、页面和差异清单；
   - 先比对差异，再定位差异命中的少量记录；
   - 不允许先扫全表、先翻全库、先把所有关联页都读一遍。
6. 数据任务连续两轮还没定位时，先停下并把差异和已查范围给用户，不要无限扩大搜索面。
